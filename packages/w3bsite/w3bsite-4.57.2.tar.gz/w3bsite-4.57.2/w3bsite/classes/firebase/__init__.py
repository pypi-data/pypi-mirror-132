...
class FireStore(object):
	...
	def list(self, reference):
		...
		try:
			...
			success = False
		if not success:
			return dev0s.response.error(f"Failed to load document [{reference}]: {error}.")
	...
	def save(self, reference, data):
		...
		for attempt in range(max_attempts):
			...
			if success:
				...
				if "none stream removed" in str(error).lower() and attempt < max_attempts-1:
					time.sleep(0.5)
				else:
					return dev0s.response.error(f"Failed to save document [{reference}]: {error}.")
	def delete(self, reference):
		...
		if success:
			...
			return dev0s.response.error(f"Failed to delete document [{reference}]: {error}.")
	# system functions.
