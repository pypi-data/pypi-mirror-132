...
class iCfgUpdLastChange(mettle.io.ISerializable):
    ...
    def _serialize(self, _w: mettle.io.IWriter, _oname: str = None):
        ...
        _w.write_string("idcrit", self.idcrit)
        _w.write_end(_oname)
    ...
    def _deserialize(self, _r: mettle.io.IReader, _oname: str = None):
        ...
        self.idcrit = _r.read_string("idcrit")
        _r.read_end(_oname)

    def _copy_from(self, rec: "iCfgUpdLastChange"):
        """
        Copies the column from the rec into this record.

        :param rec: The source record.
        """
        self.idcrit = rec.idcrit
    ...
    def _cache_davs(dvc=None):
        ...
        dvc.add_targ("idcrit", mettle.lib.Dav(mettle.lib.Dav.eDavType.NotNull))

        return dvc

    def _get_davs(self, dvc=None):
        iCfgUpdLastChange._cache_davs(dvc)

    def __repr__(self) -> str:
        return '<Cfg [idcrit:%.32s]>' % (
            self.idcrit)
    ...
    class List(list, mettle.io.ISerializable):
        ...
        def __repr__(self) -> str:
            return '<iCfgUpdLastChange.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None):
            return iCfgUpdLastChange._cache_davs(dvc)
