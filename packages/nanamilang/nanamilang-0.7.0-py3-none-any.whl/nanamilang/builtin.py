...
class BuiltinMacros:
    ...
    def fn_macro(token_cls, tree_slice: list, env: dict, eval_function) -> list:
        ...
        # So, once we verified that user did supply correct function parameters form, we go ahead :)

        parameters = []
        for tof in parameters_form[1:]:
            if isinstance(tof, list):
                assert tof[0].dt().origin() == 'make-vector',  'fn: needs to be Vector'
                right = []
                for x in tof[1:]:
                ...
                assert tof.type() == token_cls.Identifier,   'fn: is not an Identifier'
                parameters.append((tof.dt().origin(), None))

        # Code above is responsible to handle vector (only vector for now) parameter destructuring

        name = name_token.dt().origin() if name_token else randstr()

        # Name should be taken from optionally passed name_token, otherwise randomly generated one

        fni = fn.Fn(token_cls, body_token_or_form, env, eval_function, name, parameters)
        handle = lambda args: fni.handle(tuple(args))

        # Function handle should be initialized first, and handle lambda function right after that
        ...
        # After function handle lambda function defined, assign required meta data to that object.

        payload = {name: datatypes.Function({'function_name': name,
                                             'function_reference': handle})}

        # After function handle lambda function defined, initialize corresponding Function as well

        env.update(payload)
        fni.env().update(payload)

        # To be able to call defined function by its own name (even if randomly generated), update
        # local env (which has been passed as an argument for fn macro) and function handler env :)
    ...
    def let_macro(token_cls, tree_slice: list, env: dict, eval_function) -> list:
        ...
        for [key_token_or_form, value_token_or_form] in plain2partitioned(but_first):
            ...
            if isinstance(key_token_or_form, token_cls) and key_token_or_form.type() == token_cls.Identifier:
                ...
                if not isinstance(evaluated, datatypes.Vector):
                    raise LetDestructuringError(LetDestructuringError.Reason_RightSideIsNotAVector)
                if not len(key_token_or_form[1:]) == len(evaluated.reference()):
                    raise LetDestructuringError(LetDestructuringError.Reason_LeftAndRightLensUnequal)
                for from_left, from_right in zip(key_token_or_form[1:], evaluated.reference()):
...
class BuiltinFunctions:
    ...
    def type_func(args: List[datatypes.Base]) -> datatypes.String:
        ...
        # Since base data types can not use String, so cast it manually
    ...
    def odd_func(args: List[datatypes.Numeric]) -> datatypes.Boolean:
        ...
        # Since base data types can not use Boolean, so cast it manually
    ...
    def abs_func(args: List[datatypes.Numeric]) -> datatypes.IntegerNumber:
        ...
        # Since base data types can not use IntegerNumber, cast it manually
