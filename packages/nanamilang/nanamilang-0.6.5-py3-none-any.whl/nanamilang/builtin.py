...
class BuiltinMacros:
    ...
    def measure_macro(token_cls, tree_slice: list, env: dict, eval_function) -> list:
        ...
        evaluated_value = eval_function(env, tree_slice)
        if isinstance(evaluated_value, datatypes.NException):
            return [token_cls(token_cls.Identifier, 'identity'),
                    token_cls(token_cls.Proxy, evaluated_value)]
    ...
    def if_macro(token_cls, tree_slice: list, env: dict, eval_function) -> list:
        ...
        evaluated_value = eval_function(env, cond)
        if isinstance(evaluated_value, datatypes.NException):
            return [token_cls(token_cls.Identifier, 'identity'),
                    token_cls(token_cls.Proxy, evaluated_value)]
        return true_branch if evaluated_value.truthy() is True else else_branch
    ...
    def and_macro(token_cls, tree_slice: list, env: dict, eval_function) -> list:
        ...
        for condition in tree_slice:
            ...
            evaluated_value = eval_function(env, condition)
            if isinstance(evaluated_value, datatypes.NException):
                return [token_cls(token_cls.Identifier, 'identity'),
                        token_cls(token_cls.Proxy, evaluated_value)]
            if not evaluated_value.truthy():
                return condition
