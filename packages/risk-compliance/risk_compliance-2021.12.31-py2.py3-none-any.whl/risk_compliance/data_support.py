...
def run_hive(sql,host1,host2,port,username,message,url,users=None):
    try:
        ...
        for i in sqllist:
            ...
            print('sql执行总耗时大于3分钟: %s'%round(end-start,2) if end-start>180 else 'sql执行总耗时: %s'%round(end-start,2))
            try:
                res = cursor.fetchall()
            except:
                pass
...
class Config(object):
    ...
    def __init__(self, file="/deployments/hive.conf", *args, **kwargs):
        ...
        self.conf.read(file)
        self._attribute['host'] = self.conf.get("hive", "host")
        self._attribute['backup_host'] = self.conf.get("hive", "backup_host")
