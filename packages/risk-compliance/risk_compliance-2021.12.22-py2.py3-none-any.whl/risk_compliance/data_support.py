...
def run_hive(sql,host1,host2,port,username,message,url,users=None):
    try:
        ...
        for i in sqllist:
            ...
            print('sql执行开始时间: %s'%time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(end)))
            print('sql执行总耗时: %s',%round((end-start),2))
            try:
                res = cursor.fetchall()
            except:
                pass
...
def send_alert(message,url=None,users=None):
    ...
    if not users:
        ...
        response=requests.post(config.get_config('alarm_url') if not url else url,data=json.dumps(data),headers=headers)
    else:
        alarm_host = config.get_config('alarm_host')
        users_url = alarm_host+"?title=数据支撑告警&module=风控合规&users=" \
                  + users + "&mesg=" + message
...
class Config(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        conf.read("/deployments/hive.conf")
        self._attribute['host'] = conf.get("hive", "host")
        self._attribute['backup_host'] = conf.get("hive", "backup_host")
        self._attribute['username'] = conf.get("hive", "username")
        self._attribute['port'] = conf.get("hive", "port")
        
        self._attribute['bucket'] = conf.get("s3", "bucket")
        self._attribute['path'] = conf.get("s3", "path")
        
        self._attribute['database'] = conf.get("db", "database")
        
        self._attribute['alarm_url'] = conf.get("alarm", "url")
        
        self._attribute['alarm_host'] = conf.get("alarm", "host")
