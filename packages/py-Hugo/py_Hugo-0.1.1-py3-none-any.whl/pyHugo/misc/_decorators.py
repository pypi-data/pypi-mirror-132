...
from .. import DUAL_HNDLR, HNDLR, LOGS, SUDO_HNDLR, asst, hugo_bot, udB
...
from ..version import hugo_version as hg_ver
...
def hugo_cmd(allow_sudo=should_allow_sudo(), **args):
    ...
    def decorator(func):
        ...
        def doit(mode):
            async def wrapper(hg):
                chat = hg.chat
                if mode in ["dualmode", "official", "sudo"]:
                    if not hg.out and mode in ["dualmode", "sudo"]:
                        if not allow_all and str(hg.sender_id) not in owner_and_sudos():
                            return
                        if fullsudo and not is_fullsudo(hg.sender_id):
                            return await eod(
                                hg, "`Full Sudo User Required...`", time=15
                            )
                    if hasattr(chat, "title"):
                        if (
                            "#noub" in chat.title.lower()
                            and not (chat.admin_rights or chat.creator)
                            and not (hg.sender_id in DEVLIST)
                        ):
                            return
                    if admins_only:
                        if hg.is_private:
                            return await eod(hg, "`Use this in group/channel.`")
                        if not (chat.admin_rights or chat.creator):
                            return await eod(hg, "`I am not an admin.`")
                elif mode == "manager":
                    if not allow_pm and hg.is_private:
                        return
                    elif not (await admin_check(hg)):
                        return
                ...
                if groups_only and hg.is_private:
                    return await eod(hg, "`Use this in group/channel.`")
                try:
                    await func(hg)
                    ...
                    return await eod(hg, "`Inline Locked In This Chat.`")
                except (ChatSendMediaForbiddenError, ChatSendStickersForbiddenError):
                    return await eod(
                        hg, "`Sending media or sticker is not allowed in this chat.`"
                    )
                except (BotMethodInvalidError, UserIsBotError) as boterror:
                    return await eod(hg, str(boterror))
                except (
                    MessageIdInvalidError,
                    MessageNotModifiedError,
                    MessageDeleteForbiddenError,
                    ...
                    await asst.send_message(
                        ...
                        buttons=[
                            Button.url("Bot", "t.me/HugoStringBot?start="),
                            Button.url(
                                "Repl",
                                "https://replit.com/@TeamHugoX/HugoStringSession",
                            ),
                        ],
                    )
                    exit()
                    ...
                    ftext = "**Hugo Client Error:** `Forward this to` @HugoSupport\n\n"
                    ftext += "**Py-Hugo Version:** `" + str(pyver)
                    ftext += "`\n**Hugo Version:** `" + str(hg_ver)
                    ftext += "`\n**Telethon Version:** `" + str(telever) + "`\n\n"
                    ftext += "--------START HUGO CRASH LOG--------"
                    ftext += "\n**Date:** `" + date
                    ftext += "`\n**Group:** `" + str(hg.chat_id) + "` " + str(naam)
                    ftext += "\n**Sender ID:** `" + str(hg.sender_id)
                    ftext += "`\n**Replied:** `" + str(hg.is_reply)
                    ftext += "`\n\n**Event Trigger:**`\n"
                    ftext += str(hg.text)
                    ftext += "`\n\n**Traceback info:**`\n"
                    ...
                    if len(ftext) > 4096:
                        with BytesIO(ftext.encode()) as file:
                            ...
                            await asst.send_file(
                                int(udB["LOG_CHANNEL"]),
                                file,
                                caption="**Hugo Client Error:** `Forward this to` @HugoSupport\n\n",
                            )
                    else:
                        await asst.send_message(
                            int(udB["LOG_CHANNEL"]),
        ...
        if "official" in type:
            args["outgoing"] = True
            hugo_bot.add_event_handler(doit("official"), events.NewMessage(**args))
            if TAKE_EDITS:
                args["func"] = (
                    lambda x: not (
            ...
            if allow_sudo:
                ...
                hugo_bot.add_event_handler(doit("sudo"), events.NewMessage(**args))
                del args["outgoing"]
