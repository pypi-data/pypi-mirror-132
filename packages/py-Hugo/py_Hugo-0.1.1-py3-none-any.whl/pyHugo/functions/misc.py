...
async def randomchannel(tochat, channel, range1, range2, caption=None, client=hugo_bot):
...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="Hugo All Cmds", content=[txt])
    await eor(event, f"All Hugo Cmds : [Click Here]({t['url']})", link_preview=False)
    ...
    async with aiohttp.ClientSession() as ses:
        async with ses.post(
            ...
            if "image" in contentType:
                name = check_filename("hugo-rmbg.png")
...
async def get_insta_code(cl, username, password):
    async with asst.conversation(hugo_bot.uid, timeout=60 * 2) as conv:
