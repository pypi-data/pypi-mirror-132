...
async def metadata(file):
    ...
    try:
        ...
        data["performer"] = (
            info[0].get("Performer") or udB.get("artist") or hugo_bot.me.first_name
        )
        if len(info) >= 2:
...
async def get_chatbot_reply(message):
    ...
    req_link = chatbot_base.format(
        hugo_bot.me.first_name or "hugo user",
        message,
    )
    try:
...
def check_filename(filhg):
    if os.path.exists(filhg):
        no = 1
        while True:
            hg = "{0}_{2}{1}".format(*os.path.splitext(filhg) + (no,))
...
def telegraph_client():
    ...
    gd_name = get_display_name(hugo_bot.me)
    short_name = gd_name if len(gd_name) < 32 else "Hugo"
    profile_url = (
        f"https://t.me/{hugo_bot.me.username}" if hugo_bot.me.username else None
    )
    try:
