...
def un_plug(shortname):
    from .. import asst, hugo_bot

    try:
        for client in [hugo_bot, asst]:
...
async def safeinstall(event):
    ...
    if plug in HELP:
        ...
        for i in HELP[plug]:
            output += i
        output += "\n© @HugoProject"
        await eod(ok, f"✓ `Hugo - Installed`: `{plug}` ✓\n\n{output}")
...
def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>Hugo {hugo_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"Hugo {hugo_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
def make_html_telegraph(title, author, text):
    ...
    page = client.post(
        title=title,
        author=author,
        author_url="https://t.me/HugoProject",
        text=text,
    )
    return page["url"]
...
async def shutdown(hg, dynotype="hugo"):
    from .. import LOGS

    await eor(hg, "Shutting Down")
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            await hg.edit("`Shutting Down your app, please wait for a minute!`")
            app.process_formation()[dynotype].scale(0)
        except BaseException as e:
            LOGS.info(e)
            return await hg.edit(
...
async def heroku_logs(event):
    ...
    ok = app.get_log()
    with open("hugo-heroku.log", "w") as log:
        log.write(ok)
    await event.client.send_file(
        event.chat_id,
        file="hugo-heroku.log",
        thumb="resources/extras/hugo.png",
        caption="**Hugo Heroku Logs.**",
    )

    os.remove("hugo-heroku.log")
    await xx.delete()


async def def_logs(hg):
    await hg.client.send_file(
        hg.chat_id,
        file="hugo.log",
        thumb="resources/extras/hugo.png",
        caption="**Hugo Logs.**",
