from .util import json_util, trans_data_to_url_util, time_stamp_util, get_now_time_util, to_encrypt_util, \
    ...
    send_mail_util, xml_to_dict_util, random_string_util, telnet_util, Faker, format_quot_str_util, dict_to_xml_util
...
class Tool(object):
    ...
    # 发送钉钉机器人消息
    def send_ding_talk_msg(self, msg, at_all=None):
        if at_all is None:
            at_all = []
        send_robot_msg_util(msg, 'dingTalk', ding_talk_sign_key=self.ding_talk_sign_key,
                            ding_talk_token=self.ding_talk_token, at_all=at_all)
