...
def up(
    ...
    skip_model_mount: bool = all([x for x in [gridadmin_download_url, tables_download_url]])

    for local_path, mount_setting in settings.DOCKER_MOUNT_POINTS.items():
        if skip_model_mount and mount_setting['bind'] == '/models':
    ...
    if hasattr(settings, "CONTAINER_LOG_LEVEL") and "LOG_LEVEL" not in envs:
    ...
    if gridadmin_download_url is not None:
        args.append(str(gridadmin_download_url))
    if tables_download_url is not None:
        args.append(str(tables_download_url))
