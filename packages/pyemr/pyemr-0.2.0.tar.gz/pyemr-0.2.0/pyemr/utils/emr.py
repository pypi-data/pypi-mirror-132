...
def wait_else_cancel(cluster_id, step_id, desc="Waiting"):
    """Faits on a step to complete. If interupted it cancels the step.
...
def get_cluster_states(state_pattern: str):
    """Takes the state str pattern then returns the matching erm states.
...
def ssm_bash(cmd):
    """Run a bash comman on a target ec2 instance.
...
def list_steps(cluster_name, n, step_name_pattern, state_patterm) -> pd.DataFrame:
    ...
    Returns:
      pd.DataFrame: Dataframe containg the mathcing steps.
...
def get_last_submitted_step(cluster_name, name_pattern, state):
    """Get the last summited step for the current project.
...
def get_step_id(step_id: str, cluster_name: str, name_pattern: str, state: str):
    """Returns the latest stepid if step_id is none
