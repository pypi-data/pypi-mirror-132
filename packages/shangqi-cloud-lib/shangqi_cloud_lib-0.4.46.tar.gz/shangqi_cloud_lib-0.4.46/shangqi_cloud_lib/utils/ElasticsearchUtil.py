...
def format_es_return(bool_must_list=None, param_list=None, route_key="", sort_list=None, size=10, offset=0,
    ...
    result_list = []
    for r in res["hits"]:
        result_list.append(format_es_param_result(r, param_list, is_need_decrypt_oralce))
    result_dict = {
...
def format_es_param_result(r, param_list, is_need_decrypt_oralce):
    result_dict = {}
    if param_list:
        for key in param_list:
            is_need_decrypt = False
            if config.aes_dict and config.aes_dict.get(key):
                is_need_decrypt = True
            key_list = key.split(".")
            value = r["_source"]
            for key in key_list:
            ...
            if result_dict.get(key) is None:
                if value:
                    if is_need_decrypt:
                        if isinstance(value, list):
                            value = [encrypt_oracle(str(i)) for i in value]
                        else:
                            value = encrypt_oracle(str(value))
                        value = value
