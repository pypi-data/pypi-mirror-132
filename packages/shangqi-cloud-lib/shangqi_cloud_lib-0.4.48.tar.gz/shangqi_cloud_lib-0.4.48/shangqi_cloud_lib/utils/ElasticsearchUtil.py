...
def format_company_id_en(value, route_key):
    if route_key in ["core.patent", "core.patent_lite"]:
        for r in value:
            for data in r.get("tags", {}).get("proposer_type", []):
                data["code"] = encrypt_oracle(data["code"])
    elif route_key in ["core.research_institution"]:
        for r in value:
            for data in r.get("tags", {}).get("support_unit", []):
                data["id"] = encrypt_oracle(data["id"])
    elif route_key in ["code.investment"]:
        for r in value:
            for data in r.get("tags", {}).get("invest_company", []):
...
def format_es_param_result(r, param_list, is_need_decrypt_oralce, is_need_es_score, route_key):
    ...
    if param_list:
        for key in param_list:
            key_list = key.split(".")
            value = r["_source"]
            format_company_id_en(value, route_key)
            for key in key_list:
