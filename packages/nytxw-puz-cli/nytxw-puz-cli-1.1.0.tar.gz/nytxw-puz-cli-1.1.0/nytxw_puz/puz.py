...
class Puzzle:
    ...
    def load(self, data):
        ...
        if not s.seek_to(ACROSSDOWN, -2):
            raise PuzzleFormatError(
                "Data does not appear to represent a "
    ...
    def blacksquare(self):
        return (
            BLACKSQUARE2 if self.puzzletype == PuzzleType.Diagramless else BLACKSQUARE
        )
    ...
    def unlock_solution(self, key):
        if self.is_solution_locked():
            unscrambled = unscramble_solution(
                self.solution,
                self.width,
                self.height,
                key,
    ...
    def lock_solution(self, key):
        if not self.is_solution_locked():
            ...
            self.scrambled_cksum = scrambled_cksum(
                self.solution,
                self.width,
                self.height,
                ignore_chars=self.blacksquare(),
...
class Rebus:
    def __init__(self, puzzle):
        ...
        self.solutions = dict(
            (int(item[0]), item[1]) for item in parse_dict(solutions_str).items()
        )
        self.fill = dict(
...
def scrambled_cksum(
    scrambled, width, height, ignore_chars=BLACKSQUARE, encoding=ENCODING
):
...
def shift(s, key):
    ...
    return "".join(
        atoz[(atoz.index(c) + key[i % len(key)]) % len(atoz)] for i, c in enumerate(s)
    )
