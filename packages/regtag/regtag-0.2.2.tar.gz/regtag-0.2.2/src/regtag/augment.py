...
def get_random_oov():
    ...
    if oov_dict is None:
        oov_dict = []
        with open('./oov_datasets.json', 'r', encoding='utf-8') as file:
            for line in file:
...
def get_random_abb(val):
    ...
    return abb_data[val], abb_data[val]
...
def get_random_decimal(min_num, max_num, read_single=False):
    ...
    if is_negative:
        ...
        return str(number), "{} {}".format(random.choice(['âm']), convert.to_vn_str((str(number * -1))))

    if read_single:
        return str(number), "{}".format(' '.join([convert.to_vn_str(i) for i in list(str(number))]))
    return str(number), "{}".format(convert.to_vn_str(str(number)))


def get_random_float(max_int, max_real):
    min_num_int, max_num_int = -(10 ** max_int - 1), 10 ** max_int - 1
    ...
    return "{}{}{}".format(int_number[0], sep[0], real_number[0]), "{} {} {}".format(int_number[1], sep[1],
                                                                                     real_number[1])
...
def get_random_date():
    ...
    return random.choice(candidates + candidates_single + day_only + month_only + year_only)
...
def get_random_scale(val=None):
    scale_dict = {"%": "phần trăm", "$": "đô la", "euro": random.choice(["êu rô", "ê rô"]),
                  ...
                  "kb": random.choice(["ki lô bít", "ki lô bai"]), "kg": "kí lô gam",
                  "mg": "mi li gam", "m3": "mét khối", "km3": "kí lô mét khối", "dm3": "đề xi mét khối",
                  "cc": "xen ti mét khối", "cm3": "xen ti mét khối", "m2": "mét vuông", "km2": "kí lô mét vuông",
                  "dm2": "đề xi mét vuông", "cm2": "xen ti mét vuông", "mm2": "mi li mét vuông", "km": "kí lô mét",
...
def get_random_num_scale():
    num_int = get_random_decimal(1, 1000)
    num_float = get_random_float(4, 4)
    ...
    return random.choice(candidates + candidates_2_scale)


if __name__ == "__main__":
    # get_random_decimal(-10000, 10000)
    # get_random_float(4, 3)
    # get_random_num_scale()
    # get_random_date()
    # get_random_time()
    # get_random_id()
