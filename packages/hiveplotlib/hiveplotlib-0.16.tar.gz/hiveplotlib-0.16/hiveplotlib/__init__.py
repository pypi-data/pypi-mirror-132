...
def hive_plot_n_axes(node_list: List, edges: np.ndarray or list, axes_assignments: List,
    ...
    Edges directed counterclockwise will be drawn as solid lines by default. Clockwise edges will be drawn as dashed
    ...
    :param ccw_edge_kwargs: kwargs for edges going counterclockwise. Default ``None`` specifies a dashed line.
    ...
    for i, axis_name in enumerate(axes_names):
        ...
        for idx, e in enumerate(edges):
            ...
            hp.connect_axes(edges=e, axis_id_1=first_axis_name, axis_id_2=next_axis_name,
                            a2_to_a1=False, **collated_kwargs_cw)
            ...
            hp.connect_axes(edges=e, axis_id_1=first_axis_name, axis_id_2=next_axis_name,
                            a1_to_a2=False, **collated_kwargs_ccw)
...
class P2CP:
    ...
    def set_axes(self, columns: List, angles: List or None = None,
                 vmins: List or None = None, vmaxes: List or None = None,
    ...
    def build_edges(self, indices: List or str = "all", tag: Hashable or None = None,
...
def p2cp_n_axes(data: pd.DataFrame,
                indices: List or str = "all", split_on_column: Hashable or None = None,
    ...
    a column name to the ``split_on_column`` parameter, data will be added in chunks according to the unique values in
    that column. Each subset of ``data`` corresponding to a unique column value will be given a separate tag, with the
    ...
    value in the list of lists in ``indices``. If instead ``split_on_column`` was specified as a column value, then tags
    will be the unique values in the specified column. Regardless of splitting methodology, existing tags can be found
    under the returned ``P2CP.tags``.
    ...
    :param split_on_column: column name from ``data``. If specified, the resulting ``P2CP`` instance will split data
        according to unique values with respect to this column, with each subset of data given a tag of the unique
        column value corresponding to each subset. When specified, this column will be excluded from consideration if
        ``axes`` is ``None``. Note: this subsetting can only be run when ``indices="all"``.
    :param axes: list of ``Hashable`` column names in ``data``. Each column name will be assigned to a separate axis in
        ...
        dataframe, unless ``split_on_column`` is not ``None``, in which case that specified column will be excluded from
    ...
    :param indices_list_kwargs: list of dictionaries of kwargs for each element of ``indices`` when ``indices`` is a
        list of lists or ``split_on_column`` is not ``None``. The ith set of kwargs in ``indices_list_kwargs`` will only
        be applied to index values corresponding to the ith list in ``indices`` or to index values which have the ith
        unique value in a sorted list of unique values in ``split_on_column``. Default ``None`` provides no additional
        kwargs. Note, this list must be same length as ``indices`` or the same number of values as the number of unique
        values in ``split_on_column``.
    :return: ``P2CP`` instance.
    """
    ...
    if indices == "all":
        if split_on_column is not None:
            split_dict = indices_for_unique_values(df=data, column=split_on_column)
            ...
            if indices_list_kwargs is None:
                ...
                assert len(indices) == len(indices_list_kwargs), \
                    ...
                    f"(currently unique splits specified by `split_on_column`={split_on_column} is " + \
        ...
        if is_list_of_lists != [1]:
            assert split_on_column is None, \
                "You can only specify `split_on_column` when you are not providing list of lists inputs to `indices`."
    ...
    if axes is None:
        ...
        if split_on_column is not None:
            axes = np.delete(axes, np.where(axes == split_on_column))
            extra_warning_message += "\n(One axis was removed because it is already used by `split_on_column`)"
