...
def plot_miget_output(path_to_txt, save = False, name_saved_file = None, file_extension = "jpg", resolution = 600):
    ...
    if shunt >= perfusions.max():
        ...
        ax.plot(0.001, perfusions.max(), 'firebrick', marker=r'$\uparrow$', markersize=20)
        ax.annotate(f"Shunt = {qs_100}%", (0.0006, perfusions.max()-0.04), color = "firebrick", fontsize = 13)
    else:
    ...
    ax.plot(1000, perfusions.max(), 'cornflowerblue', marker=r'$\uparrow$', markersize=20)
    ax.annotate(f"Deadspace = {vd_vt_100}%", (40, perfusions.max()-0.04), color = "cornflowerblue", fontsize = 13)
