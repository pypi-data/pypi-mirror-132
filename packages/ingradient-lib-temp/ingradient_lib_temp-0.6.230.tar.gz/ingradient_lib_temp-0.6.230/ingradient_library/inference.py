...
class CC_Remover(object):
    ...
    def __init__(self, remain_num = 2, frequency_order = [2,3], distance = True, axis_z = 2, cc_label_counts = 5):
    ...
    def __call__(self, mask):
        ...
        if self.distance != False:
            remained_components_indices = np.argsort(count)[::-1][:self.cc_label_counts]
            center_coords = []
            for r_i in remained_components_indices:
                #center_coords.append([np.where(conected_component_mask == r_i)[0].max() - np.where(conected_component_mask == r_i)[0].min(), np.where(conected_component_mask == r_i)[1].max() - np.where(conected_component_mask == r_i)[1].min(), np.where(conected_component_mask == r_i)[2].max()- np.where(conected_component_mask == r_i)[2].min()])
