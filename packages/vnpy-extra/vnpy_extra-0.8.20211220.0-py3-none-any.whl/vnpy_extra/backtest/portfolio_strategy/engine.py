...
class BacktestingEngine(BacktestingEngineBase):
    ...
    def calculate_result_old(self) -> None:
        """"""
    ...
    def calculate_by_trade_old(self, df: DataFrame = None):
        """
    ...
    def calculate_by_trade(self, daily_stats_dic, df: DataFrame = None, trade_dict=None):
        """
        按每笔交易计算指标（建仓后平仓为一次交易）
        :return {'indicator1':'value'}:返回指标dict
        """
    ...
    def calc_statistic_by_pnl_s_old(self, stats_group=None, df: DataFrame = None) -> dict:
        """根据每日盈亏进行相关收益指标统计"""
        if df is None:
            df = self.daily_df
    ...
    def calculate_statistics_old(self, df: DataFrame = None, output=True) -> dict:
        """"""
    ...
    def _show_chart(self, df: DataFrame = None,
    ...
    def show_chart(self, df: DataFrame = None,
        ...
        for end_name in self.three_parts:
            ...
            charts_data = self._show_chart(df=df,
