...
def build_emails(from_mail, to_mail_list, subject, attach_file_paths: Optional[dict] = None,
                 block_size=1024 * 1024 * 10):
    tio = BytesIO()
    tar = tarfile.open(mode="w:gz", fileobj=tio)
    attach_file_paths = {k: v for k, v in attach_file_paths.items() if v is not None}
    if attach_file_paths:
