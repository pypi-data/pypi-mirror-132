...
#   Copyright (c) 2016 Edward Rosten <ed@edwardrosten.com>
#
...
m4_define([_AX_CXX_COMPILE_STDCXX_14_testbody], [[
  template <typename T>
    ...
    // Check for C++14 attribute support
...
AC_DEFUN([AX_CXX_COMPILE_STDCXX_14], [dnl
  ...
  m4_if([$1], [ext], [], [dnl
  if test x$ac_success = xno; then
    dnl HP's aCC needs +std=c++14 according to:
    dnl http://h21007.www2.hp.com/portal/download/files/unprot/aCxx/PDF_Release_Notes/769149-001.pdf
    dnl Cray's crayCC needs "-h std=c++14"
    for switch in -std=c++14 -std=c++1y +std=c++14 "-h std=c++14"; do
      cachevar=AS_TR_SH([ax_cv_cxx_compile_cxx14_$switch])
      AC_CACHE_CHECK(whether $CXX supports C++14 features with $switch,
                     $cachevar,
        [ac_save_CXXFLAGS="$CXXFLAGS"
