...
class WorkFlow:
    ...
    async def invoke(self, flow: Flow, event, debug=False) -> Tuple[DebugInfo, List[Log], 'Event']:
        ...
        if self.flow_history.is_acyclic(flow.id):
            ...
            await exec_dag.init(flow, self.flow_history, event, self.session, self.profile)
