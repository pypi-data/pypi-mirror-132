...
class SerializerMixin(SerializerBase):
    ...
    def from_xml(cls, xml, *args, **kwargs):
        if xmltodict is None:
            raise NotImplementedError(f"from_xml requires 'xmltodict' package to be installed.")
    ...
    def as_xml(self, use_internal_names=False):
        if xmltodict is None:
            raise NotImplementedError(f"from_xml requires 'dicttoxml' package to be installed.")
