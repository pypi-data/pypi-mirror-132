...
from ..parser import build_parser, build_parsers_map, clean_up_all
...
def cast_element(obj, *args, clean_up=False, strict=False, casters=DEFAULT_CASTERS, **kwargs):
    cast_str = guess_cast_item(obj, casters=casters, strict=strict)
    return clean_up_all(cast_str) if clean_up else cast_str


STRING_CASTERS = build_parsers_map(cast_element, elements=tuple(element for element, *_ in DEFAULT_CASTERS))


cast = build_parser(cast_element, parsers=STRING_CASTERS)
