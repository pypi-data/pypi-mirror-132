...
from typing import Any, Dict, List, Generator, Callable, Union
...
def get_words() -> Generator[str, None, None]:
    ...
    """
    with open("words.en.txt", mode="r", encoding="utf_8") as wfile:
...
def get_game() -> Union[Game, Any]:
    """
    Get the current game progress from a JSON file.
    """
    filename = f"{datetime.now(timezone('CET')).strftime('%Y%m%d')}.json"
    try:
        with open(filename, mode="r", encoding="utf_8") as gfile:
            return GameSchema().loads(gfile.read())
    except FileNotFoundError:
        # If the game file was not found, we create it
        emptygame: dict[str, List[Any]] = {"played": []}
        with open(filename, mode="w", encoding="utf_8") as gfile:
...
def main() -> None:
    ...
    if ARGS.all:
        print(f"Here is the complete list of possibilities: {list(words)}")
    else:
