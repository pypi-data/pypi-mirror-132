...
class BrowserRecorder(selenium.webdriver.remote.webdriver.WebDriver):
    ...
    def click(self, locator: Locator, **kwargs):
        if "context" not in kwargs:
            kwargs["context"] = f"Click on {locator.description}"
    ...
    def wait_for(self, locator: Locator, **kwargs):
        ...
        if "context" not in kwargs:
            kwargs["context"] = f"Wait for {locator.description}"
    ...
    def get(self, url: string, snap: bool = False, context: Optional[str] = None):
        self.execute_script("console.clear();")
        super().get(url)
        if self.autocapture and snap:
            if not context:
                context = f"Render {url}"
            self.snap(context=context)

    def snap(self, context: Optional[str] = None, is_error: bool = False):
        """
    ...
    def wrap_exception(self, message):
        ...
        try:
            ...
            if not self.pngs or not self.pngs[-1].is_error:
                try:
                    self.snap(context=f"Python error: {type(e)}", is_error=True)
...
class Waiter(WebDriverWait):
    ...
    def until(
        self, *args, capture_delay: int = 0, context: Optional[str] = None, is_error: Optional[bool] = False, **kwargs
    ) -> WebElement:
        """
        ...
        try:
            ...
            if self.__driver.autocapture:
                ...
                self.__driver.snap(context=context, is_error=not found)
        return element
