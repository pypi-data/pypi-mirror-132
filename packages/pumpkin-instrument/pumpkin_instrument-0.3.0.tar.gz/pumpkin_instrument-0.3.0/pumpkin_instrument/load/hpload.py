...
class _HP606XBContext(Load):
    ...
    def _validate_channel(self, channel):
        ...
        if channel >= self.num_channels:
            raise ValueError(
                f'There is no #{channel + 1} channel on the {self.name}')
    ...
    def set_output_state(self, channel: int, mode: LoadMode, value: float):
        ...
        if mode == LoadMode.Current:
            ...
            raise ValueError(
                f'Load mode {mode} is not supported on the HP 6060B.')
    ...
    def get_load_voltage(self, channel: int) -> float:
        ...
        """
        self._write_cmd('VOLT?')
        resp = str(
            self.controller.read_until(
                self.address,
                _HP606XB_TERMINATOR.encode('ascii')),
            'ascii')
...
def HP606XBInstrument(load_type):
    ...
    try:
        ...
        # This is really a type error, user specified power supply that doesnt
        # exist.
        raise TypeError(
            f'{load_type} is not apart of HP662XType or HP663XType.')
    ...
    def wrap(c):
        class _HP66XXA(Instrument, c):
            ...
            def __init__(self, controller: PrologixController,
                         gpib_address: int):
                """
