...
def float_or_none(value):
    """Returns float of value if possible, None if not"""
    try:
        return float(value)
...
class _KoradKD3005PContext(PowerSupply):
    def __init__(self, ser: serial.Serial):
        ...
        self.ser.write('VSET1:0.0'.encode('ascii'))
        time.sleep(KORAD_DELAY_TIME)
        self.ser.write('ISET1:0.0'.encode('ascii'))
        time.sleep(KORAD_DELAY_TIME)
        self.ser.write('OUT0'.encode('ascii'))
        time.sleep(KORAD_DELAY_TIME)
    ...
    def get_output_voltage(self, channel: int) -> float:
        ...
        """
        self.ser.write('VOUT1?'.encode('ascii'))
        time.sleep(KORAD_DELAY_TIME)
    ...
    def get_output_current(self, channel: int) -> float:
        ...
        """
        self.ser.write('IOUT1?'.encode('ascii'))
        time.sleep(KORAD_DELAY_TIME)
        result = read_korad_string(self.ser, 5)
        return float_or_none(result)
    ...
    def close(self):
        ...
        self.ser.write('VSET1:0'.encode('ascii'))
        time.sleep(KORAD_DELAY_TIME)
        self.ser.write('ISET1:0'.encode('ascii'))
        time.sleep(KORAD_DELAY_TIME)
        self.ser.write('OUT0'.encode('ascii'))
