...
class TreeView(QTreeWidget,DragDrop):
    ...
    def __init__(self, parent=None):
        ...
        self.menu.addAction(folderIcon,'Add Module',self.mainWindow.addModuleDialogue)
    ...
    def mousePressEvent(self, event):
        ...
        if si:selectedItems = si
        elif sm:selectedItems = sm
        else:return True
        if event.buttons() == Qt.LeftButton:
            mData = QMimeData()
            urls=[]
    ...
    def makeIco(self):self.mainWindow.makeIco(self.selectedImages())
    def tint(self):self.mainWindow.tintImagesDlg(self.selectedImages(),0)
    def multiply(self):self.mainWindow.tintImagesDlg(self.selectedImages(),1)
    def restore(self):self.mainWindow.restoreImages(self.selectedImages())
    def setIcon(self):self.mainWindow.setIcon(self.currentItem())

    def open(self):
        if self.popup == 'module':self.mainWindow.openModule(self.currentItem())
        if self.popup == 'class':self.mainWindow.openClass(self.currentItem())

    def delete(self):
        if self.popup == 'qrc': self.mainWindow.deleteQrcs(self.selectedQrcs())
        if self.popup == 'prefix': self.mainWindow.deletePrefixes(self.selectedPrefixes())
        if self.popup == 'image': self.mainWindow.deleteImages(self.selectedImages())
        if self.popup == 'module': self.mainWindow.deleteModules()

    def rename(self):
        if self.popup == 'qrc': self.mainWindow.renameQrc(self.currentItem())# rename prefix
        if self.popup == 'prefix': self.mainWindow.addPrefix(add=False)# rename prefix
        if self.popup == 'image': self.mainWindow.renameImage(self.currentItem())# rename prefix
        if self.popup == 'module': self.mainWindow.renameModule(self.currentItem())# rename prefix
    ...
    def urlType(self,event):
        ...
        for url in self.getUrls(event):
            ...
            if ext not in extList and ext not in self.imgTypes and len(extList): return 'multiple'
            extList.append(ext)
        if all([e=='.py' for e in extList]):return 'module'
        if all([e=='.qrc' for e in extList]):return 'qrc'
        if all([e in self.imgTypes for e in extList]):return 'img'
    ...
    def dragMoveEvent(self, event):
        ...
        if not urlType or event.source():
            event.ignore()
            return
        elif (urlType=='img' or urlType=='qrc') and self.objectName() != 'qrcTree':
            event.ignore()
            return
        ...
        # De-Select items
        for item in self.selectedItems():item.setSelected(0)
        shouldSelect = []
        ...
        if hovered:
            if self.objectName() == 'qrcTree':
                ...
                if self.prefixParent:
                    ...
                    self.mainWindow.ui.statusbar.showMessage('A new prefix will be created!')
    ...
    def dropEvent(self, event):
        ...
        if urlType and event.source() is None:
            ...
            if urlType=='qrc':self.droppedQrc.emit(self.getUrls(event))
            if urlType=='module':self.droppedModule.emit(self.getUrls(event))
        else:
            event.ignore()
