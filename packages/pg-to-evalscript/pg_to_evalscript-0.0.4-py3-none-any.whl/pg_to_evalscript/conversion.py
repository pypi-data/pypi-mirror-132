...
def generate_nodes_from_process_graph(process_graph, bands_dimension_name, temporal_dimension_name, level=1, node_varname_prefix="node_"):
    dependency_graph = get_dependencies(process_graph)
    dependents = get_dependents(dependency_graph)
    execution_order = get_execution_order(dependency_graph, dependents)
    ...
    for node_id in execution_order:
        ...
        if process_id == "load_collection":
            ...
            initial_data_name = node_varname_prefix + node_id
            continue
