...
def transform_():
    ...
    [A.Resize(128, 171, always_apply=True),A.CenterCrop(112, 112, always_apply=True),
     A.Normalize(mean = [0.4552, 0.4550, 0.4530],std = [0.1918, 0.1870, 0.1905], always_apply=True)]
     )
    return transform
...
def showIference(sequence,skip,input_video_file_path,output_video_file_path,showInfo):
    # Perform Accident Detection on the Test Video.
    predict_on_video(input_video_file_path, output_video_file_path,sequence,skip,showInfo)
    return output_video_file_path

def crashDetection(inputPath,seq,skip,outputPath,showInfo):
    # Perform Accident Detection on the Test Video.
...
def streaming_predict(frames):
    prediction,probb = streaming_framesInference(frames)
    global predicted_class_name
    predicted_class_name = prediction


def start_streaming(streamingUrl):
