...
class Robot():
    ...
    def move(self, step=1):
        ...
        if not isinstance(step, int):
            raise ValueError("step params must be integer.")
    ...
    def set_trace(self, color='red'):
        ...
        if not isinstance(color, str):
            raise ValueError("color params must be valid color.")
    ...
    def set_speed(self, speed=0.1):
        ...
        if not isinstance(speed, (float, int)) :
            raise ValueError("speed params must be a number.")
        self.speed = speed
    ...
    def has_object(self, obj_name=None):
        if obj_name is not None and not isinstance(obj_name, str):
            raise ValueError("obj_name params must be a string.")
    ...
    def read_message(self, wait_For=5):
        if not isinstance(wait_For, int):
            raise ValueError("wait_For params must be an integer.")
    ...
    def on_object(self, obj_type=None):
        if obj_type is not None and not isinstance(obj_type, str):
            raise ValueError("obj_type params must be a string.")
        self.world_model.incr_instruction()
        k = "{},{}".format(self.x, self.y)
        obj = self.world_model.objects.get(k, None)
        valid_type = True
    ...
    def take_if(self, obj_type=None):
