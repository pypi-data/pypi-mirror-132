    ...
    async def process_data(self, ipt, ids, *args, **kwargs):
        ...
        if not self.sta_init.get(ids):
            ...
            try:
                ...
                log.exception(
                    "PD_00: Conexión de estación con falla ->%s:%s" %
                    (ids, code))
        ...
        if self.is_connected(ids):
            ...
            if heartbeat:
                ...
                try:
                    ...
                    rprint("Error al cancelar get_message_header")
                    kwargs["origin_exception"] = f"PD_T16_00 + {code} => {ce}"                                                            
                    log.exception(f"Tiempo fuera, en get message_header idc {idc}, estación {code} , ids {ids}-> {sta_insta}, error {ce}")
                    v = await self.reset_station_conn(sta_insta, ids, idc)
                    return [ipt, ids, v, (sta_insta, db_insta)], kwargs                                                                       

                except Exception as ex:
                    kwargs["origin_exception"] = "PD_T17_00 + %s" %code                                                            
                    log.exception(
        ...
        if self.db_connect.get(idd):
            ...
            try:
                ...
                log.exception("PD_TO1: Tiempo fuera para conectar db %s" %db_insta)
                db_insta.close()
                self.db_connect[idd]=True
