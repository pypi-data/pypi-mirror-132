...
class ServoFunction(object):
    ...
    def absolute_measure_evaluation(self, device_id, absolute_measure):
        key_pattern = "device:" + device_id
        old_absolute_measure = self.r.get(key_pattern + ":absolute_measure")
        measure = "-"
        if old_absolute_measure != absolute_measure:
            measure = "off"
            if old_absolute_measure > absolute_measure:
                measure = "on"
        else:
    ...
    def device_evaluation(self, device_id, absolute_measure):
        ...
        if self.r.exists(key_pattern + ":user_id") == 1:
            ...
            measure = self.absolute_measure_evaluation(device_id, absolute_measure)
            ...
            if evaluated_measure != measure:
                ...
                if evaluated_measure == "on":
                    output.measure = self.r.get(key_pattern + ":setting_on")
                else:
                    output.measure = self.r.get(key_pattern + ":setting_off")
        return output
