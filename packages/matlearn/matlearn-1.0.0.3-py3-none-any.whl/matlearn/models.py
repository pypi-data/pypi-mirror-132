...
class Optimization:
    ...
    def _set_target(self, y):
        ...
        if isinstance(self, LinearRegressor):
            ...
            if size == 2:
                ...
                self._app_loss = 'multi_class_cross_entropy'
    ...
    @staticmethod
    def _ohe_weight_grad(x, y, p):
        m = y.shape[0]
        p[range(m),y] -= 1
        return 1/len(x) * np.dot(x.T, (p))

    @staticmethod
    def _ohe_bias_grad(y, p): return np.average(p [range(len(y)), y] - 1)
    ...
    def GD(self, x, y):
        ...
        if self._multiclass:
            self.weight = np.random.randn(x.shape[1], np.unique(y).size)
        ...
        while epoch != self.maxEpoch+1:
            ...
            for idx in list_idx:
                ...
                if self.train_bias:
                        if isinstance(self, LogisticRegressor) and self._multiclass:
                            self.bias = (self.bias) - (self.lr *
                                        Optimization._ohe_bias_grad(yit_train, p_train))
                        else:
                            self.bias = (self.bias) - (self.lr *
                                        Optimization._bias_grad(yit_train, p_train))

                if isinstance(self, LogisticRegressor) and self._multiclass:
                    self.weight = self.weight - (self.lr * Optimization._ohe_weight_grad(xit_train, yit_train, p_train))
                else:
...
class _BaseModel(Optimization):
    ...
    def _check_params(self):
        ...
        if self.regulariz_type not in ['L1','L2']:
            raise ValueError (f'{self.regulariz_type} not supported.')
        if self.alpha <0:
            raise ValueError ('alpha must be a positive number.')
        if self.lr < 0:
            raise ValueError(f'Learning rate must be a positive number.')
        if self.n_iter_per_epoch < 1:
            raise ValueError(
                'n_iter_per_epoch must be equal or greater than 1.')
        if self.n_reports <0:
            raise ValueError('n_reports must be zero or a positive number.')
        if self.maxEpoch <=0:
            raise ValueError('maxEpoch must be greater than zero')
...
class LinearRegressor(_BaseModel):
    ...
    Args:
        regulariz_type (string, optional): used to specify the type of regularization.
        Only [L1,L2] are supported. Defaults to L2.
        
        alpha ([Float, Integer], optional): Alpha hyperparameter. Used to Adjust the importance of the penalty. 
        Defaults to .00001
        
        lr ([Float, Integer], optional): Set learning rate hyperparameter for gradient descent.
        Defaults to .0001
        
        train_bias (bool, optional): Whether to train the bias or not. Defaults to True.
        
        n_reports (int, optional): Number of reports of training the data.
        Prints iteration numbers, training loss, and validation loss (if use_validation is True). 
        Set it to 0 to disable printing reports. The maximum number of reports is maxEpoch * n_iter_per_epoch. Defaults to 0
        
        maxEpoch (int, optional): Maximum number of epoch. Defaults to 100.
        
        converLim (int, optional): Used to set a threshold for convergence. 
        if differences in loss functions, during training, is less than ConverLim, the model will stop training. Defaults to .001
        
        n_converLim (int, optional): Number of convergence limits.
        For Example, if it is set to 5, then  differences in loss functions must be Five times less than ConverLim for five times. Otherwise, the model will continue training. Defaults to 1.
        ...
        val_ratio (Float, optional): A float number between 0 and 1. Used to set the ratio of validation set.
        Only use it if use_validation is set to True. Defaults to .1
    """
...
class LogisticRegressor(_BaseModel):
    ...
    Args:
        ...
        activ (string, optional): type of activation function. [sigmoid, softmax] are supported.
        sigmoid is recommended for binary classification and softmax for multi-class classification. Defaults to softmax.
    """

    def __init__(self, regulariz_type='L2', alpha=.00001, train_bias=True, lr=.001, n_reports=0, maxEpoch=100, converLim=.001, n_converLim=1, n_iter_per_epoch=1, use_validation = True, val_ratio = .1, activ='sigmoid'):
