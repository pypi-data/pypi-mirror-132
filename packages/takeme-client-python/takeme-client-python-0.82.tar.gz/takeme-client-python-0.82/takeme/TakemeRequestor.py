...
class TakemeRequestor(object):
    ...
    def __init__(self, corporate_code, secret_key):
    ...
    def call_api_without_bearer(self, endpoint, body):
        ...
        if body is not None:
            response = requests.post(self.BASE_URL_SANDBOX + endpoint, data=data, headers=headers)
            return response
        else:
            response = requests.get(self.BASE_URL_SANDBOX + endpoint, data=data, headers=headers)
            return response
