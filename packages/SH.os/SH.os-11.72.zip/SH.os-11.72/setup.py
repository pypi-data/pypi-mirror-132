...
def job(d): 
    ...
    while True:
        ...
        ## 條件 存在
        nameQ = os.popen(f'git config sys.cmd ').read().strip()
        if not nameQ=="":
            #################################
            sys.path.append( os.popen(f'git config sys.pwd ').read().strip() )
            globals()[ nameQ ] = importlib.import_module( nameQ )
            print("9999999999999999999123")
            ## 刪除 #######################
            #os.system('git config --global --unset  sys.cmd')
            #os.system('git config --global --unset  sys.pwd')
            #break
...
################################################
#import os
#print(os.popen("python APP.py").read())
##['''+nameA+''']
'''
