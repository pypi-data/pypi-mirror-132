...
from flame_dash import isml #it is internal module
import base64

encoded_image = base64.b64encode(open('C:/Users/Dell/Downloads/python_data_sets/data1/aset/temp.png', 'rb').read()) #it should be temp.png??
...
def update_output(i):
    caller.loadFile(i)
    return [{'label': k, 'value': v} for k,v in caller.variables.items()]

@app.callback(
    Output('heatmap', 'figure'),
    [Input('button','n_clicks')])
def update_fig(n,t,j):  #it should take only one parameter??
    fig = caller.dyPlot()
    return fig
