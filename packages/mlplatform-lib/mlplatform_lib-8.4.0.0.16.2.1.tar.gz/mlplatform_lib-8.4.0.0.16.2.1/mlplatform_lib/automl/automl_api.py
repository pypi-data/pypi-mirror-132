...
class AutomlApi:
    ...
    def insert_train_info(self, train_info_dto: TrainInfoDto):
        if self.api_client.run_mode == RunMode.KUBERNETES:
            return self.automl_client.insert_train_info(
                experiment_id=self.experiment_id,
                train_id=self.train_id,
                train_info_dto=train_info_dto,
            )
        else:
            print("Current mode is local, Skip insert_visualizations.")

    def insert_inference_info(self, inference_info_dto: InferenceInfoDto):
        if self.api_client.run_mode == RunMode.KUBERNETES:
            return self.automl_client.insert_inference_info(
                experiment_id=self.experiment_id,
                train_id=self.train_id,
                inference_info_dto=inference_info_dto
            )
        else:
            print("Current mode is local, Skip insert_visualizations.")
