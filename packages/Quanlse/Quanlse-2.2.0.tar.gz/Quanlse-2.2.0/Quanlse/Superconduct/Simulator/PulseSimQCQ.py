...
from Quanlse.Superconduct.SchedulerSupport import Scheduler, SchedulerPulseGenerator
from Quanlse.QOperation import CircuitLine
from Quanlse.QOperation.FixedGate import CR, X
...
def pulseGenerator(ham: QHamiltonian) -> SchedulerPulseGenerator:
    ...
    def generateVZ(_ham: QHamiltonian, cirLine: CircuitLine) -> QJob:
        """
    ...
    def generateHadamard(_ham: QHamiltonian, cirLine: CircuitLine, scheduler: Scheduler) -> QJob:
        """
        Generate Hadamard gate.
        """

        qIndex = list(cirLine.qRegIndexList)
        jobRX = generate1QGate(_ham, CircuitLine(RX(pi), qIndex), scheduler)
        jobRY = generate1QGate(_ham, CircuitLine(RY(-pi / 2), qIndex), scheduler)

        return jobRX + jobRY

    def generateCR(_ham: QHamiltonian, cirLine, scheduler, ampFactor=1., tFactor=1.) -> QJob:
        """
    ...
    def generateEchoedCR(ham: QHamiltonian, cirLine, scheduler) -> QJob:
        ...
        """
        qIndex = list(cirLine.qRegIndexList)
        jobCR = generateCR(_ham=ham, cirLine=CircuitLine(CR, qIndex), scheduler=scheduler, tFactor=0.5)
        jobX = generate1QGate(ham=ham, cirLine=CircuitLine(X, [qIndex[0]]), scheduler=scheduler)
        jobCRMinus = generateCR(_ham=ham, cirLine=CircuitLine(CR, qIndex), scheduler=scheduler,
    ...
    def generateCNOT(ham: QHamiltonian, cirLine, scheduler) -> QJob:
        ...
        """
        qIndex = list(cirLine.qRegIndexList)
        jobVZ = generateVZ(_ham=ham, cirLine=CircuitLine(VZ(pi / 2), [qIndex[0]]))
...
def staticZZ(obj: Union[PulseModel, QHamiltonian]) -> float:
    ...
    if ham.sysLevel == [3, 4, 4]:
        ...
        staticZZCrossTalk = originEigenVals[idx00] - originEigenVals[idx10] - originEigenVals[idx01] + \
                            originEigenVals[idx11]
