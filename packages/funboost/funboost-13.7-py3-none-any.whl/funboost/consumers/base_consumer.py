...
class DistributedConsumerStatistics(RedisMixin, LoggerMixinDefaultWithFileHandler):
    ...
    def _send_heartbeat2(self):
        # 根据队列名心跳的，值是字典，方便解析使用
        queue__consumer_identification_map_key_name = f'funboost_hearbeat_queue__dict:{self._queue_name}'
        results = self.redis_db_frame.smembers(queue__consumer_identification_map_key_name)
        with self.redis_db_frame.pipeline() as p:
            for result in results:
                ...
                if time.time() - result_dict['hearbeat_timestamp'] > 15 \
                    ...
                    p.srem(queue__consumer_identification_map_key_name, result)
            ...
            p.sadd(queue__consumer_identification_map_key_name, value)
            p.execute()

    def _send_heartbeat3(self):
        # 根据机器心跳的，值是字典，一个机器运行了哪些进程。
        server__consumer_identification_map_key_name = f'funboost_hearbeat_server__dict:{nb_log_config_default.computer_ip}'
        results = self.redis_db_frame.smembers(server__consumer_identification_map_key_name)
        with self.redis_db_frame.pipeline() as p:
            for result in results:
                result_dict = json.loads(result)
                if time.time() - result_dict['hearbeat_timestamp'] > 15 \
                        or self._consumer_identification_map['consumer_uuid'] == result_dict['consumer_uuid']:
                    # 因为这个是10秒钟运行一次，15秒还没更新，那肯定是掉线了。如果消费者本身是自己也先删除。
                    p.srem(server__consumer_identification_map_key_name, result)
            self._consumer_identification_map['hearbeat_datetime_str'] = time_util.DatetimeConverter().datetime_str
            self._consumer_identification_map['hearbeat_timestamp'] = time.time()
            value = json.dumps(self._consumer_identification_map, sort_keys=True)
            p.sadd(server__consumer_identification_map_key_name, value)
            p.execute()
    ...
    def send_heartbeat(self):
        ...
        with self.redis_db_frame.pipeline() as p:
            ...
            p.execute()
        self._send_heartbeat2()
        self._send_heartbeat3()
