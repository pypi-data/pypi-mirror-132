...
def change_xml_list_annotation(xml_dir, image_id, new_target, method_num, epoch):
    in_file = open(os.path.join(xml_dir, str(image_id) + '.xml'), encoding='utf8')  # 读取原来的xml文件
    ...
    for object in xmlroot.findall('object'):  # 找到root节点下的所有country节点
        bndbox = object.find('bndbox')  # 子节点下节点rank的值
        bndbox.find('xmin').text = str(new_target[index][0])
        bndbox.find('ymin').text = str(new_target[index][1])
        bndbox.find('xmax').text = str(new_target[index][2])
        bndbox.find('ymax').text = str(new_target[index][3])
        index = index + 1
