...
class DBClient:
    ...
    @retry
    def GetHistory(self, transactionId: str, start_date: datetime.date, end_date: datetime.date) -> Tuple[Optional[List[Claim]], Optional[str]]:
        if os.environ.get('RIALTIC_DATA_ENV') == 'local':
            ...
            if history_file.endswith('.json') and os.path.exists(full_history_path):
                return self._getHistoryLocal(full_history_path, transactionId, start_date, end_date)
            print(f"<RIALTIC_HISTORY_FILE = {history_file}> does not exist")
        else:
            return self._getHistoryDev(transactionId, start_date, end_date)

    def _getHistoryDev(self, transactionId: str, start_date: datetime.date, end_date: datetime.date) -> Tuple[Optional[List[Claim]], Optional[str]]:
        historyEvent = {
    ...
    def _getHistoryLocal(self, history_path: str, transactionId: str,
