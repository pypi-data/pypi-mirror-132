...
from cubicweb.web.views import uicfg, reledit as old_cw_reledit

uicfg.ReleditTags._keys = frozenset(
    "novalue_label novalue_include_rtype " "reload evid rvid edit_target".split()
)
...
def build_divid(rtype, role, entity_eid):
    ...
    return f"{rtype}-{role}-{entity_eid}"


@cached
def compute_ttypes(rschema, role):
    warn(
        "[inlinedit 0.3.0] compute_ttypes is deprecated, "
        "use rschema.targets(etype=, role=)",
        DeprecationWarning,
    )
...
class RelatedEntityEdit(EntityView):
    ...
    def call(self, action=None, reload=False, evid=None, **kwargs):
        ...
        if "container" in kwargs and not evid:
            ...
            etype = self._cw.entity_type(kwargs.get("container"))
        ...
        extradata = {k: kwargs.get(k) for k in self._parent_attrs}
    ...
    def cell_call(
        ...
        **kwargs,
        ...
        form_handler = self._cw.vreg["reledit"].select(
            "entity-form-handler", self._cw, entity=entity
        )
        form_handler.apply(
            self.w,
            value,
            ...
            reload,
            action,
            self._may_edit(entity),
            self._may_delete(entity),
            extradata=extradata,
        )
...
class AutoClickAndEditFormView(EntityView):
    ...
    def entity_call(
        ...
        self._cw.add_js("cubes.inlinedit.js", "cubicweb.edition.js")
        self.entity = entity
        ...
        divid = build_divid(rtype, role, self.entity.eid)
        if rschema.final:
            self._handle_attribute(rschema, role, divid, reload, action)
        else:
            if self._is_composite():
                self._handle_composite(rschema, role, divid, reload, action)
            else:
                self._handle_relation(rschema, role, divid, reload, action)
    ...
    def _handle_attribute(self, rschema, role, divid, reload, action):
        ...
        form_handler = self._cw.vreg["reledit"].select(
            "relation-form-handler", self._cw, entity=self.entity, rtype=rschema.type
        )
        form_handler.apply(
            self.w, value, self.entity, rschema, role, divid, reload, action
        )
    ...
    def _handle_relation(self, rschema, role, divid, reload, action):
    ...
    def _handle_composite(self, rschema, role, divid, reload, action):
        rvid = self._rules.get("rvid", "edit-related-entity")
        value, permission = self._compute_value_editperm(
            rschema,
            role,
            rvid,
            reload=reload,
        ...
        form_handler.apply(
            ...
            reload,
            action,
            self._may_add_related(rschema, role),
            False,
        )
    ...
    should_edit_attributes = callable_deprecated(
        "[3.9] should_edit_attributes is deprecated,"
    ...
    should_edit_relations = callable_deprecated(
        "[3.9] should_edit_relations is deprecated,"
...
class ReleditFormHandler(appobject.AppObject):
    ...
    _onclick = "cw.inlinedit.loadInlineForm(%s);"
    ...
    _editzonemsg = "click to edit this field"
    _editzonelogo = "pen_icon.png"

    def _build_edit_zone(self):
        return self._editzone % {
            "msg": xml_escape(str(self._cw._(self._editzonemsg))),
    ...
    def _setup_form_buttons(self, form, divid):
        cancel_callbackname = (
            "edit_related_form" if divid.startswith("none") else "reledit_form"
        )
    ...
    def wrap_form(
        ...
        with t.div(
            w,
            id="%s-reledit" % divid,
            onmouseout="jQuery('#%s').addClass('hidden')" % divid,
            onmouseover="jQuery('#%s').removeClass('hidden')" % divid,
            Class="releditField",
        ):
            with t.div(w, id="%s-value" % divid, Class="editableFieldValue"):
                w(value)
            form.render(renderer=renderer, w=w)
            with t.div(w, id=divid, Class="editableField hidden"):
                view_form_method()
...
class ReleditEntityFormHandler(ReleditFormHandler):
    ...
    _deletemsg = "click to delete this value"
    _deletezonelogo = "cancel.png"

    _action_formid = {"edit-related": "edition", "delete-related": "deleteconf"}
    ...
    def _build_args(self, entity, reload, action, extradata=None):
        ...
        event_args = {
            ...
            "reload": reload,
            "action": action,
            "fname": "edit_related_form",
        }
        if extradata:
            event_args.update(extradata)
        return event_args

    def _build_renderer(self, entity):
    ...
    def _build_form(self, entity, divid, reload, action, extradata=None, **formargs):
        ...
        form = self._cw.vreg["forms"].select(
            ...
            action=self._cw.build_url(
                "validateform", __onsuccess="window.parent.cw.inlinedit.onSuccess"
            ),
            cwtarget="eformframe",
        ...
        form.event_args = event_args
        return form, self._build_renderer(entity)
    ...
    def wrap_form(
        ...
        with t.div(
            w,
            id="%s-reledit" % divid,
            onmouseout="jQuery('#%s').addClass('hidden')",
            onmouseover="jQuery('#%s').removeClass('hidden')",
...
class ReleditRelationFormHandler(ReleditFormHandler):
    ...
    _addmsg = "click to add a value"
    _addzonelogo = "plus.png"

    _action_formid = {"edit-rtype": "base", "add-related": "edition"}
    ...
    def apply(
        ...
        **formargs,
        ...
        self.w = w
        form, renderer = self._build_form(
            entity, rschema, role, divid, reload, action, extradata=None, **formargs
        )
        self.view_form(divid, value, form, renderer, add_related, edit_rtype)

    def _build_add_zone(self):
        return self._addzone % {
            "msg": xml_escape(self._cw._(self._addmsg)),
            "logo": xml_escape(self._cw.data_url(self._addzonelogo)),
        }

    def _build_args(self, entity, rtype, role, reload, action, extradata=None):
        divid = build_divid(rtype, role, entity.eid)
        event_args = {
            "divid": divid,
            "eid": entity.eid,
            "rtype": rtype,
    ...
    def _prepare_form(self, entity, rschema, role, action):
        ...
        self._cw.form["__linkto"] = f"{rschema}:{entity.eid}:{neg_role(role)}"
    ...
    def _build_form(
        self, entity, rschema, role, divid, reload, action, extradata=None, **formargs
    ):
        rtype = rschema.type
        event_args = self._build_args(entity, rtype, role, reload, action, extradata)
        if not action:
            form = _DummyForm()
            form.event_args = event_args
            return form, None
    ...
    def view_form(
        self, divid, value, form=None, renderer=None, add_related=False, edit_rtype=True
    ):
        def _view_form():
            args = form.event_args.copy()
            if edit_rtype:
        ...
        self.wrap_form(
            divid, value, form, renderer, False, add_related, False, _view_form
        )
