...
def get_feats_smart_meter(s, mm_id, Pinst, max_threshold=1.3, resample=True, time_interval="15T"):
    """
    ...
    infered_freq = pd.infer_freq(s.index)
    if resample:
        s_resampled = s.resample(time_interval).mean()
    else:
        s_resampled = s
        Warning("The input series is not resampled to 55 minute intervals.")
...
def get_feats_transformer_station(s, tp_id, max_threshold=1.3, resample=True, time_interval="5T"):
    """
    Get features for a single sample. 
    The input series is resampled to 5 minute intervals.
    ...
    s : pandas.Series
        Sample.
    tp_id : str
        Transformer station ID.
    Pinst : float
        Connected power of the transformer station.
    max_threshold : float
        Maximum threshold for the power.
    resample : bool
        If True, the input series is resampled to the "time_interval" intervals (default: 5min).
    time_interval : str
        Time interval for resampling.
    ...
    Example
    -------
    >>> get_feats_transformer_station(s, tp_id, Pinst)
    """
    max_threshold = max_threshold
    ...
    invalid_mask = (below_zero_mask + missing_values_mask).rename("invalid")
