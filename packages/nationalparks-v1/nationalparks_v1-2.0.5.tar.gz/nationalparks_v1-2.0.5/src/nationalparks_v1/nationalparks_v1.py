...
Please note that for some of the functions below, you will need a US National Parks API key. A free API key can be acquired from: https://www.nps.gov/subjects/developer/get-started.htm

See what each function can do by running national_parks() helper function.
...
def activity_search(activity, token):
    ...
    assert (data_df[data_df['name'].str.lower()==activity.lower()]).empty==False, f"{activity} is not a valid activity name to serach for. Please call activity_list() to see a list of activities you can search for."
    code=data_df.loc[data_df['name'].str.lower()==activity.lower(),'id'].item()
