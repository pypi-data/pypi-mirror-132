...
def parks_list():
    ...
    """
    my_file_contents = []
    with resources.open_text('nationalparks_v1.data', 'parks_list.csv',encoding='latin-1') as fhandle: # add the 'encoding' type if you know it
        line = fhandle.readline()
        my_file_contents.append(line.split(',')) # contact me if you have problems here, I assumed a simple CSV here
        while line:
            line = fhandle.readline()
            my_file_contents.append(line.split(',')) # contact me if you have problems here, I assumed a simple CSV here

    if len(my_file_contents) > 0:
        columns = my_file_contents[0] # get the header columns
        data_df = pd.DataFrame(data = my_file_contents[1:], columns = columns)
    return data_df
...
def activity_list():
    ...
    """
    my_file_contents = []
    with resources.open_text('nationalparks_v1.data', 'activities.csv',encoding='latin-1') as fhandle: # add the 'encoding' type if you know it
        line = fhandle.readline()
        my_file_contents.append(line.split(',')) # contact me if you have problems here, I assumed a simple CSV here
        while line:
            line = fhandle.readline()
...
def activity_search(activity, token):
    ...
    data.df=data.df.apply(lambda x: x.astype(str).str.lower())
    assert (data.df[data.df['name']==activity]).empty==False, f"{activity} is not a valid activity name to serach for. Please call activity_list() to see a list of activities you can search for."
    code=act_df.loc[data.df['name']==activity,'id'].item()
