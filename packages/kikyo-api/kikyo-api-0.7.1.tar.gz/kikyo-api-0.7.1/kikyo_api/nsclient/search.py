...
class SearchClient(NamespacedClient):
    ...
    def search(self, index: str) -> 'DataIndex':
        """
        指定索引

        :param index: index名称
        """
        warnings.warn('"search" is deprecated, use "index" instead', DeprecationWarning, stacklevel=2)
...
class DataIndex:
    ...
    def exists(self, id: str) -> bool:
        ...
        resp = self._client.session.head(
            f'{self._client.api_prefix}/{self._index}/{id}',
        )
    ...
    def update(self, id: str, data: str = None):
        ...
        read_json_data(
            self._client.session.put(
                f'{self._client.api_prefix}/{self._index}/update/{id}',
                data=_json.dumps({
                    'body': {
                        'doc': data,
                    },
    ...
    def search_by_dsl(self, query_body: dict) -> 'Response':
        data = read_json_data(
            self._client.session.post(
                f'{self._client.api_prefix}/{self._index}/search-by-dsl',
                data=_json.dumps({
                    'body': query_body,
                }).encode(),
                headers={
    ...
    def count_by_dsl(self, query_body: dict) -> 'Response':
        data = read_json_data(
            self._client.session.post(
                f'{self._client.api_prefix}/{self._index}/count-by-dsl',
                data=_json.dumps({
                    'body': query_body,
                }).encode(),
                headers={
