...
from maxda.analysis.da_tag.label_util import LabelUtil
...
class PandaLabelDictTool(object):
    ...
    def _load_dict_file(self, input_file_path, sheet_name, header=0):
        if isinstance(input_file_path, str):
            ...
            if extension.endswith("xlsx") or extension.endswith("xls"):
                df_input = pd.read_excel(input_file_path, header=header, encoding='utf_8_sig', sheet_name=sheet_name,
                                         type="object")
            elif extension.endswith("csv"):
                df_input = pd.read_csv(input_file_path, header=header, encoding='utf_8_sig', type="object")
            else:
    ...
    def load_line_dict(self, input_file_path,
        ...
        :param sheet_name: sheet name 列名
        :return:
        """
