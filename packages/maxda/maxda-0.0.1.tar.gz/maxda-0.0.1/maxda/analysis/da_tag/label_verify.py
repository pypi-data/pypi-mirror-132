...
from maxda.analysis.da_tag.label_dict import PandaLabelDictTool
        ...
        for n in range(excel.nsheets):
            ...
            if '-' in sheet_name:
                ...
                for e in sheet.merged_cells:
                    ...
                    if rl >= num:
                        error_msg_list.append(['{}有不合法的合并单元格'.format(sheet_name), base_value])
                    if base_value == '':
                        error_msg_list.append(['{}合并单元格中有空单元格'.format(sheet_name), ''])
                    df.iloc[rl:rh, cl:ch] = base_value
                for k in list(df.isnull().any()):
                    if k:
                        check_code = '有空单元格'
                        error_msg_list.append(['{}有空单元格'.format(sheet_name), ''])
                        return error_msg_list
            else:
