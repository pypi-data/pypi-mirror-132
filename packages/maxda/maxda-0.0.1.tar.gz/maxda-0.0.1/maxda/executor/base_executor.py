...
# purpose : 
# author : 
# create_time : 2020/8/31 16:51
# update_time : 2020/8/31 16:51
# copyright : Lavector
# ----------------------------------------------
import pandas as pd
...
class BaseExecutor(object):
    ...
    def run_flow(self, config_flow,
        ...
        # 2.加载数据
        df_input = self.load_data(input_source, merge_task_object=task_merge_object, suffix=suffix, task_meta=self.task_meta)
    ...
    def save_result(self, df_result, task_meta, output_path=None):
        """
        保存处理结果
        :param df_result: object, 数据处理结果
        :param task_meta: dict,元数据
        :param output_path: string,结果输出文件路径
        :return:
        """
        raise NotImplemented

    def process_df_task(self, df_input, task_param_object, statis_output_path):
        """
        处理一些基于DataFrame的处理功能
        :param df_input: dataframe, 输入数据
        :param task_param_object: TaskParam, 任务参数
        :param statis_output_path: string, 统计结果输出目录
