...
def validate_url(
    ...
    if is_docker() and raw_value.lower().startswith(LOCALHOST_PREFIX):
        raise click.BadArgumentUsage(
            "{} cannot be localhost when running in Docker. Please use host.docker.internal instead.".format(
                url_type
            )
        )
    ...
    if not is_connection_healthy(target_url):
        raise click.BadArgumentUsage("Cannot reach {}: {}".format(url_type, raw_value))
    return target_url
