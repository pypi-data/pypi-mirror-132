...
def handle_finished(
    ...
    click.echo()
    click.echo(get_run_summary(context, event))
...
def get_test_suite_result_summary(
    ...
    return f"{suite_name}: {success} / {total} passed" if total else None
...
class TestPlanConsoleOutputHandler(EventHandler):
    def handle_event(self, context: ExecutionContext, event: events.Event) -> None:
        ...
        if isinstance(event, events.Initialized):
            click.echo("Starting to test the plan: {}".format(event.payload.plan_name))
            pass
        ...
        if isinstance(event, events.AfterTestCaseExecution):
            ...
            click.secho(
                "Finished the test case: {}\n".format(event.payload.name), fg="green"
            )
        ...
        if isinstance(event, events.Finished):
            click.secho("Finished running the test plan.", fg="green")
