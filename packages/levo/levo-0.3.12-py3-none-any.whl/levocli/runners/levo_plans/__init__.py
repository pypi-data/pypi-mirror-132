...
def cli_entrypoint(input_config: TestPlanCommandConfig):
    ...
    # Unfortunately plan.name is of type Optional(str).
    # However some functions below require plan.name to be str type.
    # So we use another variable to work around
    plan_name: str = "" if not plan.name else plan.name

    config = PlanConfig(
        spec_path="",  # This should be optional ideally.
        test_plan_path=os.path.join(plan.catalog, plan_name),
    ...
    logger = build_plan_logger(plan_name, context)
