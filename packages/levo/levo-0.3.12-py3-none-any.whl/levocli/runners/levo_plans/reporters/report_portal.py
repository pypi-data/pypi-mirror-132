...
from ....utils import SENSITIVE_HEADERS, format_exception, mask_sensitive_headers
...
def _send_result(event, service, test_item_id):
    serialized_result = SerializedTestResult.from_test_result(event.payload.result)
    for assertion in serialized_result.assertions.values():
    ...
    for interaction in serialized_result.interactions.values():
        interaction.request.headers = mask_sensitive_headers(
            interaction.request.headers, SENSITIVE_HEADERS
        )
        interaction.response.headers = mask_sensitive_headers(
    ...
    json_attachment = {
        ...
        "data": json.dumps(attr.asdict(serialized_result)),
...
def handle_finished(
    ...
    report_log(get_run_summary(context, event), service)
...
def handle_initialized_event(
    ...
    log.info("Started the test launch.", name=launch_name, attributes=launch_attr)
