...
class TestKafka(TestCase):
    ...
    def test_span_properties(self):
        ...
        producer.send(self.topic, value=message, partition=0, headers=test_headers[:])
        ...
        for span in spans:
            ...
            if span.kind == SpanKind.PRODUCER:
                self.assert_producer_attributes(span_topic, span_url, span_payload, span_headers, message, test_headers)
                if span_partition == 0:
                ...
                self.assert_consumer_attributes(span_topic, span_url, span_payload, span_headers, message)
                if span_partition == 0:
    ...
    def assert_producer_attributes(self, span_topic, span_url, span_payload, span_headers,
                                   expected_payload, expected_headers):
        self.assertEqual(self.topic, span_topic)
        self.assertEqual(json.dumps(self.bootstrap_servers), span_url)
        self.assertEqual(f"{expected_headers}", span_headers)
        self.assertEqual(expected_payload.decode(), span_payload)

    def assert_consumer_attributes(self, span_topic, span_url, span_payload, span_headers, expected_payload):
        self.assertEqual(self.topic, span_topic)
        self.assertEqual(json.dumps(self.bootstrap_servers), span_url)
