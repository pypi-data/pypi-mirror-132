...
class AnalyzeRequestDoc(RequestDocument):
    ...
    def __init__(self, package_manager: Text, packages: List[UnifiedPackageID] = None, install_manifest: bool = None,
                 manifest: Text = None, lockfile: Text = None, project: Text = "default", analyzer_version: int = None):
        ...
        if self.install_manifest:
            assert self.manifest, "manifest param is required when install_manifest param is set"


class AnalyzeResponseDoc(BaseModel):
    workflow_id: Text
    status: Text
    failed_checks: Optional[List[List[Dict]]]
