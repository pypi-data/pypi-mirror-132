...
bool TqPythonApi::IsChanging(py::object obj, py::str fields) {
  ...
  if (object_classname == "DataFrame" || object_classname == "Series") {
    ...
    for (auto& [key, node] : m_alive_order_view->GetCommitNodes()) {
...
bool TqPythonApi::IsChanging(py::object obj) {
  ...
  if (object_classname == "DataFrame" || object_classname == "Series") {
    ...
    for (auto [k, node] : m_alive_order_view->GetCommitNodes()) {
      if (node->Latest()->unit_id == unit_id && node->Latest()->order_id == order_id) {
        return true;
      }
    }
    ...
    return m_alive_order_view->GetCommitNodes().size();
...
std::vector<std::shared_ptr<md::InstrumentNode>> TqPythonApi::SubscribeQuoteList(
  const std::vector<std::string>& symbol_list) {
  std::vector<std::shared_ptr<md::InstrumentNode>> quote_list;

  for (auto symbol : symbol_list) {
    quote_list.push_back(SubscribeQuote(symbol));
  }
...
const std::map<std::string, std::shared_ptr<OrderNode>>& TqPythonApi::GetOrders(int unit_id) {
  try {
    return m_db_root->GetNodeMap<future::Order>();
  } catch (std::exception e) {
    CleanUp();
    throw std::exception(std::logic_error(e.what()));
  }
