...
class Builder(py7zr.SevenZipFile):
    def __init__(
            ...
            mode: str = "w",
            *,
            sfx_type: int = SFX_TYPE_INSTALLER_GUI,
        ...
        if sfx_bundle:
            if hasattr(file, "write"):
                ...
                raise ValueError("type '{}' not supported".format(type(file).__name__))
            if not isinstance(file, multivolumefile.MultiVolume):
                ...
                p("type '{}' does not support create_bundle".format(type(file).__name__), error=True)
        else:
