...
from gm.pb.data_pb2 import Tick, Bar, Quote, L2Transaction, L2Order, L2OrderQueue
...
from gm.utils import gmsdklogger, beijing_tzinfo, protobuf_timestamp2bj_datetime, round4
...
def tick_callback_new(data):
    ...
    for q in tick.quotes:  # type: Quote
        quotes.append({
            'bid_p': round4(q.bid_p),
            'bid_v': round4(q.bid_v),
            'ask_p': round4(q.ask_p),
            'ask_v': round4(q.ask_v),
        })
    ...
    ticknew = {
        ...
        'price': round4(tick.price),
        'open': round4(tick.open),
        'high': round4(tick.high),
        'low': round4(tick.low),
        'cum_volume': tick.cum_volume,
        'cum_amount': round4(tick.cum_amount),
        'cum_position': tick.cum_position,
        'last_amount': round4(tick.last_amount),
...
def bar_callback_new(data):
    ...
    barnew = {
        ...
        'low': round4(bar.low),
        'volume': bar.volume,
        'amount': round4(bar.amount),
        'pre_close': round4(bar.pre_close),
...
def l2transaction_callback(data):
    if _or_not_none(context.on_l2transaction_fun, context.on_l2transaction_fun_v2):
        trans = L2Transaction()
        trans.ParseFromString(data)
        if context.on_l2transaction_fun_v2 is not None:
            context.on_l2transaction_fun_v2(context, trans)
            return
...
def parameters_callback(data):
    if _or_not_none(context.on_parameter_fun, context.on_parameter_fun_v2):
        ...
        if context.on_parameter_fun_v2 is not None and len(parameters) > 0:
...
def algo_order_status_callback(data):
    ...
    if _or_not_none(context.on_algo_order_status_fun, context.on_algo_order_status_fun_v2):
        algo_order = AlgoOrder()
        algo_order.ParseFromString(data)
        if context.on_algo_order_status_fun_v2 is not None:
            context.on_algo_order_status_fun_v2(context, algo_order)
            return
...
def account_status_callback(data):
    if _or_not_none(context.on_account_status_fun, context.on_account_status_fun_v2):
        ...
        if account_status.status and account_id in context.inside_accounts:
            context.inside_accounts[account_id].status = account_status.status
