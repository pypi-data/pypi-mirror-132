...
import six
from typing import Text, List, Dict, NoReturn, Any, Callable
...
from gm.csdk.c_sdk import py_gmi_current, py_gmi_schedule, py_gmi_set_data_callback, py_gmi_set_strategy_id, \
    gmi_set_mode, py_gmi_set_backtest_config, py_gmi_subscribe, py_gmi_set_token, py_gmi_get_encrypted_token, py_gmi_get_orgcode, py_gmi_unsubscribe, \
...
def run(strategy_id='', filename='', mode=MODE_UNKNOWN, token='',
        ...
        serv_addr=''):
    # type: (Text, Text, int, Text, Text, Text, float, float, float, float, float, float, int, int, Text) ->NoReturn
    """
    执行策略
    """
    ...
    backtest_marginfloat_ratio1 = float(options.backtest_marginfloat_ratio1)
    backtest_marginfloat_ratio2 = float(options.backtest_marginfloat_ratio2)
    ...
    context.on_l2transaction_fun_v2 = getattr(
        fmodule, 'on_l2transaction_v2', None)
    context.init_fun = getattr(fmodule, 'init', None)
    context.on_execution_report_fun = getattr(
        fmodule, 'on_execution_report', None)
    context.on_execution_report_fun_v2 = getattr(
        fmodule, 'on_execution_report_v2', None)
    context.on_order_status_fun = getattr(fmodule, 'on_order_status', None)
    context.on_order_status_fun_v2 = getattr(
        fmodule, 'on_order_status_v2', None)
    context.on_algo_order_status_fun = getattr(
        fmodule, 'on_algo_order_status', None)
    context.on_algo_order_status_fun_v2 = getattr(
        fmodule, 'on_algo_order_status_v2', None)
    context.on_backtest_finished_fun = getattr(
        fmodule, 'on_backtest_finished', None)
    context.on_backtest_finished_fun_v2 = getattr(
        fmodule, 'on_backtest_finished_v2', None)
    ...
    context.on_account_status_fun_v2 = getattr(
        fmodule, 'on_account_status_v2', None)
    ...
    if _all_not_none(context.on_execution_report_fun, context.on_execution_report_fun_v2):
        gmsdklogger.warning('{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format(
            'on_execution_report_fun'))
    if _all_not_none(context.on_order_status_fun, context.on_order_status_fun_v2):
        gmsdklogger.warning(
            '{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format('on_order_status_fun'))
    if _all_not_none(context.on_algo_order_status_fun, context.on_algo_order_status_fun_v2):
        gmsdklogger.warning(
            '{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format('on_algo_order_status'))
    if _all_not_none(context.on_backtest_finished_fun, context.on_backtest_finished_fun_v2):
        gmsdklogger.warning('{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format(
            'on_backtest_finished_fun'))
    if _all_not_none(context.on_parameter_fun, context.on_parameter_fun_v2):
        gmsdklogger.warning(
            '{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format('on_parameter_fun'))
    if _all_not_none(context.on_account_status_fun, context.on_account_status_fun_v2):
        gmsdklogger.warning(
            '{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format('on_account_status_fun'))
    if _all_not_none(context.on_l2transaction_fun, context.on_l2transaction_fun_v2):
        gmsdklogger.warning(
            '{0} 与 {0}_v2 都定义了, 只会调用{0}_v2'.format('on_l2transaction_fun'))
