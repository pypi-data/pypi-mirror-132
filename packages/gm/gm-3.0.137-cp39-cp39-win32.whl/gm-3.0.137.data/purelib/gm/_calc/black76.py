...
def calculate_price(f: float, k: float, r: float, v: float, t: float, cp: float = 1., d1: float = None):
    # b76理论价格
    check_value(cp)
    if v <= 0:
        return max(0, cp * (f-k))
...
def calculate_delta(f: float, k: float, r: float, v: float, t: float, cp: float = 1., cash: bool = False, d1: float = None) -> float:
    # 计算delta
...
def calculate_vega(f: float, k: float, r: float, v: float, t: float, cash: bool = False, d1: float = None) -> float:
    # 计算 vage
    if v <= 0:
        return 0
...
def calculate_theta(f: float, k: float, r: float, v: float, t: float, cp: float = 1., cash: bool = False, d1: float = None) -> float:
    # 计算 theta
    check_value(cp, v=v)
    ...
    theta = - f * np.exp(-r * t) * norm.pdf(d1) * v / (2 * np.sqrt(t)) - \
...
def calculate_rho(f: float, k: float, r: float, v: float, t: float, cp: float = 1., cash: bool = False, d1: float = None) -> float:
    # 计算 rho
    check_value(cp, v=v)
...
def calculate_gamma(f: float, k: float, r: float, v: float, t: float,  cash: bool = False, d1: float = None) -> float:
    # 计算 gamma
    check_value(v=v)
...
def calculate_greeks(f: float, k: float, r: float, v: float, t: float, cp: float = 1., cash: bool = False, d1: float = None) -> float:
    # 计算 greeks
    check_value(v=v)
...
def calculate_iv(p: float, f: float, k: float, r: float, t: float, cp: float = 1.) -> float:
