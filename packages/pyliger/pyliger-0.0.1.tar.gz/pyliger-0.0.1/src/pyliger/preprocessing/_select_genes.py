...
def select_genes(liger_object,
    ...
    for i in datasets_use:
        ...
        gene_mean_upper = gene_expr_mean + norm.ppf(1 - alpha_thresh_corrected / 2) * np.sqrt(
            ne.evaluate('gene_expr_mean * nolan_constant') / adata.shape[0])
        ...
        if num_genes is not None:
            ...
            optimized = minimize(fun=num_var_genes, x0=[0], agrs=num_genes[i], tol=tol, bounds=[(0, 1.5)])
        ...
        temp = var_thresh[i]
        select_gene = ne.evaluate('((gene_expr_var / nolan_constant) > gene_mean_upper) & (log10(gene_expr_var) > (base_gene_lower + temp))')
    ...
    for idx, adata in enumerate(liger_object.adata_list):
        var_gene_idx = liger_object.adata_list[idx].var.index.isin(genes_use).nonzero()[0]
...
def _calc_var_online(adata, gene_expr_mean, chunk_size):
    ...
    with h5sparse.File(file_path, 'r') as f:
        for left, right in _h5_idx_generator(chunk_size, adata.shape[0]):
            sum_sq_dev += np.sum(np.square(np.absolute(np.subtract(f['norm_data'][left:right].toarray(), gene_expr_mean))), axis=0)
...
def _calc_var_matrix(adata):
    ...
    gene_expr_var = ne.evaluate('gene_expr_var * num_sampels / (num_sampels-1)') # change ddof
    adata.var['norm_var'] = gene_expr_var
