...
def plot_gene_loadings(liger_object,
                       dataset1 = None,
                       dataset2 = None,
                       num_genes_show = 12,
                       num_genes = 30,
    ...
    dataset_specificity = calc_dataset_specificity(liger_object, dataset1=dataset1, dataset2=dataset2, do_plot=do_spec_plot)

    factors_use = np.abs(dataset_specificity[2][dataset_specificity[2] <= factor_share_thresh])

    markers = get_factor_markers(liger_object, dataset1=dataset1, dataset2=dataset2,
                                 factor_share_thresh=factor_share_thresh,
                                 num_genes=num_genes, log_fc_thresh=log_fc_thresh, umi_thresh=umi_thresh,
                                 frac_thresh=frac_thresh, pval_thresh=pval_thresh,
    ...
    for i in tqdm(factors_use):
        ...
        def _rescale(x, _from):
            norm = mpl.colors.TwoSlopeNorm(vmin=_from[0], vcenter=_from[0] + max_val*(_from[1]-_from[0]), vmax=_from[1])
            return norm(x)

        p1 = (ggplot(data=tsne_df, mapping=aes(x='tsne1', y='tsne2', color=factorlab)) +
        ...
        for idx, gene_list in enumerate(top_genes_list):
            ...
            gene_df = pd.DataFrame({'loadings': sorted,
                                    'xpos': np.linspace(0,1,num=len(sorted)+1),
                                    'top_k': 0})
            y_lim_text = gene_df['loadings'].max()

            # plot and annotate with top genes
            out_plot = (ggplot(data=gene_df, mapping=aes(x='xpos', y='loadings')) +
                        geom_point(size=0.4) +
                        theme_bw() +
                        theme(axis_ticks_direction_x=element_blank(),
                        ...
                        annotate('text', x=1.1,
                                 ...
                                 hjust=0,
                                 col='#8227A0') +
                        coord_cartesian(xlim=[0,1], # this focuses the x-axis on the range of interest
                                        clip='off') +
                        theme()) #TODO
            if mark_top_genes:
                out_plot = out_plot + geom_point(data=gene_df, mapping=aes('xpos', 'loadings'), color='#8227A0', size=0.5)
...
def plot_gene_loadings_spatial():
    pass