...
Usage:
  opentf-ctl config set-orchestrator NAME [--insecure-skip-tls-verify=true] [--server=server] [--eventbus-port=port] [--agentchannel-port=port] [--observer-port=port] [--receptionist-port=port] [--killswitch-port=port] [options]
...
def read_configuration():
    ...
    def _safe_get(kind, name):
        ...
        if what is None:
            logging.error(
                '%s %s is not available in configuration.', kind.title(), repr(name)
            )
            logging.error('(Using the %s configuration file.)', repr(_))
            sys.exit(2)
        return what
    ...
    def _safe_value(option):
        ...
        if what == '':
            logging.error('The %s option specifies an empty value.', option)
            sys.exit(2)
        return what
...
def _get_port(service, default):
    ...
    try:
        ...
        logging.error('Not a valid port value: %s', err)
        sys.exit(2)
...
def _read_opentfconfig():
    ...
    try:
        ...
        logging.error('Could not read configuration file %s: %s.', conf_filename, err)
        logging.error(
...
def _ensure_name_exists(name, label, config, src):
    ...
    if name not in names:
        ...
        logging.error('Available %ss: %s.', label, ','.join(names))
        sys.exit(2)
...
def use_context(context):
    ...
    try:
        ...
        logging.error('Could not modify configuration file %s: %s.', conf_filename, err)
        sys.exit(2)
        ...
        for svc in [
            ...
            if port := _get_value(f'--{svc}-port='):
                try:
                    ...
                    logging.error(
                        '%s port must be an integer: %s.', svc.title(), repr(port)
                    )
                    sys.exit(2)
                ports[svc] = port
        if ports:
...
def generate_config():
    ...
    generated_conf = {'apiVersion': 'opentestfactory.org/v1alpha1', 'kind': 'CtlConfig'}
    ...
    try:
        ...
        logging.error('Not a valid insecure-skip-tls-verify flag: %s.', err)
        sys.exit(2)
...
def print_config_help(args):
    ...
    if _is_command('config generate', args):
        ...
        logging.error('Unknown config command.  Use --help to list known commands.')
        sys.exit(1)
