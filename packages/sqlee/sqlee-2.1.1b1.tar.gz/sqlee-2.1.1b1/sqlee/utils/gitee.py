...
from sqlee.utils.exceptions import RepositoryNotFoundError, RepositoryConnectionRefusedError, InternetError
...
class GiteeRepo:
    ...
    def __init__(self, token=None, user=None, repo=None):
        ...
        try:
            status_code = requests.get(url=self.url).status_code
            if status_code == 404:
                raise RepositoryNotFoundError(self.repo)
            elif status_code // 2 != 2:
