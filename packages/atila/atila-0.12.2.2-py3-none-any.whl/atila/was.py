...
from skitai.protocols.sock.impl.http import http_date
from skitai.protocols.sock.impl.smtp import composer
from skitai import wsgiappservice
from skitai.tasks import tasks, Task, get_cloned_was
import skitai
...
class WAS (wsgiappservice.AsyncServicableWAS):
    # mehiods remap ------------------------------------------------
    ...
    def _resolve_app (self, thing):
        if isinstance (thing, str) and thing.find (":") != -1:
    ...
    def pipe (self, thing, *args, **kargs):
        ...
        return app.pipe (thing) (self, *args, **kargs)
    ...
    def WebSocketPipedProcess (self, command):
        return PipedProcess (self.websocket, command)
