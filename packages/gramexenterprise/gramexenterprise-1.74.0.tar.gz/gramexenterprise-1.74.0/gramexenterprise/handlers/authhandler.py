...
class SAMLAuth2(AuthHandler):
    ...
    def get_saml_client(self):
        ...
        if not self.sso_url_pattern:
            raise HTTPError(INTERNAL_SERVER_ERROR, reason='SAMLAuth2: missing SP URL, eg: /sso')
        if not os.path.exists(self.idp_metadata_path):
            raise HTTPError(INTERNAL_SERVER_ERROR, reason='SAMLAuth2: missing SSO IDP Metadata')
...
class DBAuth(SimpleAuth):
    ...
    def report_error(self, status, event, error, data=None):
        ...
        self.log_user_event(event='fail')
        return {'code': 'auth', 'event': event, 'error': data or error}
    ...
    def get(self):
        ...
        # When the user opens the email link, validate the OTP
            ...
            if row is not None:
                ...
                app_log.debug('%s: Invalid OTP', self.name, user)
