            ...
            while 1:
                ...
                if (isinstance(ch, str)):
                    ...
                    if ch in ["up", "down"]:
                        ...
                        if ch == "down" and self.HISTORY_POINTER < history_len:
                            ...
                            read_history = True
                        if read_history:
                            if -1 < self.HISTORY_POINTER < history_len:  # self.HISTORY_POINTER ∈(1, history_len)
                ...
                # printable
                    ...
                    if pointer == len(self.STDIN_STREAM):
                        ...
                        self.STDIN_STREAM = self.STDIN_STREAM[:pointer] + ch + self.STDIN_STREAM[pointer:]
                    pointer += 1
                ...
                # \b
                    ...
                    if pointer == len(self.STDIN_STREAM):
                        ...
                        self.STDIN_STREAM = self.STDIN_STREAM[:pointer - 1] + self.STDIN_STREAM[pointer:]
                    pointer -= 1
                ...
                if completion:
                    ...
                    # 若为参数补全
                        ...
                        line = [a+b"/"+b for a, b in zip(params["-"], params["--"])] + max(params["-"], params["--"],key=len)[len(min(params["-"], params["--"], key=len)):]
                        stdout.write("\n" + b"  ".join(last_word for last_word in line).decode() + "\n")
                        break
                ...
                stdout.write("\b" * old_pointer + " " * clean_len + "\b" * clean_len)  # 清空原本输入
                ...
                # 获取命令
                command = b" ".join(stream_list[:-1]).decode() if len(stream_list) > 1 else self.STDIN_STREAM.decode()
                # 若命令存在命令列表中
