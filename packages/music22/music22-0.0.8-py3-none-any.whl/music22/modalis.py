...
class melodies(object):
    ...
    def corr(self,metric='KL'):
        ...
        PDFS = []
        for i in range(0,len(self.melodies)):
            P = self.melodies[i].pdf
            PDFS.append(P)

        PDFS = numpy.array(PDFS)
    ...
    def matrix(self):
        ...
        try:
            stats.matrix(self.distances,self.labels, self.matrix_method)
...
class melodia(object):
    ...
    def get_tonique(self,percent=0.5,method="mode"):
        ...
        Final_Freqs = Fc[(L-int(Nb_Frames)):L]
