...
def corr(L,metric='KL'):
    """Calculate distances between PDFs.

    Args:
        L (numpy array) : array of pdf lists.
...
def matrix(distances,labels, meth):
    ...
    Y = linkage(distances, method = meth)
    ...
    Z1 = dendrogram(Y, orientation='right',labels=labels, leaf_font_size=9) # adding/removing the axes
    ax1.set_xticks([])

    # Compute and plot second dendrogram.
    ax2 = fig.add_axes([0.4,0.73,0.6,0.17])
    Z2 = dendrogram(Y,labels=labels, leaf_font_size=9) # adding/removing the axes
    ax2.set_yticks([])
