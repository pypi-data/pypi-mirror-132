...
def request(token='', ticker='', family='', group='', market='', country=Enumerations.Country.Country_Unspecified,
    ...
    if not all(reportParameter == '' for reportParameter in [document, item, chapter, section, subsection, paragraph]):
...
def requestValue(token='', ticker='', family='', group='', market='', country=Enumerations.Country.Country_Unspecified,
                 ...
                 fieldType='', versionType=Enumerations.VersionType.Version_Unspecified, author='',
                 version='', quality=Enumerations.Quality.Quality_Unspecified, timeOut=300, timeFrecuency=0.5, fileName='', format='dataFrame'):
    fieldType = Enumerations.FieldType.fromString(fieldType)
    service = ServiceRequestDataPointValue.ServiceRequestDataPointValue.create(ticker, family, group, market, country, dateStart, dateEnd, fixing, fieldType, versionType, author, version, quality)
...
if __name__=='__main__':
    if not len(sys.argv) > 1:
        ...
        country, dateStart, dateEnd, fixing, dataType, versionType, quality = Arguments.checkArguments(args)
        # Service
        if args.service == 'r':
            request = request(args.token, args.ticker, args.family, args.group, args.market, country, dateStart, dateEnd, fixing, dataType, versionType, args.author, args.version, quality, args.timeOut, args.timeFrecuency, args.filepath, args.format)
