...
class SlackApiKeyError(ValueError):
    pass

class SlackNotification:
    def __init__(self, api_key: Optional[str]) -> None:
        self.api_key = os.environ.get("SLACK_TOKEN", self.api_key)
        if self.api_key is None:
            raise SlackApiError("Slack API Token must be provided or set `SLACK_TOKEN` as an environment variable")

        self.client = WebClient(self.api_key)
...
slack = SlackNotification(api_key='xoxb-4204675967-1849720748006-sMh6DgaWuXsSiKfoLlRpEvgZ')
slack.notify(alert="This is a test notification from a bot", channel_ids='C02FUGTVD7G')