...
ACTION_SHORTCUTS = {
    'actions': 'show_actions', 'check': 'check_integrity', 'clone': 'clone_project', 'commit': 'commit_project',
    'prepare': 'prepare_commit', 'push': 'push_project', 'refresh': 'refresh_outsourced', 'release': 'release_project',
    'renew': 'new_project', 'request': 'request_merge', 'status': 'show_status'}
...
# todo: refactor to use instead: pkg_resources.Requirement.parse(package_version)
...
def _action(*project_types: str, **deco_kwargs) -> Callable:     # Callable[[Callable], Callable]:
    ...
    def _deco(fun):
        ...
        if project_types == (PARENT_PRJ, ROOT_PRJ) and 'arg_names' not in deco_kwargs:
            deco_kwargs['arg_names'] = ((ARG_ALL, ), ('children-sets-expr', ), ('children-names' + ARG_MULTIPLES, ))
...
def find_extra_modules(project_path: str, namespace_name: str, portion_name: str) -> List[str]:
    ...
    """
    modules_dir = os.path.join(project_path, namespace_name, portion_name)
    if os.path.isdir(modules_dir):
        base = os.path.basename
        return [base(_)[:-len(PY_EXT)] for _ in path_items(os.path.join(modules_dir, '*' + PY_EXT)) if _ != PY_INIT]
    return []
...
def project_dev_vars(project_path: str = "") -> PdvType:
    ...
    pdv['editable_project_path'] = editable_project_path(package_name)
    ...
    pdv['pypi_versions'] = pypi_versions(package_name)
    ...
    if project_type == ROOT_PRJ:
        ...
        _init_children_vars_and_presets(pdv, por_vars)
        ...
        for chi_prj_path in coll.paths:
            p_name = os.path.basename(chi_prj_path)
            chi_vars[p_name] = project_dev_vars(project_path=chi_prj_path)
        _init_children_vars_and_presets(pdv, chi_vars)
...
def pypi_versions(package_name: str) -> List[str]:
    ...
    :return:                    list of released versions or empty list on error.
    """
...
def _check_templates(pdv: PdvType):
    ...
    if missing or outdated:
        ...
        _chk_if(40, False, f"***** check cancelled. update outsourced files first via the 'new-{project_type}' action")

    elif checked:
        msg = ": " + (PPF(checked) if cae.verbose else chk_files) if _debug_or_verbose() else ""
...
def _check_types_linting_tests(pdv: PdvType):
    ...
    with in_wd(project_path):
        ...
        _cl(61, f"mypy {exclude_options} --lineprecision-report mypy_report"
            ...
            extra_args=mypy_options + args)
        """ TODO: refactor/extend to the --strict option/level, equivalent to the following:  ( [*] == already used )
        ...
        out: List[str] = []
        _cl(62, f"pylint --output-format=text --max-line-length={line_len} {ignore_options}", extra_args=args,
            exit_on_err=False, lines_output=out)   # alternative to _exit_on_err: run pylint with option --exit-zero
        ...
        _cl(63, f"python -m pytest --cov-report html --cov={namespace_name if is_portion else 'main'}"
                f" {pytest_option} {pdv_str(pdv, 'TESTS_FOLDER')}/")
...
def _cl(err_code: int, command_line: str, extra_args: Sequence = (), lines_output: Optional[List[str]] = None,
    ...
    if (sh_err and exit_on_err) or _debug_or_verbose():
...
def _get_branch(ini_pdv: PdvType) -> str:
    return cae.get_option('branch') or pdv_str(ini_pdv, 'git_current_branch')
...
def _git_checkout(pdv: PdvType, *extra_args: str, branch: str = "", from_branch: str = ""):
    ...
    is_clean = files_uncommitted == [] or branch not in pdv_val(pdv, 'git_branches')
...
def _git_fetch(pdv: PdvType, *extra_args: str) -> List[str]:
    ...
    output: List[str] = []
    with in_wd(project_path):
        _cl(75, "git fetch", extra_args=args, lines_output=output)
...
def _git_push(pdv: PdvType, *branches_and_tags: str, exit_on_error: bool = True, extra_args: Iterable[str] = ()) -> int:
    ...
    usr = _get_user(pdv)
    group_or_user_name = usr if 'upstream' in pdv_val(pdv, 'git_remotes') else _get_group(pdv)
    ...
    with in_wd(pdv_str(pdv, 'project_path')):
        if forked:
            ...
            if git_remotes['upstream'] != upstream_url:
                ...
                git_remotes['upstream'] = upstream_url
...
def _git_vars_update(pdv: PdvType):
    ...
    if os.path.isdir(os.path.join(project_path, GIT_FOLDER_NAME)):
        all_branches[:] = _git_branches(pdv)
        uncommitted_files[:] = _git_uncommitted(pdv)
...
def _init_act_exec_args() -> Tuple[str, PdvType, tuple]:
    ...
    * run optional pre_action.
    ...
    :return:                    tuple of action name to be executed and optional action function/method args.
    """
    ...
    while act_name not in actions:
        if not act_args:
            if initial_action in ACTION_SHORTCUTS:
                act_name = ACTION_SHORTCUTS[initial_action]
                act_args[:] = initial_args
                break
            _exit_error(30, f"****  invalid action '{act_name.replace('_', '-')}'. valid actions: {actions}")
    ...
    act_spec = REGISTERED_ACTIONS.get(act_name,
    ...
    if '_children' in act_name:  # multiple namespace portions or parent children action, where ini_pdv is root | parent
        act_args[:] = _init_children_pdv_args(ini_pdv, act_args)
        extra_msg += f" {len(act_args)} {_children_desc(ini_pdv)}: " + " ".join(
            _['package_name'] for _ in act_args.values())
    ...
    cae.po(f"----- {act_name.replace('_', '-')}: {pdv_str(ini_pdv, 'project_name')}{extra_msg}")

    return act_name, ini_pdv, act_args
...
def _init_children_pdv_args(ini_pdv: PdvType, act_args: ActionArgs) -> List[PdvType]:
    ...
    if act_args == [ARG_ALL]:
        ...
        pkg_names = try_eval(" ".join(act_args), (Exception, ), glo_vars=pdv_val(ini_pdv, 'children_presets'))
        if pkg_names is UNSET:
            pkg_names = act_args

    _chk_if(38, bool(pkg_names) and isinstance(pkg_names, (list, set, tuple)),
    ...
    filter_branch = cae.get_option('filterBranch')
    if filter_branch:
        pkg_names = [pkg for pkg in pkg_names if filter_branch in chi_vars.get(pkg, {}).get('git_branches', {})]
    filter_expr = cae.get_option('filter')
    if filter_expr:
        pkg_names = [pkg for pkg in pkg_names if try_eval(filter_expr, (Exception, ), glo_vars=ini_pdv)]

    if _debug_or_verbose() and len(pkg_names) < len(set(pkg_names)):
        cae.po(f"   ** {len(pkg_names) - len(set(pkg_names))} duplicate children specified: {duplicates(pkg_names)}")

    return [chi_vars.get(p_name, {'package_name': p_name}) for p_name in pkg_names]


def _init_children_vars_and_presets(pdv: PdvType, chi_vars):
    pdv['children_presets'] = chi_ps = {
        ARG_ALL: set(_['package_name'] for _ in chi_vars.values()),
        'editable': set(_['package_name'] for _ in chi_vars.values() if _['editable_project_path']),
        'modified': set(_['package_name'] for _ in chi_vars.values() if _['git_uncommitted_files']),
        'develop': set(_['package_name'] for _ in chi_vars.values() if _['git_current_branch'] == MAIN_BRANCH),
    }
    branch = cae.get_option('branch')
    if branch:
        chi_ps['branch'] = set(_['package_name'] for _ in chi_vars.values() if _['git_current_branch'] == branch)
    branch = cae.get_option('filterBranch')
    if branch:
        chi_ps['filterBranch'] = set(_['package_name'] for _ in chi_vars.values() if _['git_current_branch'] == branch)
    expr = cae.get_option('filter')
    if expr:
        chi_ps['filter'] = set(_['package_name'] for _ in chi_vars.values()
                               if try_eval(expr, ignored_exceptions=(Exception, ), glo_vars=_))

    pdv['children_project_vars'] = chi_vars
...
def _print_pdv(pdv: PdvType):
    cae.po(f"  --- {pdv_str(pdv, 'prj_id' if cae.debug else 'project_name')} project environment:")

    if not cae.get_option('verbose'):
        pdv = pdv.copy()
        pdv['setup_kwargs'] = (pdv_val(pdv, 'setup_kwargs') or {}).copy()
        ...
        pdv['git_branches'] = " ".join(pdv_val(pdv, 'git_branches'))
        pdv['git_uncommitted_files'] = " ".join(pdv_val(pdv, 'git_uncommitted_files'))
        pdv['git_remotes'] = " ".join(f"{name}={url}" for name, url in (pdv_val(pdv, 'git_remotes') or {}).items())
        pdv['install_require'] = " ".join(pdv_val(pdv, 'install_require'))
        if 'long_desc_content' in pdv:
            pdv['long_desc_content'] = pdv['setup_kwargs']['long_description'] = \
                pdv_str(pdv, 'long_desc_content')[:33] + "..."
        nsp_len = len(pdv_str(pdv, 'namespace_name')) + 1
        pdv['portions_packages'] = " ".join(pkg_name[nsp_len:] for pkg_name in pdv_val(pdv, 'portions_packages'))
        if pdv_str(pdv, 'project_type') in (PARENT_PRJ, ROOT_PRJ):
            pdv['children_presets'] = [f"{preset: <9} == {' '.join(pkg_name[nsp_len:] for pkg_name in dep_packages)}"
                                       for preset, dep_packages in (pdv_val(pdv, 'children_presets') or {}).items()]
    ...
    if not cae.verbose:
        ...
        for name, val in list(pdv.items()):
            if not val or name in (
                    name.upper(), 'portions_project_vars', 'import_name', 'long_desc_content', 'long_desc_type',
                    'namespace_name', 'pip_name', 'portion_name',
                    'portions_pypi_refs', 'portions_pypi_refs_md', 'portions_import_names',
                    'repo_domain', 'repo_group', 'repo_host', 'repo_root', 'setup_kwargs', ):
                pdv.pop(name, None)
...
def _renew_prj_dir(new_pdv: PdvType):
    ...
    if not is_file(main_file):
        _RCS[_rc_id(ae.base, 'write_file')](main_file, f"\"\"\" {package_name} {project_type} main module \"\"\"{sep}"
                                                       f"{sep}"
                                                       f"{VERSION_PREFIX}0.0.0{VERSION_QUOTE}{sep}")
...
def _renew_project(ini_pdv: PdvType, project_type: str) -> PdvType:
    ...
    new_pdv = {'namespace_name': namespace_name, 'package_name': package_name,
               'project_path': project_path, 'project_type': project_type}
    new_repo = _git_init_if_needed(new_pdv)
    errors = _git_fetch(new_pdv)
    _chk_if(15, not bool(errors), f" **** git fetch errors: {PPF(errors)}")

    _git_vars_update(new_pdv)
    cur_branch = pdv_str(new_pdv, 'git_current_branch')
    req_branch = cae.get_option('branch')
    if req_branch or cur_branch == MAIN_BRANCH:
        ...
        co_args = ("--merge", "--track") if f"remotes/origin/{renew_branch}" in pdv_val(new_pdv, 'git_branches') else ()
...
class GitlabCom(_RemoteHost):
    ...
    def project_from_name(self, err_code: int, err_msg: str, project_name: str, owned: bool = True) -> Project:
        """ convert group/package_name or a fragment if it to a project object/instance of the remote repo api.

        :param err_code:        error code, pass 0 to not exit on error.
        :param err_msg:         error message to display on error.
        :param project_name:    group/package name or fragment to search.
        :param owned:           if True then the fragment/list search is quicker and limited to owned repos.
                                pass False to search in all repos for the specified project name fragment.
        :return:                gitlab project instance if found, else return None if err_code is zero else exit.
        """
        try:
    ...
    def clean_releases(self, ini_pdv: PdvType) -> List[str]:
        ...
        pypi_releases = pdv_val(ini_pdv, 'pypi_versions')
        ...
        for branch_name in all_branches:
            ...
            if chk == 'remotes/origin/':        # un-deployed remote release branch found
                ...
                project = self.project_from_name(35, "   ** {name} not found at origin", package_name)
                try:
    ...
    @_action(PARENT_PRJ, *ANY_PRJ_TYPE)
    def fork_project(self, ini_pdv: PdvType):
        """ create/renew fork of a remote repo into our user/group namespace. """
        if 'upstream' in pdv_val(ini_pdv, 'git_remotes'):    # renew origin from upstream if already forked
            ...
            with in_wd(parent_path):
                ...
                _git_vars_update(ini_pdv)       # renew git_remotes
    ...
    def push_children(self, ini_pdv: PdvType, *children_pdv: PdvType):
        ...
        cae.po(f"  === {len(children_pdv)} {_children_desc(ini_pdv)} pushed")
    ...
    def push_project(self, ini_pdv: PdvType, branch_name: str = ''):
        ...
        if not self.project_from_name(0, "", package_name):
            ...
            cae.po(f"   == created new project {project} under remote user/group {group_name}")
            _wait()
        ...
        if pkg_version != file_version:     # and cae.get_option('force')
    ...
    def release_children(self, ini_pdv: PdvType, *children_pdv: PdvType):
        ...
        for chi_pdv in children_pdv:
            self.release_project(chi_pdv, "HEAD")
        cae.po(f"  === {len(children_pdv)} {_children_desc(ini_pdv)} released")

    @_action(arg_names=(("push-tag", ), ("HEAD", )))
    def release_project(self, ini_pdv: PdvType, push_tag: str):
        """ release latest/specified develop push commit. """
        errors = _git_fetch(ini_pdv)
        _chk_if(86, not bool(errors), f" **** git fetch errors: {PPF(errors)}" + _hint(
            self.release_project, " later to retry if server is currently unavailable, or fix remote settings"))
    ...
    def request_merge(self, ini_pdv: PdvType):
        ...
        origin_user = pdv_val(ini_pdv, 'git_remotes').get('origin', "/").split('/')[-2]
        _chk_if(64, origin_user == user_name, f"   ** user mismatch specified={user_name} != origin={origin_user}")
        upstream_group = pdv_val(ini_pdv, 'git_remotes').get('upstream', "/").split('/')[-2]
        ...
        # merge_req.assignee_id = self.connection.users.get("user name").id  # assign it to a coworker
        if _debug_or_verbose():
            cae.po(f"    . merge request diffs: {PPF([_.attributes for _ in merge_req.diffs.list()])}")

        cae.po(f"   == requested merge of branch {branch} from fork/origin ({src_prj.id}) into upstream ({tgt_prj.id})")
    ...
    def show_repo(self, ini_pdv: PdvType):
        ...
        if isinstance(repo_api, Project):
            cae.po(f"  --- {group_project} remote repository:")
            for attr in ('created_at', 'default_branch', 'id', 'name', 'namespace_id', 'visibility'):
...
@_action(ROOT_PRJ, PARENT_PRJ, arg_names=(("--package", ), ("--path", ), ("package-name", ), ("portion-name", )),
         pre_action=_check_children_not_exist)
...
@_action(local_action=False)    # local_actions explicitly set to include remote actions in console output
def show_actions(ini_pdv: PdvType):
    ...
    if compact:
        ...
        cae.po(f"      {'   '.join(act.replace('_', '-') for act in actions if _act_callable(ini_pdv, act))}")
    else:
        ...
        def _act_print(act_reg_key: str):
            ...
            shortcut = next((sc for sc, act in ACTION_SHORTCUTS.items() if act == act_reg_key), None)
            if shortcut:
...
@_action(PARENT_PRJ, ROOT_PRJ)
def show_children_status(_ini_pdv: PdvType, *portions_args: PdvType):
    """ run integrity checks for the specified portions of a namespace. """
    for por_pdv in portions_args:
        cae.po(f" ---  {pdv_str(por_pdv, 'package_name') + '  ---': <18}{pdv_str(por_pdv, 'project_desc')}")
        show_status(por_pdv)
...
def show_status(ini_pdv: PdvType):
    ...
    if pdv_str(ini_pdv, 'project_type') != PARENT_PRJ:
        cae.po("  --- git diff - to be staged/added:")
        cae.po(PPF(_git_diff(ini_pdv)))

        if pdv_val(ini_pdv, 'git_current_branch') != MAIN_BRANCH:
            cae.po(f"  --- git diff to local branch '{MAIN_BRANCH}':")
            cae.po(PPF(_git_diff(ini_pdv, MAIN_BRANCH)))

        cae.po(f"  --- git diff {MAIN_BRANCH} origin/{MAIN_BRANCH} (to fix err run: git fetch origin {MAIN_BRANCH}):")
        cae.po(PPF(_git_diff(ini_pdv, MAIN_BRANCH, f"origin/{MAIN_BRANCH}")))

        cae.po("  --- git status:")
        cae.po(PPF(_git_status(ini_pdv)))
...
def main():                                                                                 # pragma: no cover
    ...
    cae.add_argument('arguments',
                     help="additional arguments, depending on specified action, e.g. all children actions expecting"
                          ...
                          " sets like all|editable|modified|local|develop",
                     nargs='*')
    cae.add_option('branch', "branch or version-tag to checkout/filter-/work-on", "")
    cae.add_option('delay', "seconds to wait between actions of a bulk, e.g. on multiple portions", 69, short_opt='w')
    ...
    cae.add_option('filter', "filter expression reducing children arguments specified as expression", "", short_opt='F')
    cae.add_option('filterBranch', "branch to filter children arguments specified as expression", "", short_opt='B')
    ...
    act_name, ini_pdv, act_args = _init_act_exec_args()     # init globals, check if action is implemented, compile args
