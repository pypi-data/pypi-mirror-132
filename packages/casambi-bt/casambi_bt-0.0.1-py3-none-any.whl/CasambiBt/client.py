...
from . import ConnectionStateError, ProtocolError
...
class CasambiClient:
    ...
    def __init__(
        ...
        self._connectionState = _ConnectionState.NONE
        self._dataCallback = dataCallback

    def _checkState(self, desired: _ConnectionState) -> None:
        if self._connectionState != desired:
            raise ConnectionStateError(
                f"State is {self._connectionState.name}, expected {desired.name}"
            )

    async def connect(self) -> Awaitable[None]:
        self._checkState(_ConnectionState.NONE)
        ...
        self._connectionState = _ConnectionState.CONNECTED

    async def exchangeKey(self) -> Awaitable[None]:
        self._checkState(_ConnectionState.CONNECTED)
        ...
        if not (firstResp[0] == 0x1 and firstResp[1] == 0xA):
            self._connectionState = _ConnectionState.ERROR
            raise ProtocolError(
        ...
        if self._connectionState == _ConnectionState.ERROR:
        ...
        if self._connectionState == _ConnectionState.ERROR:
            ...
            self._connectionState = _ConnectionState.KEY_EXCHANGED

    def _callbackMulitplexer(self, handle: int, data: bytes) -> None:
        self._logger.debug(f"Callback on handle {handle}: {b2a(data)}")

        if self._connectionState == _ConnectionState.CONNECTED:
            self._exchNofityCallback(handle, data)
        elif self._connectionState == _ConnectionState.KEY_EXCHANGED:
            self._authNofityCallback(handle, data)
        elif self._connectionState == _ConnectionState.AUTHENTICATED:
    ...
    async def authenticate(self, keystore: KeyStore) -> Awaitable[None]:
        self._checkState(_ConnectionState.KEY_EXCHANGED)
        ...
        if self._connectionState == _ConnectionState.ERROR:
            ...
            self._connectionState = _ConnectionState.AUTHENTICATED
    ...
    async def send(self, packet: bytes) -> Awaitable[None]:
        self._checkState(_ConnectionState.AUTHENTICATED)
