...
class Processor(object):
    ...
    def __process_sql(self, data, context):
        ...
        if name is not None:
            ...
            if key == "$whole":
                value = TableParse(res.res).get_value(key)
            elif key == "$rows":
                value = res.rows
            elif key == "$columns":
                value = res.columns
                ...
                value = TableParse(res.res).get_value(key)
                if value is Utils.ERROR:
                    raise Exception("sql标签未提取到变量值，可能是指定的key格式不正确或错误！\n该sql执行结果为 {}".format(res.res))
            if len(names) > 1:
                if type(value) not in [list, tuple]:
                    raise Exception(
                        "sql标签取到的值 {} 类型 {} 不正确，当需要设置多个变量时，值的类型必须是列表或元祖".format(value, type(value)))
                if len(names) != len(value):
                    raise Exception("sql标签需要的变量数:{}个与实际得到的变量数:{}个不等".format(len(names), len(value)))
                for i in range(len(names)):
                    context[names[i]] = Processor.__get_specified_type_var(value[i], var_type)
                    Printer.print_sql_for_set(names[i], value[i])
            else:
                context[names[0]] = Processor.__get_specified_type_var(value, var_type)
                Printer.print_sql_for_set(names[0], value)
