...
class HTTP(AbstractClient):
    ...
    def __get_header(self):
        ...
        :return:
        """
        if self._interface_info.header is not None and not JsonParse.is_correct_json(
                self._interface_info.header):
            raise Exception(
                "接口 {} 的请求头 {} 格式不正确，请检查！".format(self._interface_info.name, self._interface_info.header))
        ...
        if body is not None:
            if body_type == "key_value":
                if not JsonParse.is_correct_json_dict(body):
                    raise Exception("请求报文不是正确的表单格式，请检查！该报文为:\n{}".format(body))
                ...
                if not JsonParse.is_correct_json(body):
                    raise Exception("请求报文不是正确的json报文，请检查！该报文为:\n{}".format(body))
                ...
                raise Exception("HTTP的 {} 请求不支持对此种格式 {} 的处理".format(method, body_type))
        if param is not None and method == "get":
            if not JsonParse.is_correct_json_dict(param):
                raise Exception("请求参数不是正确的键值对格式，请检查！该参数为:\n{}".format(param))
    ...
    def __get_param(self):
        ...
        if param is not None:
            if not JsonParse.is_correct_json_dict(param):
                raise Exception("请求参数不是正确的键值对格式，请检查！")
    ...
    def __get_files(self):
        ...
        if len(files) == 0:
            return None
        if method != "post":
            raise Exception("包含文件的请求必须使用post方式！")
        n_files = {}
    ...
    def __get_cookies(self):
        ...
        if not JsonParse.is_correct_json_dict(cookies):
            raise Exception("不是正确格式的cookies")
    ...
    def __get_auth(self):
        ...
        if auth_type in [None, "HTTPBasicAuth"]:
            ...
            raise Exception("该认证方式 {} 暂不支持".format(auth_type))
            ...
            if files:
                ...
                if method == "get":
                    r = self.__session.get(url, params=body, headers=headers, cookies=cookies, auth=auth, timeout=(
                        float(config.http_connection_timeout), float(config.http_read_timeout)))
                elif method == "patch":
                    r = self.__session.patch(url, params=param, data=body, headers=headers, cookies=cookies, auth=auth,
                                             timeout=(
                                                 float(config.http_connection_timeout),
                                                 float(config.http_read_timeout)))
                elif method == "put":
            ...
            if type(files) is dict:
                for n, f in files.items():
                    if type(f[1]) is BufferedReader:
                        f[1].close()
            if err_type == 1:
                raise Exception("计算机拒绝了您的连接，请检查请求地址是否正确、网络是否正常!")
            elif err_type == 2:
                raise Exception("读取响应超时，请检查服务器是否正常！")
