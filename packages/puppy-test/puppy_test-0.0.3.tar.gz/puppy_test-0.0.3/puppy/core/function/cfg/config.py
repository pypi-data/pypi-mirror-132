...
class ConfigLoader(object):
    ...
    def __init__(self, config_file=None):
        ...
        if config_file:
            ...
            if not os.path.exists(self.__config_file):
                dir_of_config_file = os.path.dirname(self.__config_file)
                if not os.path.exists(dir_of_config_file):
                    os.makedirs(dir_of_config_file)
                open(self.__config_file, "w+", encoding='utf-8').close()
        # 默认配置
        ...
        if type(cfg) is not _type:
            if _type is bool:
                if cfg in ["True","1","true"]:
                    ...
                    raise BaseException(" {} 配置项配置不正确，只能是True或False，不能是 {}".format(name,cfg))
            elif _type is int:
                if cfg.isnumeric():
                    return int(cfg)
                else:
                    raise BaseException(" {} 配置项不正确，只能是数字，不能是 {}".format(name,cfg))
    ...
    def start_test(self):
        ...
        self.set_config("beautify",False)
