...
class DB(object):
    ...
    def connect(self):
        ...
        if self.__software == "mysql":
            ...
            raise Exception("不支持对此类数据库的操作：{}".format(self.__software))

    def __connect_fro_mysql(self):
        if importlib.util.find_spec("pymysql") is None:
            raise Exception("要访问和操作mysql数据库，请安装PyMySQL==0.9.3!")
        ...
        try:
            ...
            if conn_error is not None:
                raise Exception("连接mysql数据库错误，提示：{}".format(conn_error))

    def __connect_for_oracle(self):
        if importlib.util.find_spec("cx_Oracle") is None or importlib.util.find_spec("pandas") is None:
            raise Exception("要访问和操作oracle数据库，请安装cx-Oracle==7.1.1和pandas==1.0.1!")
        ...
        try:
            ...
            if conn_error == 1:
                raise Exception("python连接oracle的环境未配置正确，请检查！")
            elif conn_error == 2:
                raise Exception("连接数据库超时，请检查连接配置是否正确：{}".format(self.__db_info))
            elif conn_error == 3:
                raise Exception("oracle数据库版本不正确，请检查！")
    ...
    def __execute_for_mysql(self, sql: str):
        ...
        try:
            ...
            if not self.is_a_query_statement(sql):
                max_ = int(config.max_number_non_select_sql)
...
最大操作记录请在config文件内配置【max_number_non_select_sql项】。'''.format(
            ...
            if cursor:
                cursor.close()
            if cursor_error:
                raise Exception("执行sql语句出错，数据库返回错误信息为：{}\n此sql语句为：{}".format(cursor_error, sql))
...
最大操作记录请在config文件内配置【max_number_non_select_sql项】。'''.format(
    ...
    def check_sql(cls, sql):
        ...
        if not (sql_temp.startswith("select") or sql_temp.startswith("insert") or sql_temp.startswith(
            ...
            raise Exception("sql语句执行只支持增删改查，不支持其他操作，请检查此语句：{}".format(sql))
        if (sql_temp.startswith("insert") or sql_temp.startswith("delete") or sql_temp.startswith(
                "update")) and "where" not in sql_temp:
            raise Exception("sql语句执行非查询语句时必须至少带一个where字句，防止修改整张表，此sql语句未带字句，请检查:{}".format(sql))
