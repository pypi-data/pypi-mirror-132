...
class TestIterators(TestBase.TestBase):
    ...
    def test_set_iterators(self):
        ...
        self.assertEqual(
            ampl.get_set('x').num_instances(),
            len(dict(ampl.get_set('x')))
        )
        ...
        self.assertEqual(
            max(s.num_instances() for name, s in ampl.get_sets()), 8
        )
        self.assertEqual(
            ampl.get_set('x').name(),
    ...
    def test_parameter_iterators(self):
        ...
        ampl.eval('param x;')
        self.assertEqual(
            len(ampl.get_parameters()),
            ampl.get_parameters().size()
        )
        self.assertEqual(
            ampl.get_parameters().size(),
            len(list(ampl.get_parameters()))
        )
        ...
        self.assertEqual(
            ampl.get_parameter('x').num_instances(),
            len(dict(ampl.get_parameter('x')))
        )
        ...
        self.assertEqual(
            max(p.num_instances() for name, p in ampl.get_parameters()), 8
        )
        self.assertEqual(
    ...
    def test_variable_iterators(self):
        ...
        ampl.eval('var x;')
        self.assertEqual(
            len(ampl.get_variables()),
            ampl.get_variables().size()
        )
        self.assertEqual(
            ampl.get_variables().size(),
            len(list(ampl.get_variables()))
        )
        ...
        self.assertEqual(
            ampl.get_variable('x').num_instances(),
            len(dict(ampl.get_variable('x')))
        )
    ...
    def test_constraint_iterators(self):
        ...
        self.assertEqual(
            ampl.get_constraints().size(),
            len(ampl.get_constraints())
        )
        self.assertEqual(
            ampl.get_constraints().size(),
            len(list(ampl.get_constraints()))
        )
    ...
    def test_objective_iterators(self):
        ...
        self.assertEqual(
            ampl.get_objectives().size(),
            len(ampl.get_objectives())
        )
        self.assertEqual(
            ampl.get_objectives().size(),
            len(list(ampl.get_objectives()))
        )
