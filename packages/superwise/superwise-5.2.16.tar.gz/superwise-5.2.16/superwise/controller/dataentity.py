...
class DataEntityController(BaseController):
    ...
    def summarise(
        ...
        **kwargs
        ...
        :param data: a dataframe of basline data
        :param entities_dtypes: dtypes dictinary or None for auto infer internaly
           HOW TO USE:
           1. Manually create dictionary as:
                {"feature_0" : "Boolean",
                "feature_1" : "Categorical"
                }
           2. run infer to get dictinary and pass the value to allow override of infer results:
                from superwise.controller.infer import infer_dtype
                dtypes = infer_dtypes(basline_df)
           3. pass None
              the summarise function will auto generate the dtypes.

        :param specific_roles: dictionary of roles, for example:
             {
             "feature_1" : "feature",
             "record_id" : "id"
             }
        :param default_role: a default role used by SDK (normallu and by default is feature)
        :param importance_mapping: mapping dictionary for feature importance, example:
            {
            "feature_0" : 0.4
            }
        :param importance_target_label: option to specificy a lable target for feature importance
        :param importance_sample: allow sampling of data for feature importnace.
               this option might help with big basline
        :param base_version: a base version. usefull to inherit information from already exist version

        :return: list of DataEntity objects.
