...
class Superwise:
    ...
    def __init__(
        ...
        :param client_id: client access token
        :param secret: secret access token
        :param _rest_client: Optional to insert rest client if needed (allow mocking of rest api calls)
        :param email: email of user (optional)
        :param password: password of user (optional)
             email+password are alternative way to use the SDK (instead of API KEY)
        :param _auth_url: for internal use only
        :param _superwise_host: for iternal use only
        """
