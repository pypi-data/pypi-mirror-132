...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[obpPN])[0]
 def get_physical_resource_id(self,attribute=obpPN,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if obpPh(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return obpPv(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @obpPO
 def add_defaults(resource,stack_name:obpPC):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return obpPv(SESReceiptRule,self).get_cfn_attribute(attribute)
 @obpPc
