...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=PWzkl,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if PWzkB(perm["IpProtocol"])!=PWzkB(rp["IpProtocol"]):
    continue
 ...
 @PWzkE
 def get_vpc_and_name_for_security_group(cls,group_id:PWzkB)->Tuple[PWzkB,PWzkB]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or PWzkl
  group_name=groups and groups[0]["GroupName"]or PWzkl
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",PWzkl)
   vpc_id=PWzkl
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",PWzkl)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=PWzkB(result.get("IpProtocol"))
   description=result.pop("Description",PWzkl)
   cidr_ipv6=result.pop("CidrIpv6",PWzkl)
