...
def canonicalize_name(name:mFgdA)->mFgdA:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[mFgdU])[0]
 def get_physical_resource_id(self,attribute=mFgdU,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return mFgdY(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @mFgdc
