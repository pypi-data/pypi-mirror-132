...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=doMjO(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not doMjJ
   return result
  except doMjK:
   props=self.props
   ...
   if parts[0]in candidates and doMjG(parts)>1:
