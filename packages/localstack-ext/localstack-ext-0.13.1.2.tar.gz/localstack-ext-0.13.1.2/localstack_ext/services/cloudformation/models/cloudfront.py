...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or ouUWk
 @ouUWE
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return ouUWv(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @ouUWR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if ouUWf(origins,ouUWi)else origins.get("Items")
  origins_to_create=ouUWn([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if ouUWf(aliases,ouUWi)else aliases.get("Items")
  aliases_to_create=ouUWn(aliases)
  for distr in distrs:
   origins=ouUWn([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=ouUWn(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",ouUWc)
   config["Enabled"]=ouUWr(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if ouUWf(custom_errors,ouUWB)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=ouUWr(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":ouUWc}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=ouUWr(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=ouUWk):
    parent=config if parent is ouUWk else parent
    items=parent.get(key)or[]
    if ouUWf(items,ouUWB)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":ouUWw(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if ouUWf(cache_behavs,ouUWB)else cache_behavs)
