...
CAPTURE_METAMODEL_SEPARATELY=GQCjF
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=GQCjw(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=GQCjw(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->GQCjE:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[GQCjE,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,GQCjm)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(GQCjm,state_files_to)
...
class CommitMetamodelUtils:
 @GQCjI
 def create_metadata_archive(cls,version:Version,delete_reference:bool=GQCjF,overwrite:bool=GQCjF,metamodels_file:GQCjE=GQCjm):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @GQCjI
 def create_delta_metamodel_file(cls,version:GQCjv,revision:GQCjv,store_to_zip=GQCjF)->Tuple[Dict,Dict]:
 ...
 @GQCjI
 def reconstruct_metamodel(cls,version:GQCjv,revision:GQCjv)->Dict:
  result={}
  for rev in GQCjh(1,revision+1):
  ...
  return result
 @GQCjI
 def get_version_metamodel(cls,version:GQCjv,revision:GQCjv=GQCjm)->Dict:
 ...
 @GQCjI
 def get_commit_diff(cls,version_no:GQCjv,commit_no:GQCjv)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->GQCjA:
 if GQCja(clazz,GQCjS):
  return{}
 constructor=GQCjO(clazz,"__init__",GQCjm)
...
def _create_metamodel_helper(obj,width=25,visited:set=GQCjm):
 if obj is GQCjm:
  return obj
