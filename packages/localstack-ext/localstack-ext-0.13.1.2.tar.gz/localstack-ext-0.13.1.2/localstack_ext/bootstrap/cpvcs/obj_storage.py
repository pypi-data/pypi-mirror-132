...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=MSheG
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:MSheN)->MSheN:
  pass
 @MSheI
 def get(cls,requested_file_locator:MSheN):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=MSheG
 @MSheI
 def get(cls,state_file_locator:MSheN,requested_storage:MSheN,serializers:Dict[MSheN,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[MSheN,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:MSheN)->MSheN:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:MSheN)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:MSheN)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:MSheN)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:MSheN)->MSheN:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[MSheN]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:MSheN,new_key:MSheN,referenced_by_version:MSheN=MSheG):
  pass
 @abc.abstractmethod
 def version_exists(self,key:MSheN)->MSheX:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:MSheN,key:MSheN):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:MSheN,new_key:MSheN,base_path:MSheN)->MSheX:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[MSheN]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[MSheN,MSheN],remote_location:MSheN=MSheG)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[MSheN]:
  ...
  for cpvcs_obj in args:
   if MSheE(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif MSheE(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[MSheN,MSheN],remote_location:MSheN=MSheG)->List[Union[Revision,Version]]:
