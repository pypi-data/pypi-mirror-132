...
class DecryptionHandler:
 decryption_key:VyMCF
 def __init__(self,decryption_key:VyMCF):
  self.decryption_key=decryption_key
 def decrypt(self,content)->VyMCF:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=VyMCu):
  if path and not VyMCG(path,VyMCq):
   path=VyMCq(VyMCm(path,"_path",[]))
  if not path:
   return VyMCu
...
def patch_traceback_lines():
 if VyMCm(traceback.FrameSummary,"_ls_patch_applied",VyMCu):
  return
 @VyMCK
 def line(self):
  try:
 ...
 VyMCP(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=VyMCf
def patch_inspect_findsource():
 if VyMCm(inspect,"_ls_patch_applied",VyMCu):
  return
 ...
 inspect._ls_patch_applied=VyMCf
