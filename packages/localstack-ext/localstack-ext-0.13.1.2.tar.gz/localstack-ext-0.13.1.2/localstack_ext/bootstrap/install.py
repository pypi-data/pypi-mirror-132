...
def install_package(packages:Union[List[fKTkv],fKTkv],api_name:fKTkv):
 if not(is_debian()and in_docker()):
  ...
  return
 if fKTkw(packages,fKTkv):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=fKTkp)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=fKTkp)
def check_or_install(command:fKTkv,packages:Union[List[fKTkv],fKTkv],api:fKTkv):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:fKTkv,target_file:fKTkv,timeout:int=fKTkc):
 def do_download(url,print_error=fKTkp):
  try:
