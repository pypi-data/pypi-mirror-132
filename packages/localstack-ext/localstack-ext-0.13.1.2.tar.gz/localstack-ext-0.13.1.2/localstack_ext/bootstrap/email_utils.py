...
SENT_EMAILS=[]
EMAIL_BLACKLIST=NJEmd()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",NJEmf(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=NJEmK,from_name=NJEmK,smtp_host=NJEmK,smtp_user=NJEmK,smtp_pass=NJEmK,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if NJEmX(recipients,NJEmo)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=NJEmK):
 ...
 result["From"]=formataddr((NJEmf(Header(from_name,"utf-8")),from_email))
 return result
