...
class PodInfo:
 def __init__(self,name=xgIVo,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(xgIVr):
 BACKEND="_none_"
 def __init__(self,pod_name=xgIVo,config=xgIVo):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise xgIVc("Not implemented")
 def delete(self,remote:xgIVU)->xgIVU:
  raise xgIVc("Not implemented")
 def push(self,comment:xgIVX=xgIVo,three_way:xgIVU=xgIVR)->PodInfo:
  raise xgIVc("Not implemented")
 def push_overwrite(self,version:xgIVT,comment:xgIVX=xgIVo):
  pass
 def pull(self,inject_version_state:xgIVU=xgIVR,reset_state_before:xgIVU=xgIVR,lazy:xgIVU=xgIVR):
  raise xgIVc("Not implemented")
 def commit(self,message:xgIVX=xgIVo):
  raise xgIVc("Not implemented")
 def inject(self,version:xgIVT,reset_state:xgIVU)->xgIVU:
  raise xgIVc("Not implemented")
 def list_versions(self)->List[xgIVX]:
  raise xgIVc("Not implemented")
 def version_info(self,version:xgIVT):
  raise xgIVc("Not implemented")
 def version_metamodel(self,version:xgIVT)->Dict:
  raise xgIVc("Not implemented")
 def set_version(self,version:xgIVT,inject_version_state:xgIVU,reset_state:xgIVU,commit_before:xgIVU):
  raise xgIVc("Not implemented")
 def list_version_commits(self,version:xgIVT)->List[xgIVX]:
  raise xgIVc("Not implemented")
 def get_commit_diff(self,version:xgIVT,commit:xgIVT)->Dict:
  raise xgIVc("Not implemented")
 def register_remote(self,pod_name:xgIVX,ci_pod:xgIVU)->xgIVU:
  raise xgIVc("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->xgIVU:
  raise xgIVc("Not implemented")
 def list_pods(self,fetch_remote:xgIVU)->Set[xgIVX]:
  raise xgIVc("Not implemented")
 ...
 def get(cls,pod_name,pre_config=xgIVo):
  ...
  raise xgIVc('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=xgIVR
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=xgIVm)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=xgIVm
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise xgIVc("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @xgIVB
 def get_state_zip_from_instance(get_content=xgIVR):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise xgIVc("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:xgIVX=xgIVo):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @xgIVB
 def parse_pod_name_from_qualifying_name(qualifying_name:xgIVX)->xgIVX:
 ...
 @xgIVB
 def _get_max_version_for_pod_from_platform(pod_name:xgIVX,auth_headers):
  ...
  remote_max_ver=xgIVT(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with xgIVP(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with xgIVP(meta_archive,"rb")as meta_archive_content:
   ...
   with xgIVP(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=xgIVo)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:xgIVX):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:xgIVU=xgIVR):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:xgIVU)->xgIVU:
  ...
  if remote:
   pass
  return xgIVR
 def _push_to_remote(self,url:xgIVX):
 ...
 def pull(self,inject_version_state:xgIVU=xgIVR,reset_state_before:xgIVU=xgIVR,lazy:xgIVU=xgIVR):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=xgIVR):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(xgIVb(lambda ver:xgIVX(ver),xgIVD(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:xgIVT,retain:xgIVU=xgIVR)->Dict[xgIVX,xgIVX]:
 ...
 def _get_presigned_url_for_version_product(self,version:xgIVT)->Dict:
 ...
 def _inject_from_remote(self,version:xgIVT,retain:xgIVU=xgIVR)->xgIVU:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:xgIVT,reset_state:xgIVU)->xgIVU:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=xgIVm)
    if not product_archive_path:
     return xgIVR
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=xgIVm,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_metamodel(self,version:xgIVT)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=xgIVm)
   result=cpvcs_api.CommitMetamodelUtils.create_metamodel_from_state_files(version=version)
  return result[0]
 def set_version(self,version:xgIVT,inject_version_state:xgIVU,reset_state:xgIVU,commit_before:xgIVU):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def get_commit_diff(self,version:xgIVT,commit:xgIVT)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:xgIVX,ci_pod:xgIVU=xgIVR)->xgIVU:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:xgIVU)->Set[xgIVX]:
  ...
  return result
 @xgIVY
 def upload_content(cls,presigned_url:xgIVX,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise xgIVc("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @xgIVB
 def create_platform_url(request:xgIVX)->xgIVX:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:xgIVX=xgIVo,three_way:xgIVU=xgIVR)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=xgIVm)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(xgIVO(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=xgIVO(zip_data_content)
  return result
 def pull(self,inject_version_state:xgIVU=xgIVR,reset_state_before:xgIVU=xgIVR,lazy:xgIVU=xgIVR):
  ...
  rm_rf(zip_path)
 @xgIVB
 def presigned_url(pod_name:xgIVX,mode:xgIVX)->xgIVX:
  ...
  if response.status_code>=400:
   raise xgIVc("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise xgIVc("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:xgIVX=xgIVo,three_way:xgIVU=xgIVR):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=xgIVm)
 ...
 def pull(self,inject_version_state:xgIVU=xgIVR,reset_state_before:xgIVU=xgIVR,lazy:xgIVU=xgIVR):
  ...
  if remote_ref not in remote_refs:
   raise xgIVc('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=xgIVp(self,"clone_dir",xgIVo)
...
class PodConfigManagerMeta(xgIVa):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=xgIVo
   for manager in cls.CHAIN:
    try:
     tmp=xgIVp(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif xgIVs(tmp,xgIVt)and xgIVs(result,xgIVt):
       result.extend(tmp)
    except xgIVc:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not xgIVo:
    return result
   raise xgIVc('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(xgIVr,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @xgIVY
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise xgIVc('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(xgIVr):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=xgIVm).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=xgIVJ([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=xgIVR):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except xgIVc:
   if safe:
    return{}
   raise xgIVc('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(xgIVr):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise xgIVc("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise xgIVc("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:xgIVX,remote:xgIVU,pre_config:Dict[xgIVX,xgIVX])->xgIVU:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],**kwargs)->xgIVU:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",xgIVR))
 return result
def rename_pod(current_pod_name:xgIVX,new_pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:xgIVU,pre_config:Dict[xgIVX,xgIVX],**kwargs)->List[xgIVX]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],message:xgIVX=xgIVo,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:xgIVX,version:xgIVT,reset_state:xgIVU,pre_config:Dict[xgIVX,xgIVX],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],**kwargs)->List[xgIVX]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:xgIVT,pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:xgIVT,pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:xgIVT,inject_version_state:xgIVU,reset_state:xgIVU,commit_before:xgIVU,pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX],**kwargs)->xgIVU:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:xgIVT,pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX])->List[xgIVX]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:xgIVT,commit:xgIVT,pod_name:xgIVX,pre_config:Dict[xgIVX,xgIVX])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:xgIVT,pod_name:xgIVX,comment:xgIVX,pre_config:Dict[xgIVX,xgIVX]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=xgIVo,squash_commits=xgIVR,comment=xgIVo,three_way=xgIVR,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=xgIVR,reset_state_before=xgIVR,lazy=xgIVR,**kwargs):
 pre_config=kwargs.get("pre_config",xgIVo)
 if not pod_name:
  raise xgIVc("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=xgIVR,exclude_from_reset:List[xgIVX]=xgIVo):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise xgIVc("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->xgIVX:
 try:
...
def get_persisted_resource_names(data_dir)->List[xgIVX]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and xgIVO(os.listdir(entry.path))>0:
    names.append(entry.name)
