...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return yTOtU if e else yTOtG
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->yTOtE:
...
@click.option("--revert",is_flag=yTOtU,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:yTOtd):
...
def _cpvcs_initialized(pod_name:yTOtj)->yTOtd:
...
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_init(name:yTOtj):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=yTOtU,default=yTOtG)
def cmd_cpvcs_delete(name:yTOtj,remote:yTOtd):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_register_remote(name:yTOtj):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=yTOtU)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=yTOtU)
def cmd_cpvcs_rename(name:yTOtj,new_name:yTOtj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_commit(message:yTOtj,name:yTOtj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=yTOtU,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=yTOtU,default=yTOtG,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_push(squash:yTOtd,message:yTOtj,name:yTOtj,three_way:yTOtd):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
@click.option("-v","--version",type=yTOtC)
@click.option("-m","--message",required=yTOtG)
def cmd_cpvcs_push_overwrite(version:yTOtC,message:yTOtj,name:yTOtj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=yTOtC,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=yTOtU,default=yTOtG,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_inject(version:yTOtC,reset:yTOtd,name:yTOtj):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=yTOtU,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=yTOtU,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
@click.option("--lazy/--eager",default=yTOtU,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:yTOtj,inject:yTOtd,reset:yTOtd,lazy:yTOtd):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=yTOtU,default=yTOtG)
def cmd_cpvcs_list_pods(remote:yTOtd):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_versions(name:yTOtj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=yTOtU,type=yTOtC)
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_version_info(version:yTOtC,name:yTOtj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=yTOtC,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_version_metamodel(version:yTOtC,name:yTOtj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=yTOtU,type=yTOtC,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=yTOtU,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=yTOtU,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=yTOtG,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_set_version(version:yTOtC,inject:yTOtd,reset:yTOtd,commit_before:yTOtd,name:yTOtj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_commits(version:yTOtC,name:yTOtj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=yTOtU)
@click.option("--commit","-c",required=yTOtU)
@click.option("-n","--name",help="Name of the cloud pod",required=yTOtU)
def cmd_cpvcs_commit_diff(version:yTOtC,commit:yTOtC,name:yTOtj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(yTOtE)
  status.stop()
...
def print_table(columns:Dict[yTOtj,yTOtj],rows:List[Dict[yTOtj,Any]],formatters:Dict[yTOtj,Callable[[Any],yTOtj]]=yTOtE):
 from rich.table import Table
 if formatters is yTOtE:
  formatters=yTOtF()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  yTOtL(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->yTOtE:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->yTOtE:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->yTOtE:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->yTOtE:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
