...
def api_key_configured()->ymrBG:
 return(ymrBl if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else ymrBC)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if ymrBN([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
