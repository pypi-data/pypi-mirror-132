...
def api_key_configured()->JEVHT:
 return(JEVHN if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else JEVHP)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if JEVHf([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
