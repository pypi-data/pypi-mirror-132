...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return zamWX if e else zamWw
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->zamWq:
...
@click.option("--revert",is_flag=zamWX,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:zamWL):
...
def _cpvcs_initialized(pod_name:zamWY)->zamWL:
...
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_init(name:zamWY):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=zamWX,default=zamWw)
def cmd_cpvcs_delete(name:zamWY,remote:zamWL):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_register_remote(name:zamWY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=zamWX)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=zamWX)
def cmd_cpvcs_rename(name:zamWY,new_name:zamWY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_commit(message:zamWY,name:zamWY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=zamWX,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=zamWX,default=zamWw,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_push(squash:zamWL,message:zamWY,name:zamWY,three_way:zamWL):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
@click.option("-v","--version",type=zamWB)
@click.option("-m","--message",required=zamWw)
def cmd_cpvcs_push_overwrite(version:zamWB,message:zamWY,name:zamWY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=zamWB,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=zamWX,default=zamWw,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_inject(version:zamWB,reset:zamWL,name:zamWY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=zamWX,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=zamWX,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
@click.option("--lazy/--eager",default=zamWX,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:zamWY,inject:zamWL,reset:zamWL,lazy:zamWL):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=zamWX,default=zamWw)
def cmd_cpvcs_list_pods(remote:zamWL):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_versions(name:zamWY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=zamWX,type=zamWB)
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_version_info(version:zamWB,name:zamWY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=zamWB,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_version_metamodel(version:zamWB,name:zamWY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=zamWX,type=zamWB,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=zamWX,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=zamWX,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=zamWw,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_set_version(version:zamWB,inject:zamWL,reset:zamWL,commit_before:zamWL,name:zamWY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_commits(version:zamWB,name:zamWY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=zamWX)
@click.option("--commit","-c",required=zamWX)
@click.option("-n","--name",help="Name of the cloud pod",required=zamWX)
def cmd_cpvcs_commit_diff(version:zamWB,commit:zamWB,name:zamWY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(zamWq)
  status.stop()
...
def print_table(columns:Dict[zamWY,zamWY],rows:List[Dict[zamWY,Any]],formatters:Dict[zamWY,Callable[[Any],zamWY]]=zamWq):
 from rich.table import Table
 if formatters is zamWq:
  formatters=zamWn()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  zamWP(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->zamWq:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->zamWq:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->zamWq:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->zamWq:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
