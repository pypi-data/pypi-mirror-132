...
def install_package(packages:Union[List[lIjLr],lIjLr],api_name:lIjLr):
 if not(is_debian()and in_docker()):
  ...
  return
 if lIjLJ(packages,lIjLr):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=lIjLg)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=lIjLg)
def check_or_install(command:lIjLr,packages:Union[List[lIjLr],lIjLr],api:lIjLr):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:lIjLr,target_file:lIjLr,timeout:int=lIjLw):
 def do_download(url,print_error=lIjLg):
  try:
