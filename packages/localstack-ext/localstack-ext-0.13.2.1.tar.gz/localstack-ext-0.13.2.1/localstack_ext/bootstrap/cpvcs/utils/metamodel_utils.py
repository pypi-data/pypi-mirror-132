...
CAPTURE_METAMODEL_SEPARATELY=WhdXG
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=WhdXS(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=WhdXS(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->WhdXI:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[WhdXI,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,WhdXV)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(WhdXV,state_files_to)
...
class CommitMetamodelUtils:
 @WhdXR
 def create_metadata_archive(cls,version:Version,delete_reference:bool=WhdXG,overwrite:bool=WhdXG,metamodels_file:WhdXI=WhdXV):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @WhdXR
 def create_delta_metamodel_file(cls,version:WhdXJ,revision:WhdXJ,store_to_zip=WhdXG)->Tuple[Dict,Dict]:
 ...
 @WhdXR
 def reconstruct_metamodel(cls,version:WhdXJ,revision:WhdXJ=WhdXV)->Dict:
  result={}
  for rev in WhdXF(1,revision+1):
  ...
  return result
 @WhdXR
 def get_version_metamodel(cls,version:WhdXJ,revision:WhdXJ=WhdXV)->Dict:
 ...
 @WhdXR
 def get_commit_diff(cls,version_no:WhdXJ,commit_no:WhdXJ)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->WhdXo:
 if WhdXf(clazz,WhdXr):
  return{}
 constructor=WhdXl(clazz,"__init__",WhdXV)
...
def _create_metamodel_helper(obj,width=25,visited:set=WhdXV):
 if obj is WhdXV:
  return obj
