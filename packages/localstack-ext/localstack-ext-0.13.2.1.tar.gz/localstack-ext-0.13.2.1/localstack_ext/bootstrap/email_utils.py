...
SENT_EMAILS=[]
EMAIL_BLACKLIST=hKFMn()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",hKFMo(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=hKFMm,from_name=hKFMm,smtp_host=hKFMm,smtp_user=hKFMm,smtp_pass=hKFMm,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if hKFMI(recipients,hKFMB)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=hKFMm):
 ...
 result["From"]=formataddr((hKFMo(Header(from_name,"utf-8")),from_email))
 return result
