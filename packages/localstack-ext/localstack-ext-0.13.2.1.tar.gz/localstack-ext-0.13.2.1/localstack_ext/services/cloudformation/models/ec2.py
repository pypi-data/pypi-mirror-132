...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=IFLQq,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if IFLQO(perm["IpProtocol"])!=IFLQO(rp["IpProtocol"]):
    continue
 ...
 @IFLQT
 def get_vpc_and_name_for_security_group(cls,group_id:IFLQO)->Tuple[IFLQO,IFLQO]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or IFLQq
  group_name=groups and groups[0]["GroupName"]or IFLQq
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",IFLQq)
   vpc_id=IFLQq
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",IFLQq)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=IFLQO(result.get("IpProtocol"))
   description=result.pop("Description",IFLQq)
   cidr_ipv6=result.pop("CidrIpv6",IFLQq)
