...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[IQmTz])[0]
 def get_physical_resource_id(self,attribute=IQmTz,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if IQmTo(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return IQmTk(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @IQmTR
 def add_defaults(resource,stack_name:IQmTn):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return IQmTk(SESReceiptRule,self).get_cfn_attribute(attribute)
 @IQmTG
