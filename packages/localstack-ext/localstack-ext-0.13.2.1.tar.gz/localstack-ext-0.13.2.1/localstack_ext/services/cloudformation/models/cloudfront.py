...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or pNdli
 @pNdlb
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return pNdlU(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @pNdlX
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if pNdlg(origins,pNdlB)else origins.get("Items")
  origins_to_create=pNdlL([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if pNdlg(aliases,pNdlB)else aliases.get("Items")
  aliases_to_create=pNdlL(aliases)
  for distr in distrs:
   origins=pNdlL([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=pNdlL(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",pNdlc)
   config["Enabled"]=pNdlF(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if pNdlg(custom_errors,pNdly)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=pNdlF(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":pNdlc}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=pNdlF(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=pNdli):
    parent=config if parent is pNdli else parent
    items=parent.get(key)or[]
    if pNdlg(items,pNdly)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":pNdlO(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if pNdlg(cache_behavs,pNdly)else cache_behavs)
