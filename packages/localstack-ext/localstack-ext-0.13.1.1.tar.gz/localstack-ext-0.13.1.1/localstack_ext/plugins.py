...
def api_key_configured()->Azpmv:
 return(AzpmG if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else AzpmI)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if AzpmX([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
