...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or ozKaY
 @ozKaH
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return ozKaC(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @ozKat
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if ozKau(origins,ozKav)else origins.get("Items")
  origins_to_create=ozKaj([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if ozKau(aliases,ozKav)else aliases.get("Items")
  aliases_to_create=ozKaj(aliases)
  for distr in distrs:
   origins=ozKaj([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=ozKaj(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",ozKaV)
   config["Enabled"]=ozKaO(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if ozKau(custom_errors,ozKab)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=ozKaO(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":ozKaV}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=ozKaO(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=ozKaY):
    parent=config if parent is ozKaY else parent
    items=parent.get(key)or[]
    if ozKau(items,ozKab)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":ozKaD(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if ozKau(cache_behavs,ozKab)else cache_behavs)
