...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[oytAw])[0]
 def get_physical_resource_id(self,attribute=oytAw,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if oytAK(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return oytAE(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @oytAW
 def add_defaults(resource,stack_name:oytAq):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return oytAE(SESReceiptRule,self).get_cfn_attribute(attribute)
 @oytAr
