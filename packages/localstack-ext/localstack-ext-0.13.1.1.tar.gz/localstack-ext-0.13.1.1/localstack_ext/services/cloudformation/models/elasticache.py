...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=uXtVb(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not uXtVM
   return result
  except uXtVR:
   props=self.props
   ...
   if parts[0]in candidates and uXtVF(parts)>1:
