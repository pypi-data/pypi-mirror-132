...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=LCBJV,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if LCBJH(perm["IpProtocol"])!=LCBJH(rp["IpProtocol"]):
    continue
 ...
 @LCBJo
 def get_vpc_and_name_for_security_group(cls,group_id:LCBJH)->Tuple[LCBJH,LCBJH]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or LCBJV
  group_name=groups and groups[0]["GroupName"]or LCBJV
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",LCBJV)
   vpc_id=LCBJV
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",LCBJV)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=LCBJH(result.get("IpProtocol"))
   description=result.pop("Description",LCBJV)
   cidr_ipv6=result.pop("CidrIpv6",LCBJV)
