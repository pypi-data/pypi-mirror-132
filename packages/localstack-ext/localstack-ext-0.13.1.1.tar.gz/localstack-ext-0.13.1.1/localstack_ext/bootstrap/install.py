...
def install_package(packages:Union[List[nIkbF],nIkbF],api_name:nIkbF):
 if not(is_debian()and in_docker()):
  ...
  return
 if nIkbW(packages,nIkbF):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=nIkbA)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=nIkbA)
def check_or_install(command:nIkbF,packages:Union[List[nIkbF],nIkbF],api:nIkbF):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:nIkbF,target_file:nIkbF,timeout:int=nIkbc):
 def do_download(url,print_error=nIkbA):
  try:
