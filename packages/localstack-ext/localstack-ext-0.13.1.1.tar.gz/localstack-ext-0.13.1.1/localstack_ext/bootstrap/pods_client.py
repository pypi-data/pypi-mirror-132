...
class PodInfo:
 def __init__(self,name=rRSkT,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(rRSkw):
 BACKEND="_none_"
 def __init__(self,pod_name=rRSkT,config=rRSkT):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise rRSkB("Not implemented")
 def delete(self,remote:rRSkY)->rRSkY:
  raise rRSkB("Not implemented")
 def push(self,comment:rRSkx=rRSkT,three_way:rRSkY=rRSkq)->PodInfo:
  raise rRSkB("Not implemented")
 def push_overwrite(self,version:rRSkK,comment:rRSkx=rRSkT):
  pass
 def pull(self,inject_version_state:rRSkY=rRSkq,reset_state_before:rRSkY=rRSkq,lazy:rRSkY=rRSkq):
  raise rRSkB("Not implemented")
 def commit(self,message:rRSkx=rRSkT):
  raise rRSkB("Not implemented")
 def inject(self,version:rRSkK,reset_state:rRSkY)->rRSkY:
  raise rRSkB("Not implemented")
 def list_versions(self)->List[rRSkx]:
  raise rRSkB("Not implemented")
 def version_info(self,version:rRSkK):
  raise rRSkB("Not implemented")
 def version_metamodel(self,version:rRSkK)->Dict:
  raise rRSkB("Not implemented")
 def set_version(self,version:rRSkK,inject_version_state:rRSkY,reset_state:rRSkY,commit_before:rRSkY):
  raise rRSkB("Not implemented")
 def list_version_commits(self,version:rRSkK)->List[rRSkx]:
  raise rRSkB("Not implemented")
 def get_commit_diff(self,version:rRSkK,commit:rRSkK)->Dict:
  raise rRSkB("Not implemented")
 def register_remote(self,pod_name:rRSkx,ci_pod:rRSkY)->rRSkY:
  raise rRSkB("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->rRSkY:
  raise rRSkB("Not implemented")
 def list_pods(self,fetch_remote:rRSkY)->Set[rRSkx]:
  raise rRSkB("Not implemented")
 ...
 def get(cls,pod_name,pre_config=rRSkT):
  ...
  raise rRSkB('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=rRSkq
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=rRSkE)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=rRSkE
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise rRSkB("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @rRSkP
 def get_state_zip_from_instance(get_content=rRSkq):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise rRSkB("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:rRSkx=rRSkT):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @rRSkP
 def parse_pod_name_from_qualifying_name(qualifying_name:rRSkx)->rRSkx:
 ...
 @rRSkP
 def _get_max_version_for_pod_from_platform(pod_name:rRSkx,auth_headers):
  ...
  remote_max_ver=rRSkK(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with rRSkL(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with rRSkL(meta_archive,"rb")as meta_archive_content:
   ...
   with rRSkL(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=rRSkT)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:rRSkx):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:rRSkY=rRSkq):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:rRSkY)->rRSkY:
  ...
  if remote:
   pass
  return rRSkq
 def _push_to_remote(self,url:rRSkx):
 ...
 def pull(self,inject_version_state:rRSkY=rRSkq,reset_state_before:rRSkY=rRSkq,lazy:rRSkY=rRSkq):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=rRSkq):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(rRSkU(lambda ver:rRSkx(ver),rRSkc(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:rRSkK,retain:rRSkY=rRSkq)->Dict[rRSkx,rRSkx]:
 ...
 def _get_presigned_url_for_version_product(self,version:rRSkK)->Dict:
 ...
 def _inject_from_remote(self,version:rRSkK,retain:rRSkY=rRSkq)->rRSkY:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:rRSkK,reset_state:rRSkY)->rRSkY:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=rRSkE)
    if not product_archive_path:
     return rRSkq
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=rRSkE,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_metamodel(self,version:rRSkK)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=rRSkE)
   result=cpvcs_api.CommitMetamodelUtils.create_metamodel_from_state_files(version=version)
  return result[0]
 def set_version(self,version:rRSkK,inject_version_state:rRSkY,reset_state:rRSkY,commit_before:rRSkY):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def get_commit_diff(self,version:rRSkK,commit:rRSkK)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:rRSkx,ci_pod:rRSkY=rRSkq)->rRSkY:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:rRSkY)->Set[rRSkx]:
  ...
  return result
 @rRSkl
 def upload_content(cls,presigned_url:rRSkx,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise rRSkB("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @rRSkP
 def create_platform_url(request:rRSkx)->rRSkx:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:rRSkx=rRSkT,three_way:rRSkY=rRSkq)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=rRSkE)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(rRSkm(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=rRSkm(zip_data_content)
  return result
 def pull(self,inject_version_state:rRSkY=rRSkq,reset_state_before:rRSkY=rRSkq,lazy:rRSkY=rRSkq):
  ...
  rm_rf(zip_path)
 @rRSkP
 def presigned_url(pod_name:rRSkx,mode:rRSkx)->rRSkx:
  ...
  if response.status_code>=400:
   raise rRSkB("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise rRSkB("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:rRSkx=rRSkT,three_way:rRSkY=rRSkq):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=rRSkE)
 ...
 def pull(self,inject_version_state:rRSkY=rRSkq,reset_state_before:rRSkY=rRSkq,lazy:rRSkY=rRSkq):
  ...
  if remote_ref not in remote_refs:
   raise rRSkB('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=rRSka(self,"clone_dir",rRSkT)
...
class PodConfigManagerMeta(rRSkh):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=rRSkT
   for manager in cls.CHAIN:
    try:
     tmp=rRSka(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif rRSkG(tmp,rRSkd)and rRSkG(result,rRSkd):
       result.extend(tmp)
    except rRSkB:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not rRSkT:
    return result
   raise rRSkB('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(rRSkw,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @rRSkl
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise rRSkB('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(rRSkw):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=rRSkE).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=rRSkg([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=rRSkq):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except rRSkB:
   if safe:
    return{}
   raise rRSkB('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(rRSkw):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise rRSkB("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise rRSkB("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:rRSkx,remote:rRSkY,pre_config:Dict[rRSkx,rRSkx])->rRSkY:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],**kwargs)->rRSkY:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",rRSkq))
 return result
def rename_pod(current_pod_name:rRSkx,new_pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:rRSkY,pre_config:Dict[rRSkx,rRSkx],**kwargs)->List[rRSkx]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],message:rRSkx=rRSkT,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:rRSkx,version:rRSkK,reset_state:rRSkY,pre_config:Dict[rRSkx,rRSkx],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],**kwargs)->List[rRSkx]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:rRSkK,pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:rRSkK,pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:rRSkK,inject_version_state:rRSkY,reset_state:rRSkY,commit_before:rRSkY,pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx],**kwargs)->rRSkY:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:rRSkK,pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx])->List[rRSkx]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:rRSkK,commit:rRSkK,pod_name:rRSkx,pre_config:Dict[rRSkx,rRSkx])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:rRSkK,pod_name:rRSkx,comment:rRSkx,pre_config:Dict[rRSkx,rRSkx]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=rRSkT,squash_commits=rRSkq,comment=rRSkT,three_way=rRSkq,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=rRSkq,reset_state_before=rRSkq,lazy=rRSkq,**kwargs):
 pre_config=kwargs.get("pre_config",rRSkT)
 if not pod_name:
  raise rRSkB("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=rRSkq,exclude_from_reset:List[rRSkx]=rRSkT):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise rRSkB("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->rRSkx:
 try:
...
def get_persisted_resource_names(data_dir)->List[rRSkx]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and rRSkm(os.listdir(entry.path))>0:
    names.append(entry.name)
