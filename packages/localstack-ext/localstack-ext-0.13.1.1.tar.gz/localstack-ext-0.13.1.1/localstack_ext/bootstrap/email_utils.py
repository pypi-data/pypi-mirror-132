...
SENT_EMAILS=[]
EMAIL_BLACKLIST=OKcvQ()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",OKcvV(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=OKcvM,from_name=OKcvM,smtp_host=OKcvM,smtp_user=OKcvM,smtp_pass=OKcvM,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OKcvg(recipients,OKcvi)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=OKcvM):
 ...
 result["From"]=formataddr((OKcvV(Header(from_name,"utf-8")),from_email))
 return result
