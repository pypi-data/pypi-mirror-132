...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=IztOp
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:IztOn)->IztOn:
  pass
 @IztOu
 def get(cls,requested_file_locator:IztOn):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=IztOp
 @IztOu
 def get(cls,state_file_locator:IztOn,requested_storage:IztOn,serializers:Dict[IztOn,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[IztOn,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:IztOn)->IztOn:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:IztOn)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:IztOn)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:IztOn)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:IztOn)->IztOn:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[IztOn]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:IztOn,new_key:IztOn,referenced_by_version:IztOn=IztOp):
  pass
 @abc.abstractmethod
 def version_exists(self,key:IztOn)->IztOF:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:IztOn,key:IztOn):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:IztOn,new_key:IztOn,base_path:IztOn)->IztOF:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[IztOn]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[IztOn,IztOn],remote_location:IztOn=IztOp)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[IztOn]:
  ...
  for cpvcs_obj in args:
   if IztOc(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif IztOc(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[IztOn,IztOn],remote_location:IztOn=IztOp)->List[Union[Revision,Version]]:
