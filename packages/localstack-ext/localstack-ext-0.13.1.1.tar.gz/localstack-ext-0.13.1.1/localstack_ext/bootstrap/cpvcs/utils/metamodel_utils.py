...
from localstack_ext.utils.common import SubtypesInstanceManager
LOG=logging.getLogger(__name__)
CAPTURE_METAMODEL_SEPARATELY=LkcAs
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=LkcAl(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=LkcAl(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->LkcAW:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[LkcAW,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,LkcAy)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(LkcAy,state_files_to)
...
class CommitMetamodelUtils:
 @LkcAe
 def create_metadata_archive(cls,version:Version,delete_reference:bool=LkcAs,overwrite:bool=LkcAs,metamodels_file:LkcAW=LkcAy):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @LkcAe
 def create_delta_metamodel_file(cls,version:LkcAK,revision:LkcAK,store_to_zip=LkcAs)->Tuple[Dict,Dict]:
 ...
 @LkcAe
 def reconstruct_metamodel(cls,version:LkcAK,revision:LkcAK)->Dict:
  result={}
  for rev in LkcAa(1,revision+1):
  ...
  return result
 @LkcAe
 def get_version_metamodel(cls,version:LkcAK,revision:LkcAK=LkcAy)->Dict:
 ...
 @LkcAe
 def get_commit_diff(cls,version_no:LkcAK,commit_no:LkcAK)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->LkcAQ:
 if LkcAt(clazz,LkcAq):
  return{}
 constructor=LkcAO(clazz,"__init__",LkcAy)
...
def _create_metamodel_helper(obj,width=25,visited:set=LkcAy):
 if obj is LkcAy:
  return obj
