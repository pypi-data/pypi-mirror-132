...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return IKBlE if e else IKBla
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->IKBlY:
...
@click.option("--revert",is_flag=IKBlE,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:IKBlx):
...
def _cpvcs_initialized(pod_name:IKBlh)->IKBlx:
...
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_init(name:IKBlh):
...
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=IKBlE,default=IKBla)
def cmd_cpvcs_delete(name:IKBlh,remote:IKBlx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_register_remote(name:IKBlh):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=IKBlE)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=IKBlE)
def cmd_cpvcs_rename(name:IKBlh,new_name:IKBlh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_commit(message:IKBlh,name:IKBlh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=IKBlE,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=IKBlE,default=IKBla,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_push(squash:IKBlx,message:IKBlh,name:IKBlh,three_way:IKBlx):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
@click.option("-v","--version",type=IKBlT)
@click.option("-m","--message",required=IKBla)
def cmd_cpvcs_push_overwrite(version:IKBlT,message:IKBlh,name:IKBlh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=IKBlT,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=IKBlE,default=IKBla,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_inject(version:IKBlT,reset:IKBlx,name:IKBlh):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=IKBlE,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=IKBlE,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
@click.option("--lazy/--eager",default=IKBlE,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:IKBlh,inject:IKBlx,reset:IKBlx,lazy:IKBlx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=IKBlE,default=IKBla)
def cmd_cpvcs_list_pods(remote:IKBlx):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_versions(name:IKBlh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=IKBlE,type=IKBlT)
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_version_info(version:IKBlT,name:IKBlh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=IKBlT,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_version_metamodel(version:IKBlT,name:IKBlh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=IKBlE,type=IKBlT,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=IKBlE,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=IKBlE,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=IKBla,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_set_version(version:IKBlT,inject:IKBlx,reset:IKBlx,commit_before:IKBlx,name:IKBlh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_commits(version:IKBlT,name:IKBlh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=IKBlE)
@click.option("--commit","-c",required=IKBlE)
@click.option("-n","--name",help="Name of the cloud pod",required=IKBlE)
def cmd_cpvcs_commit_diff(version:IKBlT,commit:IKBlT,name:IKBlh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(IKBlY)
  status.stop()
...
def print_table(columns:Dict[IKBlh,IKBlh],rows:List[Dict[IKBlh,Any]],formatters:Dict[IKBlh,Callable[[Any],IKBlh]]=IKBlY):
 from rich.table import Table
 if formatters is IKBlY:
  formatters=IKBlm()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  IKBlj(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->IKBlY:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->IKBlY:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->IKBlY:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->IKBlY:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
