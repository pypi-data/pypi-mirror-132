...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return hQzOE if e else hQzOM
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->hQzOd:
...
@click.option("--revert",is_flag=hQzOE,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:hQzOG):
...
def _cpvcs_initialized(pod_name:hQzOa)->hQzOG:
...
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_init(name:hQzOa):
...
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=hQzOE,default=hQzOM)
def cmd_cpvcs_delete(name:hQzOa,remote:hQzOG):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_register_remote(name:hQzOa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=hQzOE)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=hQzOE)
def cmd_cpvcs_rename(name:hQzOa,new_name:hQzOa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_commit(message:hQzOa,name:hQzOa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=hQzOE,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=hQzOE,default=hQzOM,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_push(squash:hQzOG,message:hQzOa,name:hQzOa,three_way:hQzOG):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
@click.option("-v","--version",type=hQzOo)
@click.option("-m","--message",required=hQzOM)
def cmd_cpvcs_push_overwrite(version:hQzOo,message:hQzOa,name:hQzOa):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=hQzOo,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=hQzOE,default=hQzOM,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_inject(version:hQzOo,reset:hQzOG,name:hQzOa):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=hQzOE,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=hQzOE,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
@click.option("--lazy/--eager",default=hQzOE,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:hQzOa,inject:hQzOG,reset:hQzOG,lazy:hQzOG):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=hQzOE,default=hQzOM)
def cmd_cpvcs_list_pods(remote:hQzOG):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_versions(name:hQzOa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=hQzOE,type=hQzOo)
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_version_info(version:hQzOo,name:hQzOa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=hQzOo,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_version_metamodel(version:hQzOo,name:hQzOa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=hQzOE,type=hQzOo,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=hQzOE,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=hQzOE,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=hQzOM,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_set_version(version:hQzOo,inject:hQzOG,reset:hQzOG,commit_before:hQzOG,name:hQzOa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_commits(version:hQzOo,name:hQzOa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=hQzOE)
@click.option("--commit","-c",required=hQzOE)
@click.option("-n","--name",help="Name of the cloud pod",required=hQzOE)
def cmd_cpvcs_commit_diff(version:hQzOo,commit:hQzOo,name:hQzOa):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(hQzOd)
  status.stop()
...
def print_table(columns:Dict[hQzOa,hQzOa],rows:List[Dict[hQzOa,Any]],formatters:Dict[hQzOa,Callable[[Any],hQzOa]]=hQzOd):
 from rich.table import Table
 if formatters is hQzOd:
  formatters=hQzOU()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  hQzOW(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->hQzOd:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->hQzOd:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->hQzOd:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->hQzOd:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
