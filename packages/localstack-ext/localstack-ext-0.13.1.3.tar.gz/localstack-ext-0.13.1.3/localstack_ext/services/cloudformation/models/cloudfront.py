...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or SlMgI
 @SlMgN
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return SlMgv(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @SlMgE
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if SlMgr(origins,SlMgG)else origins.get("Items")
  origins_to_create=SlMgd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if SlMgr(aliases,SlMgG)else aliases.get("Items")
  aliases_to_create=SlMgd(aliases)
  for distr in distrs:
   origins=SlMgd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=SlMgd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",SlMgx)
   config["Enabled"]=SlMgf(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if SlMgr(custom_errors,SlMgU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=SlMgf(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":SlMgx}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=SlMgf(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=SlMgI):
    parent=config if parent is SlMgI else parent
    items=parent.get(key)or[]
    if SlMgr(items,SlMgU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":SlMgz(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if SlMgr(cache_behavs,SlMgU)else cache_behavs)
