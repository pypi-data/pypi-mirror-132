...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=cAyYK,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if cAyYS(perm["IpProtocol"])!=cAyYS(rp["IpProtocol"]):
    continue
 ...
 @cAyYJ
 def get_vpc_and_name_for_security_group(cls,group_id:cAyYS)->Tuple[cAyYS,cAyYS]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or cAyYK
  group_name=groups and groups[0]["GroupName"]or cAyYK
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",cAyYK)
   vpc_id=cAyYK
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",cAyYK)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=cAyYS(result.get("IpProtocol"))
   description=result.pop("Description",cAyYK)
   cidr_ipv6=result.pop("CidrIpv6",cAyYK)
