...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[KWPgj])[0]
 def get_physical_resource_id(self,attribute=KWPgj,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if KWPgq(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return KWPgc(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @KWPgV
 def add_defaults(resource,stack_name:KWPgY):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return KWPgc(SESReceiptRule,self).get_cfn_attribute(attribute)
 @KWPgh
