...
def api_key_configured()->uIPBl:
 return(uIPBD if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else uIPBc)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if uIPBF([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
