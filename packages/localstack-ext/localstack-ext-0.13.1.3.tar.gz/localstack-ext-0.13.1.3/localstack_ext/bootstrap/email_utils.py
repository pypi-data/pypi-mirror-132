...
SENT_EMAILS=[]
EMAIL_BLACKLIST=jhRPG()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",jhRPr(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=jhRPg,from_name=jhRPg,smtp_host=jhRPg,smtp_user=jhRPg,smtp_pass=jhRPg,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if jhRPW(recipients,jhRPb)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=jhRPg):
 ...
 result["From"]=formataddr((jhRPr(Header(from_name,"utf-8")),from_email))
 return result
