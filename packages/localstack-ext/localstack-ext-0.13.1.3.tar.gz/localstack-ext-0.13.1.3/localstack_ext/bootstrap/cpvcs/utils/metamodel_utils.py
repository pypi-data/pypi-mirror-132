...
CAPTURE_METAMODEL_SEPARATELY=JQfdP
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=JQfdh(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=JQfdh(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->JQfde:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[JQfde,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,JQfdm)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(JQfdm,state_files_to)
...
class CommitMetamodelUtils:
 @JQfdY
 def create_metadata_archive(cls,version:Version,delete_reference:bool=JQfdP,overwrite:bool=JQfdP,metamodels_file:JQfde=JQfdm):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @JQfdY
 def create_delta_metamodel_file(cls,version:JQfdV,revision:JQfdV,store_to_zip=JQfdP)->Tuple[Dict,Dict]:
 ...
 @JQfdY
 def reconstruct_metamodel(cls,version:JQfdV,revision:JQfdV=JQfdm)->Dict:
  result={}
  for rev in JQfdz(1,revision+1):
  ...
  return result
 @JQfdY
 def get_version_metamodel(cls,version:JQfdV,revision:JQfdV=JQfdm)->Dict:
 ...
 @JQfdY
 def get_commit_diff(cls,version_no:JQfdV,commit_no:JQfdV)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->JQfdU:
 if JQfdy(clazz,JQfda):
  return{}
 constructor=JQfdC(clazz,"__init__",JQfdm)
...
def _create_metamodel_helper(obj,width=25,visited:set=JQfdm):
 if obj is JQfdm:
  return obj
