...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=gSHYr
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:gSHYJ)->gSHYJ:
  pass
 @gSHYM
 def get(cls,requested_file_locator:gSHYJ):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=gSHYr
 @gSHYM
 def get(cls,state_file_locator:gSHYJ,requested_storage:gSHYJ,serializers:Dict[gSHYJ,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[gSHYJ,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:gSHYJ)->gSHYJ:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:gSHYJ)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:gSHYJ)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:gSHYJ)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:gSHYJ)->gSHYJ:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[gSHYJ]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:gSHYJ,new_key:gSHYJ,referenced_by_version:gSHYJ=gSHYr):
  pass
 @abc.abstractmethod
 def version_exists(self,key:gSHYJ)->gSHYt:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:gSHYJ,key:gSHYJ):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:gSHYJ,new_key:gSHYJ,base_path:gSHYJ)->gSHYt:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[gSHYJ]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[gSHYJ,gSHYJ],remote_location:gSHYJ=gSHYr)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[gSHYJ]:
  ...
  for cpvcs_obj in args:
   if gSHYO(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif gSHYO(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[gSHYJ,gSHYJ],remote_location:gSHYJ=gSHYr)->List[Union[Revision,Version]]:
