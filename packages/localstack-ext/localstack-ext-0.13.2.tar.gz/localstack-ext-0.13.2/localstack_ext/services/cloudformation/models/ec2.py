...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=aiQOI,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if aiQOk(perm["IpProtocol"])!=aiQOk(rp["IpProtocol"]):
    continue
 ...
 @aiQOF
 def get_vpc_and_name_for_security_group(cls,group_id:aiQOk)->Tuple[aiQOk,aiQOk]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or aiQOI
  group_name=groups and groups[0]["GroupName"]or aiQOI
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",aiQOI)
   vpc_id=aiQOI
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",aiQOI)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=aiQOk(result.get("IpProtocol"))
   description=result.pop("Description",aiQOI)
   cidr_ipv6=result.pop("CidrIpv6",aiQOI)
