...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[hrHXc])[0]
 def get_physical_resource_id(self,attribute=hrHXc,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if hrHXe(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return hrHXl(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @hrHXV
 def add_defaults(resource,stack_name:hrHXG):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return hrHXl(SESReceiptRule,self).get_cfn_attribute(attribute)
 @hrHXO
