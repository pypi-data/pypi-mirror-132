...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or dWzjB
 @dWzjQ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return dWzjO(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @dWzjR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if dWzjq(origins,dWzjb)else origins.get("Items")
  origins_to_create=dWzjy([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if dWzjq(aliases,dWzjb)else aliases.get("Items")
  aliases_to_create=dWzjy(aliases)
  for distr in distrs:
   origins=dWzjy([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=dWzjy(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",dWzja)
   config["Enabled"]=dWzjp(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if dWzjq(custom_errors,dWzjV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=dWzjp(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":dWzja}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=dWzjp(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=dWzjB):
    parent=config if parent is dWzjB else parent
    items=parent.get(key)or[]
    if dWzjq(items,dWzjV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":dWzje(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if dWzjq(cache_behavs,dWzjV)else cache_behavs)
