...
def api_key_configured()->QUHOd:
 return(QUHOo if os.environ.get("LOCALSTACK_API_KEY")and os.environ.get("LOCALSTACK_API_KEY").strip()else QUHOW)
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if QUHOP([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
