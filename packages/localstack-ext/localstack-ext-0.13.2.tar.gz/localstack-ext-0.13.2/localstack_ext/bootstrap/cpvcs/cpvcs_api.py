...
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=qeHSt())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=qeHSI,state_files=qeHSt(),version_number=0)
 ...
 with qeHSQ(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with qeHSQ(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with qeHSQ(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:qeHSD,meta_archives:Dict[qeHSD,qeHSD],state_archives:Dict[qeHSD,qeHSD],remote_info:Dict[qeHSD,qeHSD],pod_name:qeHSD):
...
def merge_from_remote(version_space_archive:qeHSD,meta_archives:Dict[qeHSD,qeHSD],state_archives:Dict[qeHSD,qeHSD]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->qeHSJ:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:qeHSD):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:qeHSD)->qeHSJ:
...
def list_locally_available_pods(show_remote_or_local:qeHSJ=qeHSF)->Set[qeHSD]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:qeHSD,file_name:qeHSD,service:qeHSD,region:qeHSD)->qeHSD:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->qeHSD:
 ...
 return key
def _get_state_file_path(key:qeHSD)->qeHSD:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=qeHSt(qeHSm(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:qeHSD)->Optional[qeHSD]:
...
def get_version_info(version_no:qeHSL)->Dict[qeHSD,qeHSD]:
 ...
 return result
def commit(message:qeHSD=qeHSI)->Revision:
 ...
 curr_expansion_point_parent_state_files=qeHSt()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=qeHSI
 ...
 new_expansion_point=Revision(hash_ref=random_hash(),state_files=qeHSt(),parent_ptr=curr_expansion_point_hash,creator=curr_expansion_point.creator,rid=short_uid(),revision_number=curr_expansion_point.revision_number+1)
...
def create_version_space_archive()->qeHSD:
...
def get_max_version_no()->qeHSL:
 with qeHSQ(config_context.get_max_ver_path())as fp:
  return qeHSL(os.path.basename(fp.readline()))
...
def push_overwrite(version:qeHSL,comment:qeHSD)->qeHSJ:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=qeHSF,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return qeHSF
def push(comment:qeHSD=qeHSI,three_way:qeHSJ=qeHSx)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=qeHSt(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,PodFilePaths.get_version_state_archive(new_max_version_no)),state_files=qeHSt(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,qeHSt())
 ...
 return new_version
def _create_state_directory(version_number:qeHSL,state_file_refs:Set[StateFileRef],delete_files=qeHSx,archive=qeHSF):
...
def set_active_version(version_no:qeHSL,commit_before=qeHSx)->qeHSJ:
 ...
 return qeHSx
def _set_active_version(key:qeHSD):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=qeHSt(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:qeHSL)->Version:
 versions=load_version_references()
 version_ref=qeHSK((version[1]for version in versions if version[0]==version_no),qeHSI)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[qeHSL,qeHSD]]:
 result={}
 with qeHSQ(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with qeHSQ(symlink,"r")as sp:
    result[qeHSL(os.path.basename(symlink))]=sp.readline()
 return qeHSG(result.items(),key=lambda x:x[0],reverse=qeHSF)
def list_versions()->List[qeHSD]:
 ...
 return result
def list_version_commits(version_no:qeHSL)->List[qeHSD]:
 if version_no==-1:
 ...
 return result
def _update_head(new_head_ver_no,new_head_key)->qeHSD:
 with qeHSQ(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->qeHSD:
 with qeHSQ(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->qeHSD:
 with qeHSQ(config_context.get_known_ver_path(),"a")as fp:
...
def _is_special_case(obj)->qeHSJ:
 return qeHSn(obj,(Queue,FakeBucket))
...
def _merge_two_state_dir_func(**kwargs):
 ...
 if os.path.isfile(dst_state_file_path):
  ...
  mkdir(dst_state_dir)
  persist_object(src_state,dst_state_file_path)
def _merge_expansion_point_with_max(three_way=qeHSx):
 ...
 common_ancestor_state_archive=qeHSI
 tmp_common_ancestor_state_dir=qeHSI
 ...
 new_max_version_states_dir=_create_state_directory(version_number=new_version_no,state_file_refs=expansion_point.state_files,archive=qeHSx)
...
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->qeHSD:
 try:
