...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=SmXnJ
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:SmXnF)->SmXnF:
  pass
 @SmXnN
 def get(cls,requested_file_locator:SmXnF):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=SmXnJ
 @SmXnN
 def get(cls,state_file_locator:SmXnF,requested_storage:SmXnF,serializers:Dict[SmXnF,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[SmXnF,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:SmXnF)->SmXnF:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:SmXnF)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:SmXnF)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:SmXnF)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:SmXnF)->SmXnF:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[SmXnF]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:SmXnF,new_key:SmXnF,referenced_by_version:SmXnF=SmXnJ):
  pass
 @abc.abstractmethod
 def version_exists(self,key:SmXnF)->SmXni:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:SmXnF,key:SmXnF):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:SmXnF,new_key:SmXnF,base_path:SmXnF)->SmXni:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[SmXnF]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[SmXnF,SmXnF],remote_location:SmXnF=SmXnJ)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[SmXnF]:
  ...
  for cpvcs_obj in args:
   if SmXnv(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif SmXnv(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[SmXnF,SmXnF],remote_location:SmXnF=SmXnJ)->List[Union[Revision,Version]]:
