...
CAPTURE_METAMODEL_SEPARATELY=WhwMr
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=WhwMT(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=WhwMT(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->WhwMp:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[WhwMp,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,WhwMO)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(WhwMO,state_files_to)
...
class CommitMetamodelUtils:
 @WhwMa
 def create_metadata_archive(cls,version:Version,delete_reference:bool=WhwMr,overwrite:bool=WhwMr,metamodels_file:WhwMp=WhwMO):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @WhwMa
 def create_delta_metamodel_file(cls,version:WhwMF,revision:WhwMF,store_to_zip=WhwMr)->Tuple[Dict,Dict]:
 ...
 @WhwMa
 def reconstruct_metamodel(cls,version:WhwMF,revision:WhwMF=WhwMO)->Dict:
  result={}
  for rev in WhwMS(1,revision+1):
  ...
  return result
 @WhwMa
 def get_version_metamodel(cls,version:WhwMF,revision:WhwMF=WhwMO)->Dict:
 ...
 @WhwMa
 def get_commit_diff(cls,version_no:WhwMF,commit_no:WhwMF)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->WhwMn:
 if WhwMN(clazz,WhwMj):
  return{}
 constructor=WhwMt(clazz,"__init__",WhwMO)
...
def _create_metamodel_helper(obj,width=25,visited:set=WhwMO):
 if obj is WhwMO:
  return obj
