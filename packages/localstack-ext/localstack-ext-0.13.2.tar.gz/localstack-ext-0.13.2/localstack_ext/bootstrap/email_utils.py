...
SENT_EMAILS=[]
EMAIL_BLACKLIST=MkwJo()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",MkwJz(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=MkwJG,from_name=MkwJG,smtp_host=MkwJG,smtp_user=MkwJG,smtp_pass=MkwJG,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if MkwJq(recipients,MkwJA)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=MkwJG):
 ...
 result["From"]=formataddr((MkwJz(Header(from_name,"utf-8")),from_email))
 return result
