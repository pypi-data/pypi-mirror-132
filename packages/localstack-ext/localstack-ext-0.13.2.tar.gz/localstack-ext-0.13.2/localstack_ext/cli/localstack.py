...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return zOWSG if e else zOWSh
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->zOWST:
...
@click.option("--revert",is_flag=zOWSG,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:zOWSP):
...
def _cpvcs_initialized(pod_name:zOWSv)->zOWSP:
...
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_init(name:zOWSv):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=zOWSG,default=zOWSh)
def cmd_cpvcs_delete(name:zOWSv,remote:zOWSP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_register_remote(name:zOWSv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=zOWSG)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=zOWSG)
def cmd_cpvcs_rename(name:zOWSv,new_name:zOWSv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_commit(message:zOWSv,name:zOWSv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=zOWSG,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=zOWSG,default=zOWSh,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_push(squash:zOWSP,message:zOWSv,name:zOWSv,three_way:zOWSP):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
@click.option("-v","--version",type=zOWSA)
@click.option("-m","--message",required=zOWSh)
def cmd_cpvcs_push_overwrite(version:zOWSA,message:zOWSv,name:zOWSv):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=zOWSA,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=zOWSG,default=zOWSh,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_inject(version:zOWSA,reset:zOWSP,name:zOWSv):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=zOWSG,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=zOWSG,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
@click.option("--lazy/--eager",default=zOWSG,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:zOWSv,inject:zOWSP,reset:zOWSP,lazy:zOWSP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=zOWSG,default=zOWSh)
def cmd_cpvcs_list_pods(remote:zOWSP):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_versions(name:zOWSv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=zOWSG,type=zOWSA)
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_version_info(version:zOWSA,name:zOWSv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=zOWSA,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_version_metamodel(version:zOWSA,name:zOWSv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=zOWSG,type=zOWSA,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=zOWSG,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=zOWSG,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=zOWSh,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_set_version(version:zOWSA,inject:zOWSP,reset:zOWSP,commit_before:zOWSP,name:zOWSv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_commits(version:zOWSA,name:zOWSv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=zOWSG)
@click.option("--commit","-c",required=zOWSG)
@click.option("-n","--name",help="Name of the cloud pod",required=zOWSG)
def cmd_cpvcs_commit_diff(version:zOWSA,commit:zOWSA,name:zOWSv):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(zOWST)
  status.stop()
...
def print_table(columns:Dict[zOWSv,zOWSv],rows:List[Dict[zOWSv,Any]],formatters:Dict[zOWSv,Callable[[Any],zOWSv]]=zOWST):
 from rich.table import Table
 if formatters is zOWST:
  formatters=zOWSs()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  zOWSm(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->zOWST:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->zOWST:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->zOWST:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->zOWST:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
