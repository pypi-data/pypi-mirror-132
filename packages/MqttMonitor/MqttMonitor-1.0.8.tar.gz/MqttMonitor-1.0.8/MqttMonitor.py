...
def addLine(str):
    ...
    global TextInfo
    global gVarCheckAutoscroll
    global gCurrentCharCountInLine
    #print(str)
    ...
    temp_str = str
    if str.find('\n') < 0:
        gCurrentCharCountInLine = gCurrentCharCountInLine + len(str)
        if gCurrentCharCountInLine > 512:
            #print("Move to the next line when the current line is too long")
            temp_str = temp_str + "\n" # Move to the next line when the current line is too long
            gCurrentCharCountInLine = 0
    else:
        gCurrentCharCountInLine = 0
    TextInfo.insert(END, temp_str)
...
def renewLogFileName(logPath):
    #print("renewLogFileName : logPath = " + logPath)
    if len(logPath) > 0:
...
def startSavingLog():
    ...
    #print("startSavingLog : gAppConfig['LogPath'] = " + gAppConfig['LogPath'])
    gLogFileName = renewLogFileName(gAppConfig['LogPath'])
    print("gLogFileName = " + gLogFileName)
