...
#Add SCPI support
from pulsar_qrm.pulsar_qrm_scpi_ifc import pulsar_qrm_scpi_ifc, scpi_error_check
...
class pulsar_qrm_ifc(pulsar_qrm_scpi_ifc):
    ...
    @staticmethod
    #--------------------------------------------------------------------------
    def _get_acq_scope_config_format():
        """
    ...
    #--------------------------------------------------------------------------
    def _set_acq_scope_config(self, cfg_dict):
        """
        Set configuration of the scope acquisition. The configuration consists of multiple parameters in a C struct format.
        ...
        cfg_dict = {**self._get_acq_scope_config(), **cfg_dict}

        #Set new configuration
        cfg = [#Scope acquisition
        ...
        super()._set_acq_scope_config(struct.pack(pulsar_qrm_ifc._get_acq_scope_config_format(), *cfg))

    #--------------------------------------------------------------------------
    def _get_acq_scope_config(self):
        """
        Get configuration of the scope acquisition. The configuration consists of multiple parameters in a C struct format.
        ...
        cfg      = struct.unpack(pulsar_qrm_ifc._get_acq_scope_config_format(), super()._get_acq_scope_config())
        cfg_dict = {#Scope acquisition
                    "sel_acq":                   cfg[0],
    ...
    #--------------------------------------------------------------------------
    def _set_sequencer_config_rotation_matrix(self, sequencer, phase_incr):
        """
        ...
        self._set_sequencer_config(sequencer, {"rotation_matrix_a11": numpy.cos(numpy.deg2rad(360 - phase_incr)),
                                               "rotation_matrix_a12": numpy.sin(numpy.deg2rad(360 - phase_incr))})

    #--------------------------------------------------------------------------
    def _get_sequencer_config_rotation_matrix(self, sequencer):
        """
        ...
        cfg        = self._get_sequencer_config(sequencer)
        phase_incr = numpy.angle(cfg["rotation_matrix_a11"] + cfg["rotation_matrix_a12"] * 1j, deg=True)
        return 0                if phase_incr == 0 else \
               360 - phase_incr if phase_incr >= 0 else \
               -1.0 * phase_incr
    ...
    #--------------------------------------------------------------------------
    def _add_acq_weight(self, sequencer, name, weight, index=None):
        """
        Add new weight to acquisition weight list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if the weight causes the weight memory limit to
        be exceeded or if the weight samples are out-of-range, an error is set in the system error. The weight names 'all' and 'ALL' are reserved and adding weights with those names
        will also result in an error being set in system error. The optional index argument is used to specify an index for the weight in the weight list which is used by the sequencer
        Q1ASM program to refer to the weight. If no index is given, the next available weight index is selected (starting from 0). If an invalid weight index is given, an error is set
        in system error.
        ...
        super()._add_acq_weight(sequencer, name, len(weight), False)
        super()._set_acq_weight_data(sequencer, name, weight)
        if index is not None:
            self._set_acq_weight_index(sequencer, name, index)

    #---------------------------------------------------------------------------
    @scpi_error_check
    def _get_acq_weights(self, sequencer):
        """
        ...
        #SCPI call
        num_weights = struct.unpack('I', self._read_bin('SEQuencer{}:ACQ:WLISt?'.format(sequencer), False))[0]
        if num_weights == 0:
            self._flush_line_end()
    ...
    #--------------------------------------------------------------------------
    def _add_acq_acquisition(self, sequencer, name, num_bins, index=None):
        """
        Add new acquisition to acquisition list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if the required acquisition memory
        cannot be allocated, an error is set in system error. The acquisition names 'all' and 'ALL' are reserved and adding those will also result in an
        error being set in system error. If no index is given, the next available weight index is selected (starting from 0). If an invalid weight index is given, an
        ...
        super()._add_acq_acquisition(sequencer, name, num_bins)
        if index is not None:
            self._set_acq_acquisition_index(sequencer, name, index)

    #---------------------------------------------------------------------------
    def _get_acq_data_and_convert(self, scpi_cmd, flush_line_end):
        """
        ...
        ----------
        scpi_cmd : str
            SCPI command string used to set initial SCPI command.
        flush_line_end : bool
        ...
        #Retrieve scope data
        scope_data = self._read_bin(scpi_cmd, False)
        acquisition_dict["scope"]["path0"]["data"]         = list(struct.unpack('i'*int(len(scope_data)/4), scope_data))
        acquisition_dict["scope"]["path0"]["out-of-range"] = struct.unpack('?', self._read_bin('', False))[0]
        acquisition_dict["scope"]["path0"]["avg_cnt"]      = struct.unpack('I', self._read_bin('', False))[0]
        scope_data = self._read_bin('', False)
        acquisition_dict["scope"]["path1"]["data"]         = list(struct.unpack('i'*int(len(scope_data)/4), scope_data))
        acquisition_dict["scope"]["path1"]["out-of-range"] = struct.unpack('?', self._read_bin('', False))[0]
        acquisition_dict["scope"]["path1"]["avg_cnt"]      = struct.unpack('I', self._read_bin('', False))[0]

        #Normalize scope data
        avg_cnt = acquisition_dict["scope"]["path0"]["avg_cnt"]
        acquisition_dict["scope"]["path0"]["data"] = [val / max_sample_value if avg_cnt == 0 else val / max_sample_value / avg_cnt for val in acquisition_dict["scope"]["path0"]["data"]]
        avg_cnt = acquisition_dict["scope"]["path1"]["avg_cnt"]
        acquisition_dict["scope"]["path1"]["data"] = [val / max_sample_value if avg_cnt == 0 else val / max_sample_value / avg_cnt for val in acquisition_dict["scope"]["path1"]["data"]]
        ...
        for bin_idx in range(0, num_bins):
            ...
            if bool(valid):
                ...
                #Normalize
                bin_path0 = bin_path0 / (max_sample_value**2) if avg_cnt == 0 else bin_path0 / (max_sample_value**2) / avg_cnt
                bin_path1 = bin_path1 / (max_sample_value**2) if avg_cnt == 0 else bin_path1 / (max_sample_value**2) / avg_cnt
                thres     = float(thres)                      if avg_cnt == 0 else thres / avg_cnt
            else:
                bin_path0 = float("NaN")
                bin_path1 = float("NaN")
                thres     = float("NaN")
    ...
    #---------------------------------------------------------------------------
    @scpi_error_check
    def _get_acq_acquisition_data(self, sequencer, name):
        """
        Get acquisition data of acquisition in acquisition list of indexed sequencer's acquisition path. The acquisition scope and bin data is normalized to a
        range of -1.0 to 1.0 taking both the bit widths of the processing path and average count into considaration. For the binned integration results, the integration
        length is not handled during normalization and therefore these values have to be divided by their respective integration lenghts. If an invalid sequencer
        index is given or if a non-existing acquisition name is given, an error is set in system error.
        ...
        #SCPI call
        return self._get_acq_data_and_convert('SEQuencer{}:ACQ:ALISt:ACQuisition:DATA? "{}"'.format(sequencer, name), True)
    ...
    def _get_acq_acquisitions(self, sequencer):
        ...
        #SCPI call
        num_acq = struct.unpack('I', self._read_bin('SEQuencer{}:ACQ:ALISt?'.format(sequencer), False))[0]
        if num_acq == 0:
            self._flush_line_end()
        ...
        for acq_it in range(0, num_acq):
            ...
            #Get data
            if acq_it < (num_acq - 1):
                acq = self._get_acq_data_and_convert('', False)
            else:
                acq = self._get_acq_data_and_convert('', True)

            #Add to dictionary
            acquisition_dict[name] = {"index":        index,
                                      "acquisition": acq}
    ...
    def _add_waveforms(self, sequencer, waveform_dict):
        """
        Add all waveforms in JSON compatible dictionary to the AWG waveform list of indexed
        sequencer. The dictionary must be structured as follows:
    ...
    def get_waveforms(self, sequencer):
        """
        Get all waveforms and weigths in the AWG waveform list of indexed sequencer.
...
#--------------------------------------------------------------------------
    def _add_weights(self, sequencer, weight_dict):
        """
        Add all weights in JSON compatible dictionary to the aquisition weight list of indexed
        sequencer. The dictionary must be structured as follows:
        ...
        for name in weight_dict:
            if "data" in weight_dict[name]:
                if "index" in weight_dict[name]:
                    self._add_acq_weight(sequencer, name, weight_dict[name]["data"], weight_dict[name]["index"])
                else:
                    self._add_acq_weight(sequencer, name, weight_dict[name]["data"])
            else:
                raise KeyError("Missing data key for {} in acquisiton weight dictionary".format(name))

    #--------------------------------------------------------------------------
    def _delete_weights(self, sequencer):
        """
        Delete all weights in the acquisition weights lists of indexed sequencer.
        ...
        self._delete_acq_weight(sequencer, "all")

    #--------------------------------------------------------------------------
    def get_weights(self, sequencer):
        """
        ...
        return self._get_acq_weights(sequencer)
    ...
    def get_acquisition_state(self, sequencer, timeout=0, timeout_poll_res=0.1):
        """
        Return acquisition binning completion state of the indexed sequencer. If an invalid sequencer is given, an error is
        ...
        the function blocks until the acquisition binning completes. If the acquisition hasn't completed before the timeout expires, a
        ...
        status       = "ACQ BINNING DONE" in state["flags"]
        elapsed_time = 0.0
        timeout      = timeout * 60.0
        while status != True and elapsed_time < timeout:
            time.sleep(timeout_poll_res)
    ...
    #--------------------------------------------------------------------------
    def _add_acquisitions(self, sequencer, acquisition_dict):
        """
        Add all waveforms and weights in JSON compatible dictionary to AWG waveform and aquisition weight lists of indexed
        sequencer. The dictionary must be structured as follows:
        ...
        for name in acquisition_dict:
            if "num_bins" in acquisition_dict[name]:
                if "index" in acquisition_dict[name]:
                    self._add_acq_acquisition(sequencer, name, acquisition_dict[name]["num_bins"], acquisition_dict[name]["index"])
                else:
                    self._add_acq_acquisition(sequencer, name, acquisition_dict[name]["num_bins"])
            else:
                raise KeyError("Missing num_bins key for {} in acquisition dictionary".format(name))
    ...
    def _delete_acquisitions(self, sequencer):
        ...
        self._delete_acq_acquisition(sequencer, "all")

    #---------------------------------------------------------------------------
    def store_scope_acquisition(self, sequencer, name):
        """
        After an acquisition has completed, store the scope acquisition results in the acquisition specified by name of the indexed sequencers. If an
        invalid sequencer index is given an error is set in system error. To get access to the acquisition results, the sequencer will be stopped when calling
        this function.
        ...
        self._set_acq_acquisition_data(sequencer, name)

    #--------------------------------------------------------------------------
    def get_acquisitions(self, sequencer):
        """
        Get all acquisitions in acquisition lists of indexed sequencer. The acquisition scope and bin data is normalized to a range of -1.0 to 1.0 taking
        both the bit widths of the processing path and average count into considaration. For the binned integration results, the integration length is
        not handled during normalization and therefore these values have to be divided by their respective integration lenghts. The returned dictionary
        ...
        return self._get_acq_acquisitions(sequencer)
