...
class FirstSeat():
    ...
    def _best_suit(self, score_reasons: Dict[str, int]) -> Suit:
        ...
        candidate_suits = player.get_list_of_best_scores(suit_scores)
        if candidate_suits:
            pass
        else:
    ...
    def _select_best_suit(self, player: Player) -> Suit:
        ...
        long_suits = player.get_list_of_best_scores(long_suit_candidates)
        if len(long_suits) == 1:
    ...
    def _play_all_winners(self) -> Card:
        ...
        if player.trump_suit:
            ...
            if player.unplayed_cards[suit_name] and player.opponents_unplayed_cards[suit_name]:
    ...
    def _select_card_from_right_hand(self, suit_name: str) -> Card:
        ...
        if partners_cards:
            if (len(my_cards) < len(partners_cards) and my_cards[-1].value < partners_cards[0].value):
