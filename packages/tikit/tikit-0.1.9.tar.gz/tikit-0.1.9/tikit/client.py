...
class Client(object):
    _tione_endpoint = "tione.tencentcloudapi.com"
    # 临时秘钥使用，提前获取临时秘钥，避免访问的时候失败
    _secret_ahead_time = 60 * 5

    def __init__(self, secret_id=None, secret_key=None, region=None, token=None, proxies=None):
        """初始化，保存用户的信息
    ...
    def _init_client(self):
        ...
        client_profile = ClientProfile(httpProfile=HttpProfile(endpoint=self._tione_endpoint, proxy=proxy))
    ...
    def create_training_job(self):
        try:
            req = models.CreateTrainingJobRequest()
            params = {
                "TrainingJobName": "xxxxx"
            }
            req.from_json_string(json.dumps(params))
            models.TrainingJobName = "xxx"

            resp = self._tione_client.CreateTrainingJob(req)
    ...
    def create_training_task(self,
        ...
        :type input_data_config:    :class:`tikit.models.TrainingDataConfig`
        ...
        try:
            ...
            req.Tags = tags
            req.CodePackagePath = self.parse_cos_info(code_package_path)
            req.StartCmdInfo = models.StartCmdInfo()
            req.StartCmdInfo.WorkerStartCmd = worker_start_cmd
            req.StartCmdInfo.PStartCmd = ps_start_cmd

            req.DataSource = input_data_config.DataSource
            req.DataConfigs = []
            if req.DataSource == "DATASET":
                for dataset_id in input_data_config.DataConfigDict:
                    ...
                    data_config.Id = dataset_id
                    req.DataConfigs.append(data_config)
            if req.DataSource == "COS":
                for cos_str in input_data_config.DataConfigDict:
                    data_config = models.DataConfig()
                    data_config.MappingPath = input_data_config.DataConfigDict[cos_str]
                    data_config.StorageDataPath = self.parse_cos_info(cos_str)
                    req.DataConfigs.append(data_config)
    ...
    def describe_training_frameworks(self):
        ...
        |   LIGHT    |  3.0-py3.6-cuda11.0-gpu |    DDP, MPI, HOROVOD    |
        +------------+-------------------------+-------------------------+
        """
        try:
    ...
    def describe_train_logs(self, pod_name, start_time=None, end_time=None, limit=None, order=None,
        ...
        def get_next_data(xx):
            if result.Context != "":
                return self.describe_train_logs(pod_name, start_time, end_time, limit, order, result.Context, filters)
            else:
    ...
    def create_training_model(self, model):
        ...
        :param model: Request instance for CreateTrainingModel.
        :type model: :class:`tencentcloud.tione.v20211111.models.CreateTrainingModelRequest`
        :rtype: :class:`tencentcloud.tione.v20211111.models.CreateTrainingModelResponse`
        """
        try:
            return self._tione_client.CreateTrainingModel(model)
        except TencentCloudSDKException as err:
            raise
