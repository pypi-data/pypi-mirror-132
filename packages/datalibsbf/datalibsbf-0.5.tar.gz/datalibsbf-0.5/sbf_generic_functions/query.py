...
def query_sql_server ( vhost, vport, vdatabase, vuid, vpwd, vquery ):
    ...
    conn_string = 'Driver={SQL Server};Server=%s,%s;Database=%s;UID=%s;PWD=%s' % conn_params
...
def query_redshift ( vhost, vport, vdatabase, vuid, vpwd, vquery ):
    ...
    conn = rc.connect( host=vhost, port=vport, database=vdatabase, user=vuid, password=vpwd, )
...
def query_athena( vquery, vdatabase, vworkgroup, vregion, vaccess_aws, vsecret_aws):
    ...
    id = wr.athena.start_query_execution(vquery, database=vdatabase, workgroup=vworkgroup,boto3_session=session)
...
def query_oracle_old ( vhost, vport, vservice_name, vlib_dir, vuid, vpwd, vquery ):
    ...
    dsn_tns = cx_Oracle.makedsn(vhost, vport, service_name=vservice_name) # caso precise, coloque um 'r' antes de qualquer parametro se tiver caracter especial
    connection = cx_Oracle.connect(user=vuid, password=vpwd, dsn=dsn_tns) # cria a conexão uid+"/"+pwd+"@"+db
...
def create_external_table_part_time_bigquery (vtable_id, vuri, vwritedisposition, vfield, vexpiration_ms):
...
def create_external_table_bigquery (vtable_id, vuri, vwritedisposition, vproject, vpartition='', vclustered='', vsource_format='', vtype='', vexpiration_ms=''):
    ## Criando tabela no bigquery
    # Construct a BigQuery client object.
...
def get_schema_dev_recreate_prod_bq(**kwargs):
    ...
    kwargs['schema'] = get_schema_bq("dev-data-platform-291914.Raw.{}".format(ctable_name)).__str__()
...
SchemaField('payload', 'RECORD', 'NULLABLE', None, (
    ...
    print(query_athena( 'ALTER TABLE centauro_spectrum_db.cnt_kfk_6000_all_simple_stock_policy ADD PARTITION (`_partitiondate` = "2021-10-18") LOCATION "s3://4insights-centauro-datafiles/DATA_FILE/cnt_kfk_6000_all_simple_stock_policy/_PARTITIONDATE=2021-10-18/"'
