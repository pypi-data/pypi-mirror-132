...
from e2j2.constants import CONFIG_SCHEMAS, DESCRIPTION, VERSION
from e2j2.display import get_colors, no_colors, write
...
def arg_parse(program, description, version):
    ...
    arg_parser.add_argument(
        "--stderr", action="store_true", help="Print to stderr instead of to error file"
    )
    ...
    if args.recursive and not args.searchlist and "E2J2_SEARCHLIST" not in os.environ:
...
def configure(args):
    ...
    config["stderr"] = args.stderr if args.stderr else config.get("stderr", False)
    config["initial_run"] = (
