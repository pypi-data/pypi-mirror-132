...
from openapi_client import CliWorkspaceBackupCreateAPIPayload
...
def backup_workspace() -> None:
    ...
    filename = f'workspace-backup-{datetime.now().strftime("%Y-%m-%d")}-{random_string()}.zip'
    full_path = os.path.join(TEMP_DIR, filename)
    zipper = Zipper(full_path, "w")
    zipper.zipdir(str(Path.home()), zipper, include_dotfiles=True)
    zipper.close()
    ...
    vessl_api.cli_workspace_backup_create_api(
        ...
        cli_workspace_backup_create_api_payload=CliWorkspaceBackupCreateAPIPayload(
            filename=filename,
        ),
    )
...
def restore_workspace() -> None:
    ...
    copy_volume_file(
        ...
        dest_volume_id=None,
        dest_path=f'{TEMP_DIR}/workspace-backup.zip',
    )

    z = ZipFile(f'{TEMP_DIR}/workspace-backup.zip')
