...
from vessl.cli.experiment import (
    cli as experiment_cli,
    create as experiment_create,
...
# Configure Sentry
sentry_sdk.init(
    os.environ.get("SENTRY_DSN", "https://e46fcd750b3a443fbd5b9dbc970e4ecf@o386227.ingest.sentry.io/5911639"),
...
@configure.vessl_command(hidden=True)
@click.pass_context
@click.argument("extras", nargs=-1)
def run(ctx, extras: List[str]):
    args = []
    args.extend(["--command", ' '.join(extras)])
