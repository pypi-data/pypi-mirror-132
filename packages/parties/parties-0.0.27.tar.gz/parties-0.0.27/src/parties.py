    ...
    def run_parallel(self,out_func=None,stop_func=None):
        ...
        def update_simulation_status():
            ...
            for item in self.sim_list:
                if item['status'] == 'Running' and item['simulation'].parties.poll() != None:
                    if item['simulation'].parties.poll() == 0:
                        item['status'] = 'Done'
                    if item['simulation'].parties.poll() == 1:
                        item['status'] = 'Crashed'
                    res += item['n_pps']
            return res

        def print_sim_table_terminal():
            for item in self.sim_list:
                if item['status'] != item['prev_status']:
                    print('')
                    print(item['postfix'])
                    print(item['prev_status'] + '->' + item['status'])
            #sys.stdout.flush()
        ...
        #and set them into separate folders
        for idx, rows in self.vars.iterrows():
            ...
            #Change parties.inp, p_mobile.inp and p_fixed.inp
            s.change_parties_inp(cols,rows)
            if self.p_fixed != None:
                s.change_file(self.p_fixed[idx],'p_fixed.inp')
            if self.p_mobile != None:
                s.change_file(self.p_mobile[idx],'p_mobile.inp')
            
            #Create simulation folder and copy inputs and executable there
