...
class SamlProvider(aws_iam.SamlProvider):
    ...
    def __init__(
        ...
        core.CfnOutput(self.stack, f"{self.node.id}-arn", value=self.saml_provider_arn)
        if self.ssm_parameter_name:
            ...
            core.CfnOutput(self.stack, f"{self.node.id}-ssm-name", value=self.ssm_parameter_name)
