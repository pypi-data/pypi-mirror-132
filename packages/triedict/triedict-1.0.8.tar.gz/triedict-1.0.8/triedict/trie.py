...
class TrieDict:
    def __init__(self, stop_chars: Union[str, List, Tuple, Set] = None, case_ignored=False):
        """vega FYI
            self.stop_chars          忽略掉的字符
            self.case_ignored:       是否忽略英文字母大小写
            self.trie:               关键字典树
            self.item_tuple_flag:    传入的text是否携带其他信息，如果
        """
    ...
    def init_or_insert_dict(self, items: Union[List, Tuple, Set], text_preprocess_flag: bool = True) -> None:
    ...
    def get_match_words(self, text: str, return_offset: bool = True) -> List[Dict]:
        """vega FYI 从text的idx 0往后匹配,如果没有匹配到，
            接着从1开始匹配，如果匹配到了，
            假设长度为3，继续从索引3开始匹配
        """
        # TODO 返回匹配到的索引  + 部分有term_info，部分没有term_info
        match_words_list = []
        text = text.lower() if self.case_ignored else text
        text_len = len(text)
        start_offset = 0
        while start_offset <= text_len - 1:
            ...
            if temp_word:
                ...
                if self.item_tuple_flag:
                    ...
                    item_match_info = {"id": len(match_words_list) + 1,
                                       "term": temp_word[0],
