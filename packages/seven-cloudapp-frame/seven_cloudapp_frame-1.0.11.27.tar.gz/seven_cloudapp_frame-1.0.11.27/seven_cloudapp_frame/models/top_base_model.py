...
@LastEditTime: 2021-12-21 15:20:21
...
class TopBaseModel():
    ...
    """
    def __init__(self,context,logging_error=None,logging_info=None):
        ...
        try:
            ...
            if is_log:
                if self.context:
                    ...
                    self.logging_link_info(str(resp) + "【access_token】：" + access_token + "【获取订购过期时间】")
    ...
    def get_member_info(self, access_token,app_key, app_secret, is_log=False):
        """
    ...
    def check_is_member(self, access_token,app_key, app_secret,is_log=False):
        """
        ...
        """
        is_member = False
        invoke_result_data = self.get_member_info(access_token,app_key, app_secret,is_log)
        if invoke_result_data.success == False:
            return is_member
    ...
    def get_project_code(self, user_nick, access_token,app_key, app_secret,is_log=False):
        ...
        try:
            ...
            if is_log:
                if self.context:
                    self.context.logging_link_info(str(resp) + "【access_token】：" + access_token + "【获取项目编码】")
                elif self.logging_link_info:
                    self.logging_link_info(str(resp) + "【access_token】：" + access_token + "【获取项目编码】")
            if "article_user_subscribe" not in resp["vas_subscribe_get_response"]["article_user_subscribes"].keys():
                return "expire"
