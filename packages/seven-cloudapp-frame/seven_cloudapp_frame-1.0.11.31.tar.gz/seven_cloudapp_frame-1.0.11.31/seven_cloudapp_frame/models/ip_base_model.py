...
@LastEditTime: 2021-12-17 10:52:01
...
class IpBaseModel():
    ...
    def _delete_ip_info_dependency_key(self,act_id):
        """
    ...
    def get_ip_info_list(self,app_id,act_id,page_size,page_index,order_by="sort_index desc",is_cache=True):
        """
        :description: 获取IP列表
        :param app_id：应用标识
        :param act_id：活动标识
        ...
        if is_cache == True:
            page_list, total = IpInfoModel(context=self.context).get_cache_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params,dependency_key=f"ip_info_list:actid_{act_id}",cache_expire=600)
        else:
            page_list, total = IpInfoModel(context=self.context).get_dict_page_list(field="*", page_index=page_index, page_size=page_size, where=condition, group_by="", order_by=order_by,params=params)
        page_info = PageInfo(page_index, page_size, total, page_list)
        return page_info

    def delete_ip_info(self,app_id,act_id,ip_id):
        """
        :description: 删除ip
        :param app_id：应用标识
        :param act_id：活动标识
        ...
        if not ip_info_dict or ip_info_dict["act_id"]!=act_id or ip_info_dict["app_id"]!=app_id:
            invoke_result_data.success = False
            invoke_result_data.error_code = "error"
            invoke_result_data.error_message = "ip信息不存在"
            return invoke_result_data
    ...
    def release_ip_info(self,app_id,act_id,ip_id,is_release):
        """
        :description: ip上下架
        :param app_id：应用标识
        :param act_id：活动标识
