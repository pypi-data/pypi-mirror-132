...
@LastEditTime: 2021-12-30 12:08:03
...
class HorseracelampListHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        condition = f"create_date>'{TimeHelper.add_hours_by_format_time(hour=-1)}'"
        params = []
        invoke_result_data = self.business_process_executing()
        if invoke_result_data.success == False:
            return self.response_json_success({"data": []})
        else:
            if invoke_result_data.data:
                condition = invoke_result_data.data["condition"] if invoke_result_data.data.__contains__("condition") else condition
                params = invoke_result_data.data["params"] if invoke_result_data.data.__contains__("params") else params
        order_base_model = OrderBaseModel(context=self)
        horseracelamp_list = self.business_process_executed(order_base_model.get_horseracelamp_list(act_id, module_id, is_search_module, condition, params, page_size), ref_params={})
...
class PrizeOrderListHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        is_search_roster = int(self.get_param("is_search_roster", 0))
        ...
        order_base_model = OrderBaseModel(context=self)
        page_info = order_base_model.get_prize_order_list(app_id, act_id, user_id, user_open_id, "", "", "", "", "", order_status, create_date_start, create_date_end, page_size, page_index, order_by, is_search_roster=is_search_roster, is_cache=True)
        ref_params = {}
...
class SelectPrizeOrderHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if invoke_result_data.success == False:
            ...
            if invoke_result_data.data:
                if invoke_result_data.data.__contains__("prize_ids"):
                    prize_ids = invoke_result_data.data["prize_ids"]
