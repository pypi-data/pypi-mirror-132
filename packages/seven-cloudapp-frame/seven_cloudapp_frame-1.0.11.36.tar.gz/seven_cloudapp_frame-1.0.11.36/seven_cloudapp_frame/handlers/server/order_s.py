...
@LastEditTime: 2021-12-24 09:36:00
...
class PayOrderListHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if invoke_result_data.success == False:
            ...
            order_by = invoke_result_data.data["order_by"] if invoke_result_data.data.__contains__("order_by") else "pay_date desc"
        order_base_model = OrderBaseModel(context=self)
        page_info = order_base_model.get_tao_pay_order_list(app_id, act_id, user_id, user_open_id, user_nick, pay_date_start, pay_date_end, page_size=page_size, page_index=page_index, order_by=order_by, condition=condition, params=params)
        ref_params = {}
...
class PrizeOrderExportHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if page_size <= 500:
            prize_order_list_dict = order_base_model.get_prize_order_list(app_id, act_id, user_id, user_open_id, user_nick, order_no, real_name, telephone, address, order_status, create_date_start, create_date_end, page_size, page_index, order_by, field=field, is_search_roster=True, is_cache=False, condition=condition, params=params).data
        else:
            max_page_size = 100
            repeat_count = page_size // max_page_size  # 循环次数
            begin_page_index = page_index * repeat_count  # 开始页码
            for i in range(repeat_count):
                list_dict = order_base_model.get_prize_order_list(app_id, act_id, user_id, user_open_id, user_nick, order_no, real_name, telephone, address, order_status, create_date_start, create_date_end, max_page_size, begin_page_index + i, order_by, field=field, is_search_roster=True, is_cache=False, condition=condition, params=params).data
                if len(list_dict) <= 0:
                    break
