...
@LastEditTime: 2021-11-18 14:30:13
...
class FrameBaseModel():
    ...
    def lottery_algorithm_probability(self, prize_list):
        """
        :description: 抽奖算法（权重）
        :param prize_list:奖品列表
        :return: 中奖的奖品
    ...
    def business_process_executing(self,app_id,act_id,module_id,user_id,login_token,handler_name,check_new_user=False,check_user_nick=True,continue_request_expire=5,acquire_lock_name="",request_limit_num=0,request_limit_time=1):
        """
        ...
        :param request_limit_num:请求限制数；0不限制
        ...
        if continue_request_expire > 0:
            if SevenHelper.is_continue_request(f"request_business_executing:{handler_name}_{act_id}_{module_id}_{user_id}",expire=continue_request_expire*1000) == True:
                invoke_result_data.success = False
        ...
        user_info_dict = user_info_model.get_cache_dict("id_md5=%s",params=[id_md5],dependency_key=f"user_info:actid_{act_id}_idmd5_{id_md5}")
        ...
        if check_user_nick == True:
            if not user_info_dict["user_nick"]:
                invoke_result_data.success = False
