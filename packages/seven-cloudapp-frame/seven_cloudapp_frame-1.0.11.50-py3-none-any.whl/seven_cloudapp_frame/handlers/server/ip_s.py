...
@LastEditTime: 2021-12-30 11:00:16
    ...
    def get_async(self):
        ...
        if invoke_result_data.success == False:
            ...
            if invoke_result_data.data:
                field = invoke_result_data.data.get("field","*")
                condition = invoke_result_data.data.get("condition","")
                params = invoke_result_data.data.get("params",[])
                order_by = invoke_result_data.data.get("order_by","sort_index desc")
        return self.response_json_success(IpBaseModel(context=self).get_ip_info_list(app_id, act_id, page_size, page_index, field, condition, order_by, is_cache=False))
...
class DeleteIpInfoHandler(ClientBaseHandler):
    ...
    :description: 删除ip
    """
    @filter_check_params("act_id,ip_id")
    def get_async(self):
        """
        :description: 删除ip
        :param app_id：应用标识
