                ...
                if parameter_name == 'self':
                    ...
                    if parameter_type is inspect.Parameter.empty:
                        if parameter.default is inspect.Parameter.empty:
                            ...
                            parameter_type = type(parameter.default)
                    # new_value = parse_val(argument_value, parameter_type)
                    # params_list.append(new_value)
                    # # 获取实参类型
                    ...
                    # print(f"parameter_name:{parameter_name},parameter_type:{parameter_type},argument_type:{argument_type}")
                    # new_value = parse_val(argument_value,)
                    # elif isinstance(argument_value, list):  # 如果参数是集合类型要进行遍历并进行转码
                    #     # 仅一个 直接存入
                    #     if len(argument_value) == 1:
                    #         params_list.append(argument_value[0].decode())
                    #     else:  # 多个要使用集合存入
                    #         params_list.append([v.decode() for v in argument_value])
                    # else:
                    #     # 其他类型的直接存入
                    #     params_list.append(argument_value)
