...
# sql_path = os.path.join(project_path, "sqls")
# convert_path = os.path.join(project_path, "converters")
...
class WebServer:
    ...
    def __init__(self):
        ...
        # 5、自动化注册Handler处理类，
        self.autoRegisterHandler(f"{os.path.abspath(os.path.dirname(sys.argv[0]))}/{options.handler_path}", options.handler_path)
