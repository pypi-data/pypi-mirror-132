...
class ElasticsearchHelper(BaseSqlHelper):
    ...
    async def execute_fetch(self, param: str, sql: list = [], column: list = None, offset=0,
                            size=100, sort: list = [], aes_enable=True, aes_key=None):
        """
        查询多条数据
        :param sort: 排序
        :param aes_key: aes密钥
    ...
    def prepare_condition_sql(self, conditions: list):
        ...
        if len(conditions) == 0:
            return {}
        filter_item = []
        current_filter_items = {"bool": {"must": []}}
        for condition in conditions:
            # 获取当前过滤条件
            operator, column, value = condition
            if operator == "AND" or operator == "OR":
                if operator == "AND":
                    # filter_item.append({"bool": {"must": []}})
                    current_filter_items = {"bool": {"must": []}}
                else:
                    # filter_item.append({"bool": {"should": []}})
                    current_filter_items = {"bool": {"should": []}}
                continue
            if operator == "=":
                ...
                continue
            if operator == "AND":
                current_filter_items["bool"]["must"].append(condition_item)
            else:
                current_filter_items["bool"]["should"].append(condition_item)
        filter_item.append(current_filter_items)
