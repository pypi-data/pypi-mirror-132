...
class gravityfield:
    """ Calculate the acceleration field in 2-dimensional space in a rotating reference frame with no relative velocity.
        ...
        SUBTRACT from the corresponding value (the position the object is) in gravityfield.g  the coriolis acceleration (in complex form) !!
        ...
        gravityfield.z - 2-dimensional array of the positions used in gravityfield.g.

        gravityfield.g - the calculated acceleration field in a 2-dimensional array of complex numbers.
        The real value is the acceleration in the x axis, the imaginary component is the acceleration in the y axis.

        gravityfield.contour() - using plotly, draw the contour of the absolute value of the acceleration field.
        
        gravityfield.surface() - using plotly, draw the surface plot of the absolute value of the acceleration field. (WIP)"""
...
def main():
    # Earth-Moon example:
    contour_title = 'Absolute value of the acceleration near the Earth-Moon system'
    angular_vel = 2*np.pi/(28*24*3600)
    bodies = [dict(mu = 3.986004418e14, position = 0, radius = 6.371e6),
              dict(mu = 4.9048695e12, position = 384.4e6, radius = 1.7374e6)]

    earth_moon = gravityfield(bodies, angular_vel)
