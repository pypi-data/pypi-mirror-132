...
def convert_to_repo_diff_summary(
    fco_diffs: List[state_update_pb2.FcoDiff], plan_id: Id, debug: bool
) -> repo_diff_pb2.TectonRepoDiffSummary:
    ...
    if not debug:
        repo_diff.plan_id = IdHelper.to_string(plan_id)
...
def print_plan_server(plan_items: Iterable[state_update_pb2.FcoDiff], plan_id: Id, debug: bool):
    if not plan_items:
    ...
    if not debug:
        printer.safe_print(Style.NORMAL, "Generated plan id is %s" % IdHelper.to_string(plan_id), Style.RESET_ALL)
...
def update_tecton_state(
    ...
    print_plan_server(response_query.diff_items, response_submit.state_id, debug)
    ...
    if json_out_path:
        repo_diff_summary = convert_to_repo_diff_summary(response_query.diff_items, response_submit.state_id, debug)
