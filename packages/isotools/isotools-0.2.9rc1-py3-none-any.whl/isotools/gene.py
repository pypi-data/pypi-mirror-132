    ...
    def _to_gtf(self, trids, source='isoseq'):
        ...
        for i in trids:
            ...
            if 'downstream_A_content' in tr:
                trinfo['downstream_A_content']=tr['downstream_A_content']
                ...
                for refid in tr['annotation'][1]['FSM']:
                    for k in self.ref_transcripts[refid]:
                        if k == 'exons':
                            ...
                            trinfo['CDS_start']=cds_start
                            trinfo['CDS_end']=cds_end
                        else:
                            trinfo[k]=str(self.ref_transcripts[refid][k])
                    
                    trinfo['ref_transcript_id']=','.join(tr['reference'][refid] )
            else:
        ...
        if len(lines) > 1:
            ...
            if 'reference' in self.data:
                info.update({k:v for k,v in self.data['reference'] if k != 'transcripts'}) # add reference gene specific fields
