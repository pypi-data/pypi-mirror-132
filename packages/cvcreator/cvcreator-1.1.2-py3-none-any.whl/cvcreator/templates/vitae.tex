...
\noindent{\LARGE \bfseries \color{xaldark} Curriculum Vitae for \VAR{name}}
...
\BLOCK{if s.volume}   Volume      & \VAR{s.volume}      \\ \BLOCK{endif}
                      Description & \VAR{s.description} \\
\BLOCK{if s.tools}    Tools       & \VAR{s.tools}       \\ \BLOCK{endif}
\BLOCK{if s.url}      URL         & \VAR{s.url}         \\ \BLOCK{endif}
  \rule{\textwidth}{0.1pt}
  \end{tabular}
  \BLOCK{endfor}
  \vspace{0.5cm}
...
\BLOCK{if s.year}Year    & \VAR{s.year} \\\BLOCK{endif}
\BLOCK{if s.summary}Summary & \VAR{s.summary} \\\BLOCK{endif}
  \rule{\textwidth}{0.1pt}
  \end{tabular}
  \BLOCK{endfor}
  \vspace{0.5cm}
