...
def get_project_name():
    return toml.load("pyproject.toml")["name"]


def get_project_version():
    return toml.load("pyproject.toml")["version"]
...
@app.command()
def docker():
    local["cp"]["Dockerfile", "dist/Dockerfile"] & FG
    local.cwd.chdir("dist")
    (
        local["docker"][
            "build",
            "--no-cache",
...
def podman_push():
    registry_name = os.environ["CI_REGISTRY_IMAGE"]
    local["podman"]["push", f"{registry_name}/{get_project_name()}"] & FG
    local["podman"]["push", f"{registry_name}/{get_name_version()}"] & FG
