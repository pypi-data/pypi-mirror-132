...
class adbClass():
    ...
    def check_port(self, timeout=5):
        ...
        for t in range(timeout):
            ...
            tid_list, adb_paths, adb_info = get_adb_devices()
    ...
    def myiperf(self, ip, port=6789):
        ...
        if (ready):
            ...
            for i in range(3):
                self.sendcmd_recv1("myIperf '-s -p %s'" % (port))  # 集成版本iperf2=myIperf
                out = self.sendshell("ps -e|grep myIperf")
                print(out)
    ...
    def checkapk(self, name="com.utsmta.app"):
        out = self.sendshell("ps -e|grep " + name)
    ...
    def sendcmd_recv2_data(self, cmd, *para):
        ...
        try:
            _cmd = "dumpsys activity com.utsmta.app/com.nes.factorytest.ui.activity.Main2Activity "
            ret = self.dev.shell("%s%s %s" % (_cmd, cmd, " ".join(para)))
            # if("initRequestKey" in cmd):
            # print("%s%s %s" % (_cmd, cmd, " ".join(para)))
    ...
    def sendshell(self, cmdline):
        if (self.has_error()): return ""
        try:
            return self.dev.shell(cmdline)
        except Exception as e:
    ...
    def agingTest(self, value):
        # value单位为分钟 value暂不用（以产测配置为准）
    ...
    def set_pms_keys(self, _token, _json, file_name="require_keys.txt"):
        if ("server_url_key_batch" in _json):
            ...
            if ("key_batch" in response_dict):
                ...
                self.sendshell('echo "%s" > /sdcard/%s' % (replace_res, file_name))  # 传入文件的方式传递
                # print(len(self.sendshell('cat /sdcard/%s' % ( file_name)))  # 传入文件的方式传递
                return True, pname
            else:
    ...
    def keys_burn(self, sn, emac="null", wmac="null", file_name="require_keys.txt"):
        ...
        if (isnew):
            ...
            self.sendshell('echo "%s" > /sdcard/config_keys.txt' % config_json)  # 传入文件的方式传递
    ...
    def burn_keys(self, conf_keys):
        ...
        if (result):
            msg = "%s" % (','.join([i for i in conf_keys.keys()]))
