...
def fpgrowth(df, min_support=0.5, return_df=False, max_length=5):
    ...
    for unique in columns:
        ...
        selected_unique = selection_support_unique(
            compressed_unique, unique_keys, min_support
        )
        combinations = combinations_unique(selected_unique, unique, max_length)

        selected_combinations, selected_supports = selection_support_df(
            df_copy, combinations, min_support
        )
