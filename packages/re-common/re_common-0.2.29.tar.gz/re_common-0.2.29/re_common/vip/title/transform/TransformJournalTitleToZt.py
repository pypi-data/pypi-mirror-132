...
class TransformJournalTitleToZt():
    ...
    def get_zt_provider(self):
        rows = self.mysqlutils.SelectFromDB(
            "select sub_db_id,provider from A2Task where source_type = '3' and out_type = 'zt'")
        for row in rows[1]:
    ...
    def transform(self, titleMap):
        ...
        if transMap["provider"] == "":
            sub = titleMap.get("sub_db_id", "")
            transMap["provider"] = self.zt_providermap[sub]
