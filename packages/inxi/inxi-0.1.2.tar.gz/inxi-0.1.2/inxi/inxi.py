...
import psutil
import typer
from cpuinfo import CPUINFO_VERSION, get_cpu_info
...
def cpufreq(verbose: bool = False):
    if verbose:
        logging.info("Getting cpu frequencies...")
    freqs = psutil.cpu_freq()
    return f"{int(freqs.current)}/{int(freqs.min)}/{int(freqs.max)} Mhz"
...
def main(verbose: Optional[bool] = False):
    console.print(
        ...
        f"[steel_blue bold]speed/min/max:[/steel_blue bold] {cpufreq(verbose)} "
