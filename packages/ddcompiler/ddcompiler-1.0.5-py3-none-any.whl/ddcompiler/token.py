...
class Token:
    def __init__(self, ttext, ttype, start_pos=None, end_pos=None):
        # token text for token types VARIABLE, STRING and NUMBER is
        # necessary; for other token types it is kind-of redundant
        self.ttext = ttext
        self.ttype = ttype
        # Gives start and end position of token in expression if it is a variables
        # For expression "de" == ${conv.lang}
