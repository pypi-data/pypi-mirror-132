...
def main():
    ...
    # create datasets
    data = glogp.data.sample_2d(dataset=a.data, n_samples=a.size).astype(np.float32)
    train_dataset = tf.data.Dataset.from_tensor_slices(data).shuffle(a.size)
    eval_data = glogp.data.sample_2d(dataset=a.data, n_samples=a.eval_size).astype(np.float32)
...
def plot_score_field(ax, trainer):
    mesh, scores = glogp.utils.sample_score_field(trainer.model.score)
    # draw scores
    ax.grid(False)
    ax.axis('off')
...
def plot_energy_field(ax, trainer):
    energy = glogp.utils.sample_energy_field(trainer.model)
    # draw energy
    ax.grid(False)
    ax.axis('off')
...
def plot_samples(ax, trainer, steps, eps):
    samples = []
    for i in range(1000):
        x = tf.random.uniform((1000, 2)) * 8 - 4
