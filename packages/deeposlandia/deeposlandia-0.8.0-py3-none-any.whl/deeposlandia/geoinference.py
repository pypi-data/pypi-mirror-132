...
def extract_images(image_paths):
    ...
    for image_path in image_paths:
        ...
        if image.size[0] != image.size[1]:
            logger.error("One of the parsed images has non-squared dimensions.")
            raise ValueError()
...
def get_labels(datapath, dataset, tile_size):
    ...
    return [label for label in test_config["labels"] if label["is_evaluate"]]
...
def fill_labelled_image(
    ...
    predicted_image = np.zeros([extended_height, extended_width], dtype=np.uint8)
...
def get_image_features(datapath, dataset, filename):
    ...
    and size (in pixels), geotransform and projection (under WKT format)

    """
    input_folder = utils.prepare_input_folder(datapath, dataset)
    filepath = os.path.join(input_folder, "testing", "images", filename + ".tif")
    img_dataset = gdal.Open(filepath)
    width = img_dataset.RasterXSize
    height = img_dataset.RasterYSize
    geotransform = img_dataset.GetGeoTransform()
    wkt_projection = img_dataset.GetProjection()
    minx = geotransform[0]
    miny = geotransform[3] + height * geotransform[5]
    maxx = geotransform[0] + width * geotransform[1]
    maxy = geotransform[3]
    srid = int(img_dataset.GetProjection().split('"')[-2])
    img_dataset = None  # Free memory used by the GDAL Dataset
    return {
        "west": minx,
        "south": miny,
        "east": maxx,
...
def store_raster(output_data, output_filename, input_dataset_features):
    """Store neural network predictions as a GeoTiff file, through GDAL toolset.

    Parameters
    ----------
    output_data : np.array
        Neural network predictions, as a 3dim-array (width, height, band)
    output_filename : pathlib.Path
        Path of the output image, on the file system
    input_dataset_features : dict
        Input image characteristic, used as a template for producing an output image. One
    specificly needs image width and height, geotransform and WKT projection.

    """
    output_filename.parent.mkdir(parents=True, exist_ok=True)
    # Reshape to a GDAL-compliant array
    img_height, img_width, nb_band = output_data.shape
    assert img_width == input_dataset_features["width"]
    assert img_height == input_dataset_features["height"]
    tifdata = output_data.transpose(2, 0, 1).reshape(nb_band, -1)
    # Create the output dataset, and write data
    ...
    # Set output dataset metadata, and commit the changes
    dst_ds.SetGeoTransform(input_dataset_features["geotransform"])
    dst_ds.SetProjection(input_dataset_features["wkt_projection"])
    dst_ds.FlushCache()
    logger.info("A new GDAL dataset has been stored at %s!", output_filename)
...
def main(args):
    ...
    features = get_image_features(args.datapath, args.dataset, args.image_basename)
    ...
    if len(image_paths) == 0:
        logger.error("No valid images in the preprocessed folder. Hint: run datagen!")
        exit(1)
    ...
    model = get_trained_model(output_folder["best-model"], args.image_size, len(labels))
    ...
    logger.info("Labelled image dimension: %s, %s" % (data.shape[0], data.shape[1]))
    colored_data = assign_label_colors(data, labels)
    if args.draw_grid:
        colored_data = draw_grid(colored_data, img_width, img_height, args.image_size)
    predicted_label_file = Path(
        output_folder["labels"],
        args.image_basename + "_" + str(args.image_size) + ".tif",
    )
    store_raster(colored_data, predicted_label_file, features)

    vectorized_labels, vectorized_data = geometries.vectorize_mask(
        data, colored_data, labels
    )
    vectorized_geometries = geometries.convert_to_geocoord(vectorized_data, features)
    gdf = gpd.GeoDataFrame(
        {"labels": vectorized_labels, "geometry": vectorized_geometries},
        crs=features["srid"],
    )
    predicted_geom_file = Path(
    ...
    predicted_raster_file = Path(
        output_folder["rasters"],
        args.image_basename + "_" + str(args.image_size) + ".tif",
    )
    store_raster(colored_raster_data, predicted_raster_file, features)
