...
from deeposlandia.metrics import dice_coef, iou
...
def run_model(
    ...
    opt = K.optimizers.Adam(lr=learning_rate, decay=learning_rate_decay)
    ...
    checkpoint_files = [
        item for item in os.listdir(output_folder) if "checkpoint-epoch" in item
    ]
    ...
    terminate_on_nan = K.callbacks.TerminateOnNaN()
...
def main(args):
    ...
    with open(output_folder["best-instance"], "w") as fobj:
        json.dump(
            {key: best_instance[key] for key in best_instance if key != "model"},
            fobj,
        )
