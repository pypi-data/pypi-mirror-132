...
from deeposlandia import AVAILABLE_MODELS, config, utils
...
def recover_image_info(dataset, filename):
    ...
    printed_labels = [
        (item["category"], utils.get_html_color(item["color"]))
        for item in img_config["labels"]
...
def predictor_demo(model, dataset, image):
    ...
    if model == "featdet":
        ...
        raise ValueError(("Unknown model, please choose amongst %s.", AVAILABLE_MODELS))
    return render_template(
        dataset + "_demo.html",
        model=model,
