...
class MapillaryDataset(Dataset):
    ...
    def _preprocess(self, image_filename, output_dir, labelling=True):
        ...
        if labelling:
            ...
            labels = utils.build_labels(img_out, self.label_ids, dataset="mapillary")
            new_out_filename = os.path.join(
                output_dir, "labels", os.path.basename(label_filename)
            )
            label_out = np.array(img_out)
            final_img_out = utils.build_image_from_config(label_out, self.label_info)
    ...
    def populate(
        ...
        image_list_longname = [
            os.path.join(input_dir, "images", image_name) for image_name in image_list
        ]
