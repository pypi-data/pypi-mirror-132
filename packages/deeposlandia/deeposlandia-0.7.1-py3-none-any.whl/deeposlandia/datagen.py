...
def main(args):
    ...
    if args.dataset == "mapillary":
        ...
        os.makedirs(os.path.join(prepro_folder["testing"], "labels"), exist_ok=True)
    ...
    if args.nb_training_image > 0:
        if os.path.isfile(prepro_folder["training_config"]):
            train_dataset.load(prepro_folder["training_config"], args.nb_training_image)
        else:
            logger.info(
    ...
    if args.nb_testing_image > 0 or args.testing_filenames is not None:
        if os.path.isfile(prepro_folder["testing_config"]):
            logger.info(
                "There are already some preprocessed files for testing dataset."
            )
        ...
        test_dataset.populate(
            ...
            image_basenames=args.testing_filenames,
            labelling=False,
            nb_processes=int(config.get("running", "processes")),
        )
