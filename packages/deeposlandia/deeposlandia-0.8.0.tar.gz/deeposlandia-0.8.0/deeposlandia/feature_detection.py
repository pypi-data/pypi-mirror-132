...
class FeatureDetectionNetwork(ConvolutionalNeuralNetwork):
    ...
    def __init__(
        ...
        if architecture == "vgg":
            self.out_tensor = self.vgg16()
        elif architecture == "inception":
            self.out_tensor = self.inception()
        elif architecture == "resnet":
            self.out_tensor = self.resnet()
        elif architecture == "simple":
            self.out_tensor = self.simple()
        else:
            logger.error(
                (
    ...
    def output_layer(self, in_layer):
        ...
        tensor
            2D output layer
        """
        out_layer = K.layers.Dense(self.nb_labels, name="output_fc")(in_layer)
        out_layer = K.layers.Activation(activation="sigmoid", name="output_activation")(
            out_layer
        )
        return out_layer
    ...
    def simple(self):
        ...
        layer = self.convolution(
            self.in_tensor, nb_filters=16, kernel_size=7, block_name="conv1"
        )
    ...
    def vgg16(self):
        ...
        compared with ground-truth values
        """
        vgg16_model = K.applications.VGG16(
            input_tensor=self.in_tensor, include_top=False
        )
        out_layer = self.flatten(vgg16_model.output, block_name="flatten")
        out_layer = self.dense(out_layer, 1024, block_name="fc1")
        out_layer = self.dense(out_layer, 1024, block_name="fc2")
        return self.output_layer(out_layer)
    ...
    def resnet(self):
        ...
        compared with ground-truth values
        """
        resnet_model = K.applications.resnet50.ResNet50(
            include_top=False, input_tensor=self.in_tensor
        )
        out_layer = self.flatten(resnet_model.output)
        return self.output_layer(out_layer)
    ...
    def inception(self):
        ...
        """
        inception_model = K.applications.inception_v3.InceptionV3(
            input_tensor=self.in_tensor, include_top=False
        )
        out_layer = K.layers.GlobalAveragePooling2D()(inception_model.output)
        return self.output_layer(out_layer)
