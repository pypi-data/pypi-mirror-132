...
class ShapeDataset(Dataset):
    ...
    def generate_labels(self, nb_images):
        ...
        """
        raw_labels = [
            np.random.choice(np.arange(nb_images), int(nb_images / 2), replace=False)
            for i in range(self.get_nb_labels())
        ]
        labels = np.zeros([nb_images, self.get_nb_labels()], dtype=int)
        for i in range(self.get_nb_labels()):
            labels[raw_labels[i], i] = 1
        return [{i: int(j) for i, j in enumerate(label)} for label in labels]
    ...
    def populate(
        ...
        if nb_tiles_per_image is not None:
            logger.warning(
                "The ``nb_tiles_per_image`` parameter is useless, it will be ignored."
            )
        ...
        for i, image_label in enumerate(shape_gen):
            ...
            shape_specs = []
            for label in image_label.items():
                if label:
    ...
    def add_image(self, image_id, background, specifications, labels):
        ...
        if image_id in self.image_info:
            logger.error("Image %s already stored into the label set.", image_id)
            return None
    ...
    def draw_image(self, image_id, datapath):
        ...
        # Get the center x, y and the size
        if image_info["labels"][self.SQUARE]:
            color, x, y, size = image_info["shape_specs"][self.SQUARE]
            color = tuple(map(int, color))
            image = cv2.rectangle(
                image, (x - size, y - size), (x + size, y + size), color, -1
            )
            label = cv2.rectangle(
                label, (x - size, y - size), (x + size, y + size), self.SQUARE_COLOR, -1
            )
        if image_info["labels"][self.CIRCLE]:
            color, x, y, size = image_info["shape_specs"][self.CIRCLE]
            color = tuple(map(int, color))
            image = cv2.circle(image, (x, y), size, color, -1)
            label = cv2.circle(label, (x, y), size, self.CIRCLE_COLOR, -1)
        if image_info["labels"][self.TRIANGLE]:
            color, x, y, size = image_info["shape_specs"][self.TRIANGLE]
            color = tuple(map(int, color))
