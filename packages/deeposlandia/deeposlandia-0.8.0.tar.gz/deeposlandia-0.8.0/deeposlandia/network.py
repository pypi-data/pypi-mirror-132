...
class ConvolutionalNeuralNetwork:
    ...
    def convolution(
        ...
        tensor
            4D output layer
        """
        in_layer = K.layers.Conv2D(
            nb_filters,
            kernel_size=kernel_size,
            strides=strides,
            dilation_rate=dilation_rate,
            padding=padding,
            name=layer_name(block_name, "_conv"),
        )(in_layer)
        if batch_norm:
            in_layer = K.layers.BatchNormalization(name=layer_name(block_name, "_bn"))(
                in_layer
            )
        in_layer = K.layers.Activation(
            activation, name=layer_name(block_name, "_activation")
        )(in_layer)
        return in_layer
    ...
    def transposed_convolution(
        ...
        tensor
            4D output layer
        """
        in_layer = K.layers.Conv2DTranspose(
            nb_filters,
            kernel_size=kernel_size,
            strides=strides,
            padding=padding,
            name=layer_name(block_name, "_transconv"),
        )(in_layer)
        if batch_norm:
            in_layer = K.layers.BatchNormalization(name=layer_name(block_name, "_bn"))(
                in_layer
    ...
    @staticmethod
    def maxpool(in_layer, pool_size, strides=1, padding="same", block_name=None):
    ...
    def dense(
        self, in_layer, depth, activation="relu", batch_norm=True, block_name=None
    ):
        ...
        tensor
            Output layer
        """
        in_layer = K.layers.Dense(depth, name=layer_name(block_name, "_fc"))(in_layer)
        if batch_norm:
            in_layer = K.layers.BatchNormalization(name=layer_name(block_name, "_bn"))(
                in_layer
            )
        ...
        in_layer = K.layers.Dropout(
            self.dropout_rate, name=layer_name(block_name, "_dropout")
        )(in_layer)
        return in_layer

    @staticmethod
    def flatten(in_layer, block_name=None):
        """Apply a flattening operation to input tensor `in_layer`, to reduce its
        ...
        tensor
            2D output layer
        """
        return K.layers.Flatten(name=block_name)(in_layer)

    @staticmethod
    def upsample(layer1, layer2, block_name=None):
        ...
        """
        upname = layer_name(block_name, "_up")
        ccname = layer_name(block_name, "_concat")
