...
from deeposlandia import AVAILABLE_MODELS, datagen, geoinference, inference, train
...
def datagen_parser(subparser, reference_func):
    ...
    parser = subparser.add_parser(
        "datagen", help="Preprocess new datasets for automated image analysis"
    )
    ...
    testing_image_group = parser.add_mutually_exclusive_group()
    testing_image_group.add_argument(
        "-T",
        "--nb-testing-image",
        type=int,
        default=0,
...
def train_parser(subparser, reference_func):
    ...
    """
    parser = subparser.add_parser(
        "train", help="Train convolutional neural networks for automated image analysis"
    )
    add_dataset_args(parser)
    add_nb_image_args(parser)
...
def inference_parser(subparser, reference_func):
    ...
    """
    parser = subparser.add_parser(
        "infer", help="Predict image labelling with a neural network model"
    )
    add_dataset_args(parser)
    add_training_args(parser)
    parser.add_argument(
...
def geoinference_parser(subparser, reference_func):
    """Add instance-specific arguments from the command line
    ...
    parser = subparser.add_parser(
        "geoinfer",
        help=(
            "Predict georeferenced vectors and rasters with a neural network"
...
def add_training_args(parser):
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[50],
        help=("Number of images that must be contained " "into a single batch"),
    )
    parser.add_argument(
        "-d",
        "--dropout",
...
def main():
    ...
    geoinference_parser(sub_parsers, reference_func=geoinference.main)
