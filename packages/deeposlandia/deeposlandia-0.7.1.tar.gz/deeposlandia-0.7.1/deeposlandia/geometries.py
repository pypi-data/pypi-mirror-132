...
def get_pixel(coord, min_coord, max_coord, size):
    ...
    if isinstance(coord, list):
        return [int(size * (c - min_coord) / (max_coord - min_coord)) for c in coord]
    if isinstance(coord, float):
        return int(size * (coord - min_coord) / (max_coord - min_coord))
    raise TypeError(f"Unknown type ({type(coord)}), pass a 'list' or a 'float'")
...
def get_geocoord(coord, min_coord, max_coord, size):
    ...
    raise TypeError(f"Unknown type ({type(coord)}), pass a 'list' or a 'int'")
...
def extract_points_from_polygon(polygon, geofeatures, min_x, min_y):
    ...
    """
    raw_xs, raw_ys = polygon.exterior.xy
    pix_x = get_pixel(
        list(raw_xs),
        geofeatures["west"],
        geofeatures["east"],
    ...
    points = np.array([[y, x] for x, y in zip(pix_x, pix_y)], dtype=np.int32)
    points[:, 0] -= min_y
    points[:, 1] -= min_x
    return points
...
def extract_tile_items(raster_features, labels, min_x, min_y, tile_width, tile_height):
    ...
    """
    area = get_tile_footprint(raster_features, min_x, min_y, tile_width, tile_height)
    bdf = gpd.GeoDataFrame(crs=from_epsg(raster_features["srid"]), geometry=[area])
...
def retrieve_area_color(data, contour, labels):
    ...
    for label in labels:
        dist = np.linalg.norm(label["color"] - np.array(mean))
        if dist < min_dist[0]:
            min_dist = (dist, label["id"])
    return min_dist[1]


def vectorize_mask(
    mask, colored_data, labels, min_area=10.0, structure_size=(10, 10), approx_eps=0.01
):
    ...
    """
    contours, hierarchy = extract_geometry_vertices(mask, structure_size, approx_eps)
    if not contours:
    ...
    fixed_all_polygons, fixed_out_labels = [], []
    for poly, label in zip(all_polygons, out_labels):
        fixed_poly = poly.buffer(0)
        if isinstance(fixed_poly, shgeom.Polygon):  # No problem
            fixed_all_polygons.append(fixed_poly)
            fixed_out_labels.append(label)
        elif isinstance(
            fixed_poly, shgeom.MultiPolygon
        ):  # The fix produces several Polygons
            for fpoly in fixed_poly:
                fixed_all_polygons.append(fpoly)
                fixed_out_labels.append(label)
        else:
            raise ValueError(
                "Unknown data produced when trying to fix an unvalid geometry..."
            )
    fixed_all_polygons = shgeom.MultiPolygon(fixed_all_polygons)
    # Sometimes buffer() converts a simple Multipolygon to just a Polygon,
    # need to keep it a Multi throughout
    if fixed_all_polygons.type == "Polygon":
        fixed_all_polygons = shgeom.MultiPolygon([fixed_all_polygons])
    return np.array(fixed_out_labels), fixed_all_polygons
...
def rasterize_polygons(polygons, labels, img_height, img_width):
    ...
    for polygon, label in zip(polygons, labels):
        ...
        interiors = [
            np.array(pi.coords).round().astype(np.int32) for pi in polygon.interiors
        ]
...
def convert_to_geocoord(polygons, geofeatures):
    ...
    for polygon in polygons:
        ...
        interiors = [pixel_to_geocoord(pi, geofeatures) for pi in polygon.interiors]
        if len(interiors) > 0:
            poly = shgeom.Polygon(shell=exterior, holes=interiors)
        else:
