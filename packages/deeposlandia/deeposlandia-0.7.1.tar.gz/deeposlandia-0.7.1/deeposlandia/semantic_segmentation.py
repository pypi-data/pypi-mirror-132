...
class SemanticSegmentationNetwork(ConvolutionalNeuralNetwork):
    ...
    def __init__(
        ...
        if architecture == "unet":
            ...
            self.out_tensor = self.unet()
        elif architecture == "dilated":
            self.out_tensor = self.dilated(add_context=True)
        elif architecture == "simple":
            self.out_tensor = self.simple()
        else:
            logger.error(
                ("Unknown network architecture. Please use " "'simple' or 'unet'.")
            )
    ...
    def output_layer(self, in_layer):
        ...
        tensor
            2D output layer
        """
        out_layer = K.layers.Conv2DTranspose(
            self.nb_labels, kernel_size=2, padding="same", name="output_trconv"
        )(in_layer)
        out_layer = K.layers.Activation("softmax", name="output_activation")(out_layer)
        return out_layer
    ...
    def simple(self):
        ...
        layer = self.convolution(
            self.in_tensor, nb_filters=32, kernel_size=3, block_name="conv1"
        )
    ...
    def unet(self):
        ...
        conv1 = self.convolution(
            self.in_tensor, nb_filters=32, kernel_size=3, block_name="conv1a"
        )
        conv1 = self.convolution(
        ...
        conv8 = self.convolution(up3, nb_filters=64, kernel_size=3, block_name="conv8a")
        conv8 = self.convolution(
            conv8, nb_filters=64, kernel_size=3, block_name="conv8b"
        )

        up4 = self.upsample(conv8, conv1, block_name="up4")
        conv9 = self.convolution(up4, nb_filters=32, kernel_size=3, block_name="conv9a")
    ...
    def dilated(self, add_context=False):
        ...
        pool1 = self.maxpool(conv1, pool_size=2, strides=2, block_name="pool1_fe")
        conv2 = self.convolution(
            pool1,
            nb_filters=128,
            kernel_size=3,
        ...
        pool2 = self.maxpool(conv2, pool_size=2, strides=2, block_name="pool2_fe")
        ...
        pool3 = self.maxpool(conv3, pool_size=2, strides=2, block_name="pool3_fe")
