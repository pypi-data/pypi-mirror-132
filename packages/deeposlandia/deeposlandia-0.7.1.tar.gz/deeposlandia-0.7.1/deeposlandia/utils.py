...
def prepare_preprocessed_folder(datapath, dataset, image_size):
    ...
    dictionary
    """
    prepro_folder = os.path.join(datapath, dataset, "preprocessed", str(image_size))
...
def get_html_color(color):
    ...
    """
    rgb_tuple = (color, color, color) if isinstance(color, int) else color
...
def build_image_from_config(data, config):
    ...
    """
    result = np.zeros(shape=(data.shape[0], data.shape[1], 3), dtype=data.dtype)
    for label, _ in enumerate(config):
        mask = data == label
