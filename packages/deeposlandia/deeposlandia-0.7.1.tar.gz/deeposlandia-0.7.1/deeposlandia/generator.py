...
def recover_label_id(img, label_config):
    ...
    label_img = np.full(
        shape=reshaped_img.shape[0], fill_value=-1, dtype=reshaped_img.dtype
    )
...
def feature_detection_labelling(img, label_config):
    ...
    if not (img.shape[1] == img.shape[2] and len(img.shape) == 4):
        raise ValueError(
            (
                "Wrong image shape. Please provide batched RGB-"
                "images with equal width and height dimensions."
            )
        )
    label_ids = [item["id"] for item in label_config if item["is_evaluate"]]
    if not all(
        isinstance(item, (int, np.uint8, np.uint32, np.uint64)) for item in label_ids
    ):
        raise ValueError(
            ("List of label IDs must contains " "integers: {}").format(label_ids)
        )
...
def create_generator(
    ...
    if model == "featdet":
        label_generator = (
            feature_detection_labelling(x, label_config) for x in label_generator
        )
    elif model == "semseg":
        label_generator = (
            semantic_segmentation_labelling(x, label_config) for x in label_generator
        )
    else:
        raise ValueError(
            "Wrong model name {} (choose amongst {})".format(model, AVAILABLE_MODELS)
        )
