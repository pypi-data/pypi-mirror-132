...
def init_model(problem, instance_name, image_size, nb_labels, dropout, network):
    ...
    if problem == "featdet":
        ...
        raise ValueError("Unrecognized model. Please enter 'featdet' or 'semseg'.")
    return K.models.Model(net.in_tensor, net.out_tensor)
...
def predict(
    ...
    output_folder = utils.prepare_output_folder(
        datapath, dataset, model_input_size, problem
    )
    dropout, network = utils.recover_instance(output_folder["best-instance"])
    model = init_model(
        problem,
        instance_name,
        model_input_size,
    ...
    if problem == "featdet":
        label_info = [
            (i["category"], utils.get_html_color(i["color"]))
        ...
        meaningful_labels = [
            x for i, x in enumerate(train_config["labels"]) if i in encountered_labels
        ]
        ...
        for predicted_labels, filename in zip(labelled_images, flattened_image_paths):
            ...
            predicted_image_path = os.path.join(output_dir, os.path.basename(filename))
        ...
        raise ValueError("Unknown model argument. Please use 'featdet' or 'semseg'.")
    return result
...
def summarize_config(config):
    ...
    """
    return [(c["category"], utils.get_html_color(c["color"])) for c in config]
