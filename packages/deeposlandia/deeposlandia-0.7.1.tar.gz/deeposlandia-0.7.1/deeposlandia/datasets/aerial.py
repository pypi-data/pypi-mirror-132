...
class AerialDataset(GeoreferencedDataset):
    ...
    def __init__(self, tile_size):
        ...
        self.add_label(label_id=0, label_name="background", color=0, is_evaluate=True)
        self.add_label(label_id=1, label_name="building", color=255, is_evaluate=True)
    ...
    def _preprocess_for_training(self, image_filename, output_dir, nb_images):
        ...
        while image_counter < nb_images and nb_attempts < 2 * nb_images:
            ...
            tile_data = image_data[x : (x + self.image_size), y : (y + self.image_size)]
            tile_image = Image.fromarray(tile_data)
            mask = labels[x : (x + self.image_size), y : (y + self.image_size)]
            label_dict = utils.build_labels(mask, range(self.get_nb_labels()), "aerial")
            ...
            nb_attempts += 1
        del raster
        logger.info("Generate %s images after %s attempts.", image_counter, nb_attempts)
        return result_dicts
