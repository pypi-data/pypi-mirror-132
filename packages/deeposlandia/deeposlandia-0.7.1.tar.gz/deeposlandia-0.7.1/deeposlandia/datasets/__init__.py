...
class Dataset(metaclass=abc.ABCMeta):
    ...
    def get_label_popularity(self):
        ...
        return np.round(
            np.divide(
                sum(np.array([list(label.values()) for label in labels])),
                self.get_nb_images(),
            ),
            3,
        )
    ...
    def add_label(
        ...
        if label_id in self.label_info:
            logger.error("Label %s already stored into the label set.", label_id)
            return None
    ...
    def load(self, filename, nb_images=None):
        ...
        self.label_info = dataset["labels"]
        if nb_images is None:
            self.image_info = dataset["images"]
        else:
            self.image_info = dataset["images"][:nb_images]
...
class GeoreferencedDataset(Dataset):
    ...
    def _serialize(
        ...
        try:
            ...
            logger.error(
                "The image %s is corrupt, hence not serialized.", image_filename
            )
            return None

    def _preprocess_tile(self, x, y, image_filename, output_dir, raster):
        ...
        """
        dirs = self._generate_preprocessed_filenames(image_filename, output_dir, x, y)
        gdal.Translate(
            dirs["image"],
            raster,
            format="PNG",
    ...
    def _preprocess_for_training(self, image_filename, output_dir, nb_images):
        ...
        To be implemented in subclasses in tanzania.py or aerial.py.
    ...
    def populate(
        ...
        image_basenames : list
            List of image basenames to consider during preprocessing
        labelling : boolean
        ...
        image_list_longname = [
            os.path.join(input_dir, "images", image_name)
            for image_name in image_list
            if not image_name.startswith(".")
        ]
        nb_image_files = len(image_list_longname)
        if image_basenames is not None:
            logger.info("Considering a set of %s basenames...", len(image_basenames))
            image_list_longname = [
                image_name
                for image_name in image_list_longname
                if Path(image_name).stem in image_basenames
            ]
        else:
        ...
        if labelling:
            if nb_processes == 1:
                for x in image_list_longname:
                    image_info.append(
                        self._preprocess_for_training(x, output_dir, nb_tiles_per_image)
                    )
            else:
                with Pool(processes=nb_processes) as pool:
                    image_info = pool.starmap(
            ...
            if nb_processes == 1:
                for x in image_list_longname:
                    image_info.append(self._preprocess_for_inference(x, output_dir))
            else:
                with Pool(processes=nb_processes) as pool:
                    image_info = pool.starmap(
        ...
        image_info = [item for sublist in image_info for item in sublist]
        logger.info(
        ...
        if labelling:
            ...
            # Concatenate image infos
            self.image_info = image_info + self.image_info
        logger.info("There are now %s images in the dataset.", len(self.image_info))
