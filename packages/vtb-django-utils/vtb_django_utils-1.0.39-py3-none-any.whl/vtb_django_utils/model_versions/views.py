...
from django.db.models import Max, Value, Func, F, Case, When, Q, OuterRef, Subquery
...
class VersionInfoMixin(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == 'list':
            ...
            last_version = version_model.objects.filter(**{
                main_model_field_name: OuterRef('pk'),
            }).order_by('pk', '-version_arr').distinct('pk')
            # json версии
            queryset = queryset.annotate(
                last_version_json_query=Subquery(last_version.values('json')[:1]),
                last_version_create_dt_query=Subquery(last_version.values('create_dt')[:1]),
                last_version_changed_by_user_query=Subquery(last_version.values('user__username')[:1]),
            )
