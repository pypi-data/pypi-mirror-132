...
from .utils.consts import START_VERSION, THERE_IS_NO_VERSION_DATA, DOES_NOT_EXIST_VERSION, REL_VERSION_FIELD_END, \
    ...
    VERSION_PATTERN_REGEX, VERSION_MODEL_SUFFIX
...
class ModelWithVersionedRelations(ModelVersionedRelFieldsInfoMixin, ModelJsonMixin, models.Model):
    ...
    def validation_versioned_instance_not_selected(self):
        ...
        for rel_field in self.rel_versioned_fields:
            ...
            if not versioned_instance_id and (version or version_pattern):
                raise ValidationError(ERR_VERSIONED_OBJ_NOT_SELECTED.format(self.class_name))
    ...
    def get_rel_model_version(self, rel_field_name):
    ...
    def get_rel_model_version_str(self, rel_field_name):
...
def prefetched_cache(cache_name: str):
    def _decorator(func):
        @wraps(func)
        def __wrap(self, *args, **kwargs):
            var_name = f'_{func.__name__}'
            if not (values_store := getattr(self, 'values_store', defaultdict(dict))):
                self.values_store = values_store
            if not (result := values_store[cache_name].get(var_name)):
                result = values_store[cache_name][var_name] = func(self, *args, **kwargs)
            return result
        return __wrap
    return _decorator
...
def clear_prefetch_cache(self, cache_name: str):
    ...
    if hasattr(self, 'values_store'):
        del self.values_store
...
class VersionedModelMixin(ModelVersionedRelFieldsInfoMixin, ModelJsonMixin):
    ...
    def create_or_update_version(self, version_str: str = None) -> Tuple[bool, VersionModel]:
        ...
        clear_prefetch_cache(self, 'versions')
