...
def one_step_process(path, company, profile=True, fold_change=5, area_threhold=500):
    '''
...
def gen_lib_result_DIA(lib_df, mzml_file, unique_excel,profile=True,mode='pos', i_min=5000, company='Waters'):
    ...
    for i in tqdm(range(len(unique_df)), desc='lib matching'):
        ...
        try:
            result = str(ms2_search(precursor, mode=mode,ms2_frag, lib_df))
        except:
            result = None
...
def identify_isotopes(cmp):
    ...
    for i in range(len(all_rts)):
        cmp_rt = cmp[cmp['rt'] == all_rts[i]].sort_values(by='mz')
        ...
        for mz in mzs:
            ...
            if len(i_Cl_C13) == 0:
                ...
                try:
                    if cmp_rt[cmp_rt['mz'] == mz]['intensity'].values / 2 > cmp_rt.loc[index_, 'intensity'].values:
                        cmp.loc[index_, 'Comments'] = f'Potential Cl isotope for  {all_rts[i]} _{mz}'
                except:
                    pass
            else:
                index_ = cmp_rt.index[i_Cl_C13]
                for index in index_:
                    try:
                        if cmp_rt[cmp_rt['mz'] == mz]['intensity'].values / 2 > cmp_rt.loc[index, 'intensity']:
                            cmp.loc[index, 'Comments'] = f'Potential Cl isotope for {all_rts[i]} _{mz}'
                    except:
                        pass
    return cmp
