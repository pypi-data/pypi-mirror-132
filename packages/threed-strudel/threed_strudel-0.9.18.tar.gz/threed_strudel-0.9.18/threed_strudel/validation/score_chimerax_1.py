...
def slave(pairs_list, lib, lock, json_out_path, csv_out_path=None, close_rate=50):
    score_list = []
    p = psutil.Process()
    log = logging.getLogger(str(p.pid))

    logging.basicConfig(filename='process_logs/' + str(p.pid) + '.process_log', level=logging.INFO,
    ...
    for k, pair in enumerate(pairs_list):
        ...
        if k % 10 == 0 or k == len(pairs_list)-1:
            scores_list_out = []
            for i in score_list:
                scores_list_out.append(i)
            if os.path.exists(json_out_path):
                copy(json_out_path, json_out_path + '_bak')
                tmp_list = read_json(json_out_path)
                scores_list_out = scores_list_out + tmp_list
...
def score_structure(known_correlations, unknown_pairs, lib, json_out_path, csv_out_path=None, np=4):
    ...
    if len(known_correlations) > 0:
        known_correlations.sort(key=operator.itemgetter(keys.CHAIN, keys.RES_NR))
        copy(json_out_path, json_out_path + '_bak')
        save_json(json_out_path, known_correlations)
        if csv_out_path:
            with open(csv_out_path, mode='w') as csv_file:
                fieldnames = [key for key in known_correlations[0].keys()]
                writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                writer.writeheader()
                for item in known_correlations:
                    writer.writerow(item)
    ...
    for batch_list in split_pairs:
        t = Process(target=slave, args=(batch_list, lib, lock, json_out_path, csv_out_path))
    ...
    #             writer.writerow(item)
    # Combine logs
    combine_files(log_tmp, 'score_' + datetime.now().strftime("%Y-%m-%d") + '.process_log')
