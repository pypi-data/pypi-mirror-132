...
def __list_jupyterlab_workspace(target_path):
    file_lists = []
    for (dirpath, dirnames, filenames) in os.walk(target_path):
        for fn in filenames:
            if not fn.endswith('.swp'):
                if 'auto' in fn: continue
...
def get_current_notebook():
    if is_in_ipython():
        ...
        if os.path.isdir(target_path): # if jupyter lab
            return __get_current_file(target_path=target_path,
                                      target_file=__list_jupyterlab_workspace(target_path=target_path)[0]
                                      ).split(':')[1]
        else:
