...
def filter_columns_data_on_hide_columns(
    model, columns_data: Dict[str, Dict]) -> Dict[str, Dict]:
    """ remove from display on edit & add form columns """
    filtered_columns = {}
    for column_name, column in columns_data.items():
        try:
            column_in_model = getattr(
            model, column_name)
        except AttributeError:
            column_in_model = getattr(model, f'{column_name}_hash')
        if column_name not in cfg.hide_columns and column_in_model not in cfg.hide_columns:
