...
class PyXploraApi:
    def __init__(self, countryPhoneNumber: str, phoneNumber: str, password: str, userLang: str, timeZone: str):
        ...
        self.watch_user_id = self.myInfo['children'][0]['ward']['id']
        self.watch_user_name = self.myInfo['children'][0]['ward']['name']
...
##### Watch Info #####
    def getWatchCurrentStep(self):
        return self.myInfo['children'][0]['ward']['currentStep']

    def getWatchTotalStep(self):
        return self.myInfo['children'][0]['ward']['totalStep']
...
##### Watch Location Info #####
    ...
    def setAllEnableSilentTime(self):
        res = []
        for time in self.schoolSilentMode():
            res.append(self.setEnableSilentTime(time['id']))
        return res

    def setAllDisableSilentTime(self):
        res = []
        for time in self.schoolSilentMode():
            res.append(self.setDisableSilentTime(time['id']))
        return res
