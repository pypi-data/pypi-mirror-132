...
try:
    ...
    reverse_spreadsheet = cdll.LoadLibrary("reverse_spreadsheet.cpython-38-darwin.so".format(path))
    TFIDF_max = cdll.LoadLibrary("TFIDF_max.cpython-38-darwin.so".format(path))
    null_parameters = cdll.LoadLibrary("null_parameters.cpython-38-darwin.so".format(path))
    certainty_search = cdll.LoadLibrary("certainty_search.cpython-38-darwin.so".format(path))
    radius_arr = cdll.LoadLibrary("radius_arr.cpython-38-darwin.so".format(path))
    TFIDF_max_elastic = cdll.LoadLibrary("TFIDF_max_elastic.cpython-38-darwin.so".format(path))
except:
