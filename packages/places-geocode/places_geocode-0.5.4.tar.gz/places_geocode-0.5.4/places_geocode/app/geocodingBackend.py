...
so_path = sysconfig.get_paths()["purelib"]
print(so_path)

try:
    spreadsheet_c = cdll.LoadLibrary("{}/{}".format(so_path, 'spreadsheet_c.cpython-38-darwin.so'))
    print("finished spreadsheet")
    reverse_spreadsheet = cdll.LoadLibrary("{}/{}".format(so_path, "reverse_spreadsheet.cpython-38-darwin.so"))
    TFIDF_max = cdll.LoadLibrary("{}/{}".format(so_path, "TFIDF_max.cpython-38-darwin.so"))
    null_parameters = cdll.LoadLibrary("{}/{}".format(so_path, "null_parameters.cpython-38-darwin.so"))
    certainty_search = cdll.LoadLibrary("{}/{}".format(so_path, "certainty_search.cpython-38-darwin.so"))
    radius_arr = cdll.LoadLibrary("{}/{}".format(so_path, "radius_arr.cpython-38-darwin.so"))
    TFIDF_max_elastic = cdll.LoadLibrary("{}/{}".format(so_path, "TFIDF_max_elastic.cpython-38-darwin.so"))
except:
...
class BuildElasticSearch:
    ...
    def create_connection(self):
        ...
        elastic_con = Elasticsearch(
            cloud_id='PlacesDB:ZWFzdHVzMi5henVyZS5lbGFzdGljLWNsb3VkLmNvbTo5MjQzJGU3NDhmMjYwOTUzNTQ1NGI5ZjkxYTBjM2ViZjg0NGUyJGMyZjRiYTg1NjdkMjQ5NjE4NGJjODg4NjJlYmM5NjY5',
            http_auth=('elastic', 'UaHMWHF2irZM7JY')
        )
