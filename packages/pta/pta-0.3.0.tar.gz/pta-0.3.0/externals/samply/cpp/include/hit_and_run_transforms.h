...
public:
    ...
    template <typename ProposalDistribution,
              typename SpaceDescriptor,
              typename ChordSampler>
    HitAndRunTransforms(const ReparametrizedObject<ProposalDistribution>& proposal_Fp,
        ...
        : from_Fi(descriptor_Fi.from_frame().template cast<TransformationScalar>()),
          ...
          Fp_to_Fi_linear(
              (descriptor_Fi.to_frame().linear() * proposal_Fp.from_frame().linear())
                  .template cast<TransformationScalar>()),
          Fp_to_Fs_linear(
              make_Fp_to_Fs_transform(proposal_Fp, descriptor_Fi, sampler_Fs)),
    ...
    DirectionsType transform_directions_from_Fp_to_Fi(
        const DirectionsType& directions_Fp)
    {
        return Fp_to_Fi_linear * directions_Fp;
    }
        ...
        const IndexVector& axis_ids)
    {
        if (Fs_equal_Fi) {
            ...
            return RaysPacket<TransformationScalar>{
...
protected:
    ...
    AffineTransform<TransformationScalar> make_Fp_to_Fs_transform(
        const ReparametrizedObject<ProposalDistribution>& proposal_Fp,
        const ReparametrizedObject<SpaceDescriptor>& descriptor_Fi,
        const ReparametrizedObject<ChordSampler>& sampler_Fs)
    {
        if (sampler_Fs.needs_frame())
            return (sampler_Fs.to_frame().linear() * proposal_Fp.from_frame().linear())
                .template cast<TransformationScalar>();
        else
            return proposal_Fp.from_frame();
    }
