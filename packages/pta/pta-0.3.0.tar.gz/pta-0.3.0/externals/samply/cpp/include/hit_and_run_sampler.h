...
class HitAndRunSampler : public MarkovChain<
  ...
  public:
    ...
    /**
     * @brief The type of the class providing the proposal distribution.
     */
    typedef ProposalDistribution<Scalar> ProposalType;
     ...
     */
    HitAndRunSampler(
        const ProposalDistribution<Scalar>& proposal,
        ...
        , proposal_distribution_Fp(proposal.get_optimally_reparametrized_descriptor())
        ...
        , transforms_(proposal_distribution_Fp, space_descriptor_Fi, chord_sampler_Fs)
    {}
  ...
  protected:
    ...
    {
        // 1) Sample new directions according to the proposal distribution.
        const auto directions_Fp{
            proposal_distribution_Fp.get_directions(static_cast<Index>(state.cols()))};

        // 2) Find the intersections between the current rays and the space
        //    descriptor in the Fi frame.
        typename ProposalType::RaysPacketType rays_Fi{
            state, transforms_.transform_directions_from_Fp_to_Fi(directions_Fp)};
        ...
        if (transforms_.is_Fs_equal_Fi()) {
            ...
            const RaysPacket<Scalar> rays_Fs =
                transforms_.transform_rays_from_Fi_to_Fs(rays_Fi, directions_Fp);
  ...
  private:
    ...
    // Proposal distribution for ray directions.
    ProposalDistribution<Scalar> proposal_distribution;
    ReparametrizedObject<ProposalDistribution<Scalar>> proposal_distribution_Fp;
