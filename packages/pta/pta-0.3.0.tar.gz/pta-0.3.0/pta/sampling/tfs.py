...
from .adaptive_directions_sampler import estimate_proposal_distribution
from .commons import (
    SamplingException,
    SamplingResult,
    apply_to_chains,
...
def sample_drg(
    ...
    adaptive_proposal_scheme: List[int] = None,
    adaptive_proposal_gain: float = 1.0,
    ...
    # Wrap the sampler in a simpler lambda.
    sampler_function = lambda s, p: pb.sample_free_energies(
        model.F.S,
        model.F.lb,
        model.F.ub,
        np.array(
    ...
    # Try to estimate a good proposal distribution.
    if adaptive_proposal_scheme is not None:
        try:
            proposal_transform = estimate_proposal_distribution(
                sampler_function,
                lambda r: r.chains,
                settings,
                num_samples,
                model.dimensionality,
                adaptive_proposal_scheme,
                adaptive_proposal_gain,
            )
        except Exception as e:
            logger.error(f"Estimation of the proposal distribution failed. {e}")
            raise SamplingException(
                "Estimation of the proposal distribution failed."
            ) from e
    else:
        proposal_transform = np.identity(model.dimensionality)
    ...
    try:
        ...
        tfs_result: pb.TFSResult = sampler_function(settings, proposal_transform)
    except Exception as e:
        ...
        basis_samples = sample_from_chains(tfs_result.chains, num_samples)
        samples = model.to_drg(basis_samples.T).T

        logger.info("Computing PSRFs.")
        basis_var_names = ["drg_var" + str(i) for i in range(model.B.dimensionality)]
        var_names = [id + "_drg" for id in model.T.reaction_ids]
        psrf_var_names = basis_var_names + var_names
        psrf = np.hstack(
            [
                split_R(tfs_result.chains),
                split_R(apply_to_chains(tfs_result.chains, model.to_drg)),
            ]
        )
