...
class DWA(object):
    ...
    def setup(self):
        ...
        workers.base_worker.cookies_domain = "." + self.app_settings["domain"]
        if not os.path.isabs(self.app_settings["db"]):
            self.app_settings["db"] = os.path.join(self.app_root, self.app_settings["db"])
        if "hide_400_error_log" in self.app_settings:
            handlers.BaseResponse.hide_400_error_log = self.app_settings["hide_400_error_log"]
        def _sql():
            return sqlq.SqlQueueU(db_port=self.app_settings["db_port"])
        @staticmethod
        def sql(*args, **kwargs):
            if self.ta.sqlqueue:
                result = self.app_settings["sql_watcher"](*args, sql=_sql().sql, **kwargs)
                if result != True:
                    if isinstance(result, Exception):
                        raise result
                    else:
                        raise Exception(result)
                result = _sql().sql(*args, **kwargs)
                ...
                return False
        def _writer():
            return omnitools.WriterU(writer_port=self.app_settings["writer_port"])
        @staticmethod
        def writer(*args, **kwargs):
            if self.ta.writer:
                return _writer().write(*args, **kwargs)
            else:
                return False
        @staticmethod
        ...
        handlers.BaseResponse.export_functions = export_functions
        self.ta.start()
