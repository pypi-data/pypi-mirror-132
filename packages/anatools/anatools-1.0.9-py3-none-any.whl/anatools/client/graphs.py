...
def create_staged_graph(self, name, channelId, graph, description=None, workspaceId=None):
    ...
    graph: str
        The graph in json format. While `YAML` files are used in channel development, the Platform SDK and API only support `JSON`. 
        Ensure that the `YAML` file is valid in order for the SDK to convert `YAML` to `JSON` for you. Otherwise,  provide a graph in `JSON` format.
    description: str
