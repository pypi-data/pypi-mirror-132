...
class Igruha:
    ...
    def main_games(self):
        ...
        resp = self.session.get(self.url, headers=h).text
...
class Game:
    def __init__(self, source, session):
        ...
        self.img_source = self.source.find('img')
        self.name = self.img_source.get('alt')
        self.link = self.source.find('a').get('href')
        self.img = self.img_source.get('src')
        ...
        try:
            if len(self.ids) == 0:
                for i in enumerate(tree.find_all('a', {'class': 'online'})):
                    self.torrents.append(Torrent(i[1], self.torrent_names[i[0]], self.session, self))
            else:
...
class Torrent:
    def __init__(self, source, name, session, game):
        self.game = game
        self.source = source
        self.session = session
        try:
