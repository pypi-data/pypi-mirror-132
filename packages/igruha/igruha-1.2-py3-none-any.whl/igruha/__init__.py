...
class Game:
    ...
    def get_details(self):
        ...
        self.released = alls.split('Год выпуска: ')[1].split('Жанр')[0]
        self.genre = alls.split('Жанр: ')[1].split('Разработчик')[0]
        self.developer = alls.split('Разработчик: ')[1].split('Издательство')[0]
        self.publisher = alls.split('Издательство: ')[1].split('Платформа')[0]
        ...
        self.tablet = alls.split('Таблeтка: ')[1].split('требования:')[0]
        ...
        self.id = int(tree.find('a', {'class': 'torrent'}).get('href').split('id=')[1])
        self.torrent = 'https://q.itorrents-igruha.org/engine/download.php?id=' + str(self.id)
        return self
