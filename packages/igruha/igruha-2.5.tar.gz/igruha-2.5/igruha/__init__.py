...
class Game:
    ...
    def get_details(self):
        ...
        self.alls = '\n'.join(
            tree.find('div', {'style': 'padding-left: 215px;'}).text.replace('<br>', "\n").replace('<br/>', '\n'))
        try:
            self.released = self.alls.split('Год выпуска: ')[1].split('Жанр')[0]
        except:
            try:
                self.released = self.alls.split('Дата выхода: ')[1].split('Жанр')[0]
            except:
                pass
        try:
            self.genre = self.alls.split('Жанр: ')[1].split('Разработчик')[0]
        except:
            pass
        try:
            self.developer = self.alls.split('Разработчик: ')[1].split('Издательство')[0]
        except:
            pass
        try:
            self.publisher = self.alls.split('Издательство: ')[1].split('Платформа')[0]
        except:
            self.publisher = self.alls.split('Издатель: ')[1].split('Платформа')[0]
        try:
            self.platform = self.alls.split('Платформа: ')[1].split('Тип')[0]
        except:
            pass
        try:
            self.edition_type = self.alls.split('Тип издания: ')[1].split('интерфейса')[0]
        except:
            pass
        try:
            self.lang_interface = self.alls.split('интерфейса: ')[1].split('озвучки: ')[0]
        except:
            pass
        try:
            self.lang_voice = self.alls.split('озвучки: ')[1].split('Таблетка')[0]
        except:
            pass
        try:
            self.tablet = self.alls.split('летка:')[1].split('Сист')[0]
        except:
            print('WARN: Не удалось найти информацию про таблетку')
        try:
            self.requirements = self.alls.split('Системные требования:')[1].split('\n\n')[0]
        except:
            pass
        try:
