...
9.Users will get reasons of features aren`t picked up,as performance is fall or p-value is more than threshold or signs is not in accord with user`s expect and so on after adding this feature 
...
def test_measureXy(X,y):
  ...
  #
  #    dr = {'redundant_1': (['模型性能=0.532386,小于等于最终模型的性能=0.532386',
...
class LogisticReg(Regression):
    ...
    Note:Try best to avoid using validation data set and test data set with confusing.When selecting feature,it means data leak that using test data set,so that may over evaludate model performance.It shoule reserve a cleaning test data set to evaluate model.
    ...
    '''
    def __init__(self,X,y,fit_weight=None,measure='ks',measure_weight=None,measure_frac=None,measure_X=None,measure_y=None,kw_measure_args=None,max_pvalue_limit=0.05,max_vif_limit=3,max_corr_limit=0.6,coef_sign=None,iter_num=20,kw_algorithm_class_args=None,n_core=None,logger_file_CH=None,logger_file_EN=None):
        Regression.__init__(self,X,y,fit_weight,measure,measure_weight,measure_frac,measure_X,measure_y,kw_measure_args,max_pvalue_limit,max_vif_limit,max_corr_limit,coef_sign,iter_num,kw_algorithm_class_args,n_core,logger_file_CH,logger_file_EN) 
...
class LinearReg(Regression):
    ...
    在0.2.1版本中新加入的功能
    11.在做变量选择时，使用另一个不同于训练集的数据集来计算性能指标，然后决定是否加入或删除该变量。（尽量不要混用验证集和测试集，因为在调参或者做变量选择时使用测试集也相当于数据泄露，这样会高估测试集的模型性能，应尽可能的保存一份干净的数据来做模型评价）
    ...
    measure_X:DataFrame | None(默认) New in 0.2.1
    当做变量选择时，用一份不同于训练集的X做模型评价
    默认：None，为使用训练集X做模型评价
    
    measure_y:Series | None(默认) New in 0.2.1
    当做变量选择时，用一份不同于训练集的y做模型评价
    默认：None，为使用训练集y做模型评价
    注意：尽量不要混用验证集和测试集，因为在做变量选择时使用测试集相当于数据泄露，这样会高估测试集的模型性能，应尽可能的保存一份干净的数据来做模型评价 
    ...
    1.Supporting forward-backward Step-Wise.
    2.Supporting multi-processing.When adding or removing features,multi-processing is used to traversal all candidate features.
    3.Supporting that user could point the index instead of AIC/BIC for measuring model performance when adding or removing feaures.That is benefit when user`s data is unbalanced.
    4.Supporting that user could point p-value threshold.If max p-value is more than this threshold,the current features will not be added,although getting a lift on performance of model.
    5.Supporting that user could point VIF threshold.If max VIF is more than this threshold,the current features will not be added,although getting a lift on performance of model.
    6.Supporting that user could point coefficient of correlation threshold.If max coefficient of correlation is more than this threshold,the current features will not be added,although getting a lift on performance of model.
    7.Supporting that user could point sign to coefficients of regression. A part of features have sense in some business like woe transfer which require that all coefficients of regression are postive or negtive.If the signs requirement is not met,the current features will not be added,although getting a lift on performance of model.
    ...
    New characteristics in 0.2.1
    11.When deciding that current feature should be in model or not, uses a new data set diffenent with trainning data set to evaluate model.(Try best to avoid using validation data set and test data set with confusing.When selecting feature,it means data leak that using test data set,so that may over evaludate model performance.It shoule reserve a cleaning test data set to evaluate model)
    ...
    '''
    def __init__(self,X,y,fit_weight=None,measure='r2',measure_weight=None,measure_X=None,measure_y=None,kw_measure_args=None,max_pvalue_limit=0.05,max_vif_limit=3,max_corr_limit=0.6,coef_sign=None,iter_num=20,kw_algorithm_class_args=None,n_core=None,logger_file_CH=None,logger_file_EN=None):
        Regression.__init__(self,X,y,fit_weight,measure,measure_weight,None,measure_X,measure_y,kw_measure_args,max_pvalue_limit,max_vif_limit,max_corr_limit,coef_sign,iter_num,kw_algorithm_class_args,n_core,logger_file_CH,logger_file_EN) 
