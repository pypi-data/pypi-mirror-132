...
# set nfs
NFS_MASTER_HOST = '10.55.41.243'
NFS_PATH = '/kubeflow-pv-test/'
...
def _dict2list(dic: dict = None):
    ...
    if dic:
        for key, value in dic.items():
            ls.append(key)
            if _is_PipelineParam(value):
                value = _input_arg_path(value)
            ls.append(value)
        return ls
    else:
        return None
...
def _uniform_data_path(dic: dict = None):
    ...
    if dic:
        for key, value in dic.items():
            if _is_PipelineParam(value):
                re_dic[key] = _input_arg_path(value)
            else:
                re_dic[key] = value
        return re_dic
    else:
...
class Component(object):
    ...
    def udf_op(self, request_cpu: Optional[str] = None, request_mem: Optional[str] = None,
        ...
        self._resource_limit(request_cpu=request_cpu, request_mem=request_mem)
        return self.op
    ...
    def _resource_limit(self, request_cpu: Optional[str] = None, request_mem: Optional[str] = None):
        """
        resource: cpu, memory
