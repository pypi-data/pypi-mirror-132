...
class AppWindow(QtWidgets.QMainWindow, layout.Ui_MainWindow):
			...
			if 'compile' in self.mode:
				try:
					...
					if self.p.version == REGISTERS.VERSION_ATMEGA32 or self.p.connected==False: #by default, compile for ATMEGA32.
						...
						cmd = 'avr-gcc -Wall -O2 -mmcu=%s -Wl,-Map="%s" -o "%s" "%s"' %('atmega32',fname+'.map',fname,self.fname) #includes MAP
	...
	def openFile(self):
		...
		if len(filename[0]):
			...
			infile.close()
			self.codingTabs.setTabText(self.codingTabs.indexOf(self.sourceTab), os.path.split(self.CFile)[1])
			self.log.setText('''<span style="color:cyan;">-- Opened File: %s --</span><br>'''%filename[0])

	def codingTabChanged(self,tabNumber):
		pass
	def fontPlus(self):
