...
from reader_rbv.constants import VERIVY_SSL
...
def get_url(
    ...
    **kwargs,
    ...
    res = session.get(
        url, params=params, headers=headers, verify=VERIVY_SSL, *args, **kwargs
    )
    if not res.ok:
...
def get_image(
    ...
    res = session.get(image.url, headers=image._headers, stream=True, verify=VERIVY_SSL)
    if not res.ok:
