...
class HestonCondMc(sv.SvABC, sv.CondMcBsmABC):
    ...
    def vol_paths(self, tobs):
        ...
        for i in range(n_dt):
            vv = (
                vv
                + self.mr * (self.theta ** 2 - vv) * dt[i]
                + np.sqrt(vv) * self.vov * dB_t[i, :]
            )  # Euler method
            vv = vv + 0.25 * self.vov ** 2 * (
                dB_t[i, :] ** 2 - dt[i]
            )  # Milstein method
    ...
    def cond_fwd_vol(self, texp):
        ...
        int_sig_dw = (
            (vv_ratio - 1) * vv0
            - self.mr * texp * (self.theta ** 2 - int_var_std * vv0)
        ) / self.vov
        fwd_cond = np.exp(
            self.rho * int_sig_dw - 0.5 * self.rho ** 2 * int_var_std * vv0 * texp
        )
    ...
    def price(
        ...
        if scheme == "QE":
            ...
            for i in range(self.step):
                ...
                s2 = vt[:, i] * (self.vov ** 2) * expo * (
                    1 - expo
                ) / self.kappa + self.theta * (self.vov ** 2) * ((1 - expo) ** 2) / (
                    2 * self.kappa
                )
                psi = s2 / m ** 2
                ...
                for k in range(len(above)):
                    if u[above[k], i] > p[k]:
                        vt[above[k], i + 1] = beta[k] ** -1 * np.log(
                            (1 - p[k]) / (1 - u[above[k], i])
                        )
                    else:
                        vt[above[k], i + 1] = 0
            ...
            for i in range(self.step):
                vt[:, i + 1] = (
                    vt[:, i]
                    + self.kappa * (self.theta - np.max(vt[:, i], 0)) * self.delta
            ...
            for i in range(self.step):
                vt[:, i + 1] = (
                    vt[:, i]
                    + self.kappa * self.theta * self.delta
                    + self.vov * np.sqrt(np.max(vt[:, i], 0) * self.delta) * z[:, i]
                    + self.vov ** 2 * 0.25 * (z[:, i] ** 2 - 1) * self.delta
                ) / (1 + self.kappa * self.delta)
            below_0 = np.where(vt < 0)
            vt[below_0] = 0
        ...
        spot_cmc = spot * np.exp(
            self.rho
            * (vt[:, -1] - vt[:, 0] - self.kappa * (self.theta * texp - vt_int))
            / self.vov
            - self.rho ** 2 * vt_int / 2
        )
...
class HestonMcAe:
    ...
    def ch_f(self, texp, sigma_0, sigma_t, chi_dim):
        ...
        ch_f_part_1 = (
            lambda a: gamma_f(a)
            * np.exp(-0.5 * (temp_f(a) - self.kappa * texp))
            * (1 - np.exp(-self.kappa * texp))
            / (self.kappa * (1 - np.exp(-temp_f(a))))
        )
    ...
    def gen_vov_t(self, chi_dim, chi_lambda, texp, n_paths):
        ...
        sigma_t = np.sqrt(
            cof * np.random.noncentral_chisquare(chi_dim, chi_lambda, n_paths)
        )
        return sigma_t
    ...
    def gen_s_t(self, spot, sigma_t, sigma_0, texp, integral_sigma_t, n_paths):
        ...
        integral_sqrt_sigma_t = (
            sigma_t ** 2
            - sigma_0 ** 2
            - self.kappa * self.theta * texp
            + self.kappa * integral_sigma_t
        ) / self.vov
        mean = np.log(spot) + (
            self.r * texp - 0.5 * integral_sigma_t + self.rho * integral_sqrt_sigma_t
        )
    ...
    def price(
        ...
        chi_lambda = (
            (4 * self.kappa * np.exp(-self.kappa * texp))
            / ((self.vov ** 2) * (1 - np.exp(-self.kappa * texp)))
            * sigma_0 ** 2
        )
