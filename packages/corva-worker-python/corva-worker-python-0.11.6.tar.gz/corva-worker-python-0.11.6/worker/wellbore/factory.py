...
def run_casingstring_and_create_wellbore(wits: dict, mud_flow_in: float = None, is_design: bool = False) -> Union[Wellbore, None]:
    """
    Get the wits record and create a wellbore
    :param wits: wits records in json format
    :param mud_flow_in: used to determine if under-reamer needs to be activated for hole enlargement
    ...
    if not casing_components:
        # If it is a casing that reaches to the surface, we can use the provided information on the top to construct a casing string.
        casing_components = [
            {
        ...
        # From the surface to the liner hanger, we create the same component with the family of DP (it is not the actual casing part and will be removed later).
    ...
    drill_sections = api_worker.get(
        path="/v1/data/corva",
        collection=drilling_data_source,
        asset_id=asset_id,
        sort=sort,
        limit=100
    ).data
    ...
    if is_design:
        hole = create_hole_using_well_sections_data(
            previous_casings,
            drill_sections,
            casing_bottom_depth
        )
    ...
    # use data.drillstring to create hole
        ...
        hole = create_hole_use_drillstring_data(
            previous_casings,
            drill_sections,
...
def create_hole_use_drillstring_data(
    ...
    """
    drillstrings = [
        string for string in drillstrings
...
def create_hole_using_well_sections_data(
        previous_casings: List[dict],
        drill_sections: List[dict],
        casing_bottom_depth: float
) -> Union[Hole, None]:
    """
    ...
    for section in drill_sections:
        ...
        if previous_casing_bottom_depth <= open_hole_top_depth <= open_hole_bottom_depth <= casing_bottom_depth + Wellbore.DEPTH_THRESHOLD:
