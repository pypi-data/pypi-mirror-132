...
class ExtendedMetaDataMatchQuery(Query):
    ...
    def match(self, item):
        emd = item[ExtendedMetaDataMatchQuery.input_field]
        meta_data = ExtendedMetaData.decode(emd)

        if meta_data is None:
            return False

        query = create_query(self.pattern)
...
class OneOfQuery(EmdQuery):
    CONTAINS_MATCHER = ':'
    _query_pattern = f'^({EmdQuery.TAG_PATTERN})({CONTAINS_MATCHER}((!?){EmdQuery.VALUE_PATTERN}(,(!?){EmdQuery.VALUE_PATTERN})*)*)?$'
...
class RegexQuery(EmdQuery):
    REGEX_MATCHER = '::'
    _query_pattern = f'^({EmdQuery.TAG_PATTERN}){REGEX_MATCHER}(.*)$'
