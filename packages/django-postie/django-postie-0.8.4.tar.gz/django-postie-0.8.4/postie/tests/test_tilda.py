...
from postie.integrations.tilda import signals
...
class TildaServicesTestCase(TestCase):
    ...
    def test_update_template_tilda_data(self, get_page_full_mock):
        ...
        self.assertEqual(services.get_template_by_page_id(1).tilda_html, 'HTML')
        ...
        self.assertEqual(services.get_template_by_page_id(2).tilda_html, 'IT HTML')
        self.assertEqual(services.get_template_by_page_id(1).tilda_html, 'HTML')
...
class TildaSignalsTestCase(TestCase):
    def test_hook_signal(self):
        data = {}

        def webhook_listener(sender, request, credentials, **kwargs):
            data['request'] = request
            data['credentials'] = credentials

        # connect & send the signal
        signals.tilda_webhook_received.connect(webhook_listener)
        self.client.get(
            reverse('webhook_receiver'),
            {"publickey": "public", "projectid": 1, 'pageid': 1}
        )
    ...
    @mock.patch('tilda_wrapper_api.client.Client.get_page_full', return_value=MockPage(result=MockPageResult('HTML')))
    def test_fetch_page_signal(self, get_page_full_mock):
        preferences = G(
            Preferences,
            public_key="public",
            private_key="private_key",
        ...
        def fetch_listener(sender, project_id, page_id, template, **kwargs):
            data['project_id'] = project_id
            data['page_id'] = page_id
            data['template'] = template

        # connect & send the signal
        signals.tilda_page_fetched.connect(fetch_listener)

        # check results
        services.update_page(1, 1)
