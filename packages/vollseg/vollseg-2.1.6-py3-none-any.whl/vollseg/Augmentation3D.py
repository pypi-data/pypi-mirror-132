...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  1 10:50:38 2020
...
class Augmentation3D(object):
    ...
    def build(self,
        ...
        label : Integer label images
            The shape of this labels should match the shape of data (batch, x, y, z).
        batch_size: int
        ...
        if self.flip_axis is not None:
            ...
            if self.flip_axis in (1, 2, 3):
                parse_dict['flip_axis'] = self.flip_axis
            elif self.flip_axis == 'random':
                parse_dict['flip_axis'] = random.randint(1, 3)
            else:
                raise ValueError('Flip axis should be 1, 2, 3 or random')

        # shift
        if (self.shift_axis is not None) and (self.shift_range is not None):
            callback = self._shift_data
            if self.shift_axis in (1, 2, 3):
                parse_dict['shift_axis'] = self.shift_axis
            elif self.shift_axis == 'random':
                parse_dict['shift_axis'] = random.randint(1, 3)
            else:
                raise ValueError('Shift axis should be 1, 2, 3 or random')
        ...
        if(self.zoom_axis is not None) and (self.zoom_range is not None):
            ...
            if self.zoom_axis in (1, 2, 3):
                parse_dict['zoom_axis'] = self.zoom_axis
            elif self.zoom_axis == 'random':
                parse_dict['zoom_axis'] = random.randint(1, 3)
            elif self.zoom_axis == 'same':
                parse_dict['zoom_axis'] = None
            else:
                raise ValueError('Zoom axis should be 1, 2, 3 or random')
