...
class Client(Callbacks, SocketHandler):
    ...
    def register(self, nickname: str, email: str, password: str, verificationCode: str, deviceId: str = device.device_id):
        ...
        **Returns**
        ...
        response = requests.post(f"{self.api}/g/s/auth/register", data=data, headers=headers.Headers(deviceId=deviceId).headers)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
    ...
    def request_verify_code(self, email: str, resetPassword: bool = False,dev:str):
        """
        ...
        **Returns**
        ...
        data = json.dumps(data)
        response = requests.post(f"{self.api}/g/s/auth/request-security-validation", headers=headers.Headers(deviceId=dev).headers)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
