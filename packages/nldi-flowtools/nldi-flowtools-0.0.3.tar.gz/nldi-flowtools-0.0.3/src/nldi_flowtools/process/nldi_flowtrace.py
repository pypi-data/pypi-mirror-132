...
PROCESS_METADATA = {
    ...
    "outputs": [
        ...
        },
        {
            ...
            'to the nearest NHD flowline. This line will only be returned if "raindropTrace" '
            "is set to True and the input point does not fall on an NHD flowline.",
            "output": {"formats": [{"mimeType": "application/geo+json"}]},
        },
    ],
    "example": {
...
class NLDIFlowtraceProcessor(BaseProcessor):
    ...
    def __repr__(self) -> str:  # noqa D105
        return "<NLDIFlowtraceProcessor> {}".format(
            self.nldi - flowtrace - response  # noqa F821
        )
