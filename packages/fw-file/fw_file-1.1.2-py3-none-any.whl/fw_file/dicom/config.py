...
def get_config() -> "DICOMConfig":
    """Return DICOMConfig object loaded from envvars (cached)."""
    return DICOMConfig()
...
class DICOMConfig(BaseSettings):
    ...
    fix_invalid_float: bool = True

    # num of instances (with local paths) to keep loaded in memory in collections
    instance_cache_size = 10000

    def __init__(self, *args, **kwargs) -> None:
        """Init config and configure pydicom."""
        super().__init__(*args, **kwargs)
        self.configure_pydicom()

    def __setattr__(self, name, value) -> None:
        """Set attribute and configure pydicom."""
    ...
    def configure_pydicom(self):
        ...
        if self.fix_invalid_float:
            self.add_invalid_float_callback()
    ...
    @staticmethod
    def add_invalid_float_callback():  # pylint: disable=invalid-name
        r"""Add data element callback to fix ``\`` in VM=1 string like data element."""
        pydicom_config.data_element_callback = functools.partial(
            invalid_float_callback,
            data_element_callback=pydicom_config.data_element_callback,
