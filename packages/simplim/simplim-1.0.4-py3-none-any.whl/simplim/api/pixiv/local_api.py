...
import json,sim,math,os
from bs4 import BeautifulSoup as bs
from sim.premium import *
...
def getIllustUrls(pid):
	...
	if not info:
		return
	return info.illust.urls, info.manga_urls
...
def getIllust(pid, path='', use_curl=True, setting='original'):
	...
	if manga_urls:
		...
		for manga_url in manga_urls:
			ret = sim.multiThreadDownload(url=manga_url[setting].replace('i.pximg.net',pximg_host),headers=pixiv.headers,path=os.path.join(path,str(cnt)+'.jpg'),verify=False)
			if not ret:
				# d=sim.download(url=manga_url[setting],headers=pixiv.headers,need_content_length)
		...
		if not path:
			path = './'
		ret = sim.multiThreadDownload(url=urls[setting].replace('i.pximg.net',pximg_host),headers=pixiv.headers,path=os.path.join(path,title+'.jpg'),verify=False)
		if not ret:
...
def getIllustByUrl(url,path='',use_curl=False):
	# pixiv.getImg(url = url.replace('i.pximg.net',pximg_host),name=sim.gettemp(),path=path,useCurl=use_curl)
	open(os.path.join(path,sim.gettemp()+'.jpg'),'wb').write(requests.get(url.replace('i.pximg.net',pximg_host), headers=headers, verify=False).content)
...
def getUserLikedWorks(pid,use_curl = False):
	...
	if pages>1:
		...
		final.dump('liked_works.'+str(pid)+'.'+sim.gettemp()+'.json')
		return final
		# params['limit']=str(total)
		# response = requests.get('https://'+host+'/ajax/user/'+str(pid)+'/illusts/bookmarks', params = params, headers = headers, verify=False)
		# sd = simdict(response.json())
		# sd.dump(str(pid)+'.'+sim.gettemp()+'.json')
	sd.body.works.dump(str(pid)+'.'+sim.gettemp()+'.json')
	return sd
