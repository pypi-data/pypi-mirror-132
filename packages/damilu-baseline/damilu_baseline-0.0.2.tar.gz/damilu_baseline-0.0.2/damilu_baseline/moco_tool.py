...
class MoCo(nn.Module):
    ...
    def __call__(self, im_q, im_k):
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            target_1linear = torch.squeeze(activation['online_network.1linear'])
            target_gap = torch.squeeze(activation['online_network.0projector'])
            if self.infoNCE == 0:
                k = nn.functional.normalize(target_gap, dim=1)
            elif self.infoNCE == 1:
                k = nn.functional.normalize(target_1linear, dim=1)
            elif self.infoNCE == 2:
