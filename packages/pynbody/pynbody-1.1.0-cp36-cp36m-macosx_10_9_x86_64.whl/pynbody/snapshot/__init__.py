...
class SimSnap(ContainerWithPhysicalUnitsOption):
    ...
    *Getting metadata*
    ...
    def _array_name_1D_to_ND(self, name):
        ...
        for k, v in list(self._split_arrays.items()):
            if name in v:
                return k
    ...
    def _array_name_implies_ND_slice(self, array_name):
        ...
        """
        for v in list(self._split_arrays.values()):
            if array_name in v:
              return True
    ...
    def __delitem__(self, name):
        if name in self._family_arrays:
            ...
            for v in list(self._family_derived_array_names.values()):
                if name in v:
                    del v[v.index(name)]
    ...
    def _clear_immediate_mode(self):
        for k, v in list(self._persistent_objects.items()):
    ...
    def __load_array_and_perform_postprocessing(self, array_name, fam=None):
        ...
        with self.delay_promotion:
            ...
            for f, vals in list(new_fam_keys.items()):
                for v in vals:
    ...
    def _dirty(self, name):
        ...
        if name=='pos':
            for v in list(self.ancestor._persistent_objects.values()):
                if 'kdtree' in v:
                    del v['kdtree']
