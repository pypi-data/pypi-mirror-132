...
for hdf_groups in list(_default_type_map.values()):
...
class GadgetHDFSnap(SimSnap):
    ...
    def __init_loadable_keys(self):
        ...
        for fam_keys in list(self._loadable_family_keys.values()):
    ...
    def __init_family_map(self):
        type_map = {}
        for fam, g_types in list(_default_type_map.items()):
            my_types = []
            for x in g_types:
    ...
    def __init_unit_information(self):
        try:
            ...
            if 'UnitVelocity_in_cm_per_s' not in list(atr.keys()):
...
def do_properties(sim):
    ...
    for s,value in list(sim._get_hdf_header_attrs().items()):
