...
from db_contrib_tool.utils.oauth import Configs, get_oauth_credentials
        ...
        if not path:
            ...
            try:
                response = self.http_client.get(
                    f"{self.host}/find_by_id", params={"build_id": build_id}
                )
...
class Symbolizer(Subcommand):
    ...
    def execute(self):
        resolver = PathResolver(host=self.host, cache_dir=self.cache_dir)
    ...
    def symbolize_frames(
        ...
        if frames is None:
            frames = self.preprocess_frames(dbg_path_resolver, trace_doc, input_format)
    ...
    def preprocess_frames(self, dbg_path_resolver, trace_doc, input_format):
