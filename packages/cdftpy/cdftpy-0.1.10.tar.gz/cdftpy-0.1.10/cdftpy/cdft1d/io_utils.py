...
def write_matrix_array(filename, grid, matrix, header=None):

    ngrid = matrix.shape[-1]
    nv = matrix.shape[0]
    ...
    with open(filename,"w") as fp:
        ...
        for k in range(ngrid):
            ...
            for i in range(nv):
                for j in range(i, nv):
                    fp.write(F"{matrix[i, j, k]}  ")
            fp.write("\n")

def write_array(filename, grid, matrix, header=None):
    ...
    with open(filename,"w") as fp:
        ...
        for k in range(ngrid):
            fp.write(F"{grid[k]}   ")
            for i in range(nv):
                    fp.write(F"{matrix[i, k]}  ")
            fp.write("\n")

def write_array1(filename, grid, matrix, header=None):
