    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) > 0:
            ...
            if re.search('str', s):
                ...
                if re.search(gpsdatetime._re_rnx3, args[0]):
                    self.rinex_t(args[0])
                    return
...
#            print("Kwargs  >  >  >  >  ", k, ' = ', v)
	...
	"""
        ...
        Tloc = tm.localtime()
        self.gmtoff = Tloc.tm_gmtoff
        self.zone = Tloc.tm_zone
...
#        s1970 = decimal.Decimal(tm.mktime(t))
    ...
    def timestamp_t(self, timestamp_str):
        """Calculate all time elements from timestamp time string
        
        ex : "14/Dec/2021:07:37:10 +0100" 
        
        Input :
          timestamp_str : timestamp time string
        ...
        T = re.split(regexPattern, timestamp_str)
        for i in range(len(gpsdatetime._MON)):
            if re.search(T[1], gpsdatetime._MON[i], re.IGNORECASE):
                T[1] = i + 1
                break
        ...
        self.ymdhms_t(T[2], T[1], T[0], T[3], T[4], T[5])
        self += 3600 * T[6]
...
#        print(self)
    ...
    def st_rinex_epoch(self):
        """ return rinex time string
        """
        return "%4d %02d %02d %02d %02d %10.7f" \
                % (self.yyyy, self.mon, self.dd, self.hh, self.min, self.sec)
    ...
    def st_timestamp_epoch(self, ndecimal=0):
        """ return timestamp
        Is is assumed that gpsdatetime object is in Z zone
        """
        Mon = gpsdatetime._MON[self.mon-1].capitalize()
        
        if ndecimal > 0:
            format_str = "%02.0f/%3s/%4.0f:%02.0f:%02.0f:%"\
                        + ("%02d.%1d" % (ndecimal+3, ndecimal)) + 'f +0000'
        else:
            format_str = "%02.0f/%3s/%4.0f:%02.0f:%02.0f:%02.0f +0000"
            
        return format_str % (self.dd, Mon, self.yyyy, self.hh, self.min, self.sec)
...
#    test()
    ...
    time_3 = gpsdatetime('18 10  9 14 20 00.00000')
    print(time_3)
    time_3 = gpsdatetime('2018 10  9 14 20 00.00000')
    print('>>>>', time_3)

    print(time_1.st_iso_epoch())
    print(time_2.st_iso_epoch())
    
    print(time_3.st_rinex_epoch())
    
    ts = "14/Dec/2021:07:37:10.5 +0100"
    
    time_5 = gpsdatetime(timestamp=ts)
    print(time_5.st_timestamp_epoch(4))
