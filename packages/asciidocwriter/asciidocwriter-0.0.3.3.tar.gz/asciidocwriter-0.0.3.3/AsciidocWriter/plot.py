...
class DocumentPlot():
    ...
    def __init__(self, name, df, title=None):
        ...
        self.path = f"{name}.jpeg"

        if df.empty:
            self.df = df

    def add_to(self, section):
        self.fig.savefig(self.path)
        section.add_paragraph(f".{self.title}\n")
        section.add_paragraph(f"image::{self.name}[{self.title}]\n\n")
...
class ScatterPlot(DocumentPlot):
    def __init__(self, name, df, title=None):

        DocumentPlot.__init__(self, name, df, title=None)

        self.kind = PlotType.scatter
...
class ProphetPlot(DocumentPlot):
    def __init__(self, name, df, title=None):
