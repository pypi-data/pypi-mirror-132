from textattack.transformations.word_swaps import word_swap
import random

class TrojWordSwapQWERTY(word_swap.WordSwap):
    def __init__(
        ...
        too quickly. Modified from the textattack library.
    ...
    def _get_replacement_words(self, word):
        ...
        if self.random_one:
            try:
                ...
                candidate_word = (word[:i] + random.choice(self._get_adjacent(word[i])) + word[i + 1 :])
...
class PronounSwap(word_swap.WordSwap):
    '''
    Swaps pronouns
    '''
    def _get_replacement_words(self, word):
        word  = word.lower()
        masc_fem = {'him':'her', 'he':'she', 'his':'hers'}
        fem_masc = {'her':'him', 'she':'he', 'hers':'his'}
        swap_dict = {**masc_fem, **fem_masc}
        if word in list(swap_dict.keys()):
            word = swap_dict[word]
