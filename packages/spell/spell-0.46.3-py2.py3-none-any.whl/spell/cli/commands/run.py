    ...
    **kwargs,
    ...
    if kubernetes:
        for unsupported_field in [
            ...
            field_var_name = unsupported_field.replace("-", "_")
            if locals().get(field_var_name) or kwargs.get(field_var_name):
                raise ExitException(f"--{unsupported_field} is not supported for --kubernetes runs")
