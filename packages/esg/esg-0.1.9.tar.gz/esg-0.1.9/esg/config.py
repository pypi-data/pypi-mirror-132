...
try:
    ...
    # Install the PyYAML package or the esg[standard] optional dependencies to
    # enable this functionality.
    pass

from esg.importer import ImportFromStringError, import_from_string
from esg.middleware.asgi2 import ASGI2Middleware
from esg.middleware.debug import DebugMiddleware
from esg.middleware.message_logger import MessageLoggerMiddleware
...
HTTP_PROTOCOLS: Dict[HTTPProtocolType, str] = {
    "auto": "esg.protocols.http.auto:AutoHTTPProtocol",
    "h11": "esg.protocols.http.h11_impl:H11Protocol",
    "httptools": "esg.protocols.http.httptools_impl:HttpToolsProtocol",
}
WS_PROTOCOLS: Dict[WSProtocolType, Optional[str]] = {
    "auto": "esg.protocols.websockets.auto:AutoWebSocketsProtocol",
    "none": None,
    "websockets": "esg.protocols.websockets.websockets_impl:WebSocketProtocol",
    "wsproto": "esg.protocols.websockets.wsproto_impl:WSProtocol",
}
LIFESPAN: Dict[LifespanType, str] = {
    "auto": "esg.lifespan.on:LifespanOn",
    "on": "esg.lifespan.on:LifespanOn",
    "off": "esg.lifespan.off:LifespanOff",
}
LOOP_SETUPS: Dict[LoopSetupType, Optional[str]] = {
    "none": None,
    "auto": "esg.loops.auto:auto_loop_setup",
    "asyncio": "esg.loops.asyncio:asyncio_setup",
    "uvloop": "esg.loops.uvloop:uvloop_setup",
}
...
LOGGING_CONFIG: dict = {
    ...
    "formatters": {
        "default": {
            "()": "esg.logging.DefaultFormatter",
            "fmt": "%(levelprefix)s %(message)s",
            "use_colors": None,
        },
        "access": {
            "()": "esg.logging.AccessFormatter",
    ...
    "loggers": {
        "esg": {"handlers": ["default"], "level": "INFO"},
        "esg.error": {"level": "INFO"},
        "esg.access": {"handlers": ["access"], "level": "INFO", "propagate": False},
    },
}
...
class Config:
    ...
    def configure_logging(self) -> None:
        ...
        if self.log_level is not None:
            ...
            logging.getLogger("esg.error").setLevel(log_level)
            logging.getLogger("esg.access").setLevel(log_level)
            logging.getLogger("esg.asgi").setLevel(log_level)
        if self.access_log is False:
            logging.getLogger("esg.access").handlers = []
            logging.getLogger("esg.access").propagate = False
    ...
    def load(self) -> None:
        ...
        self.encoded_headers = (
            [(b"server", b"esg")] + encoded_headers
    ...
    def bind_socket(self) -> socket.socket:
        ...
        if self.uds:  # pragma: py-win32
            ...
            message = "ESG running on unix socket %s (Press CTRL+C to quit)"
            sock_name_format = "%s"
            color_message = (
                "ESG running on "
            ...
            message = "ESG running on socket %s (Press CTRL+C to quit)"
            fd_name_format = "%s"
            color_message = (
                "ESG running on "
