...
class ServerState:
    ...
    __slots__ = ["total_requests", "connections", "tasks", "default_headers", "profile"]
    ...
    def measure(self, name):
        ...
        if name not in self.profile:
            self.profile[name] = [1, stop - start]
        else:
            self.profile[name][0] += 1
            self.profile[name][1] += stop - start
    ...
    def stats(self):
        print("================", flush=True)
        records = []
        for k, (cnt, t) in self.profile.items():
            records.append([k, cnt, t])

        for k, cnt, t in sorted(records, key=lambda k: k[2]):
            print(cnt, "\t", t, "\t", k, flush=True)
