                    ...
                    if not response:
                        ...
                        if event == "PlayerMessage":
                            ...
                            if message.startswith(self.tests_prefix):
                                ...
                                test_name, test_arguments = get_command_args(message)
                                ...
                                if not self.authorized(author):
                                    ...
                                    try:
                                        if self.tests_active:
                                            ...
                                            logger.debug(f"{convert_type(test, *test_arguments)}")
                                        ...
                                        if len(command_arguments) > len(param_names):
                                            ...
                                            for param in params:
                                                if param.name != "ctx":
                                                    ...
                                                    logger.debug(f"{param.name} is a required argument")
                                                    if required:
                                                        if param.name not in given:
                                                            logger.debug(f"{param.name} is not in {given}")
                                                            missing.append(param.name)
                                            if not empty(missing):
                                                logger.debug("missing args")
                                                e = self._event_prefix + "missing_arguments"
                                                if e in self.event_handlers:
