...
class Badminton():
    ...
    def __init__(self, configpath, begin_time='00:01', max_times=10, token=''):
        ...
        self.begin_hour = time(int(begin_time.split(':')[0]), int(begin_time.split(':')[1]))
        self.token = token
    ...
    def recognize(self):
        ...
        while recg_times < self.max_times:
            r = requests.post(f'http://api.95man.com:8888/api/Http/Recog?Taken={self.token}&imgtype=1&len=4',
            ...
            if int(arrstr[0]) > 0:
                ...
                return True
            recg_times += 1
            print(f'识别失败: {arrstr[1]}，第 {recg_times} 次重试')
            sleep(1)
            continue
    ...
    def waite(self):
        ...
        # 判断预订系统是否开放
        today = date.today()
        begin_time = datetime.combine(today, self.begin_hour)
        ...
        if self.stock_date == today:
            ...
            if stock == -1:
                ...
                sys.exit()
            if stock == 0:
                begin_time = datetime.combine(self.stock_date, self.begin_hour)
            ...
            if stock == -1:
                ...
                begin_time = datetime.combine(
                    self.stock_date - timedelta(days=1), self.begin_hour)
...
def main():
    parser = argparse.ArgumentParser(description='预定中山大学珠海校区羽毛球场地')
    parser.add_argument('-c', '--config', type=str, help='配置文件路径', required=True)
    parser.add_argument('-b', '--begin', type=str, help='预定开始时间', default='00:01')
    parser.add_argument('-m', '--maxtimes', type=int, help='最大预定次数', default=10)
    parser.add_argument('-t', '--token', type=str,
                        help='验证码识别平台token', default='Hvfn8AbwxNTySxrnmTknA06onCdb')
