...
def print_runs_and_jobs(profile, args):
    ...
    for run_name, run_jobs in sorted_jobs_by_run_name:
        ...
        table_rows.append([
            ...
            colored(run_status, "*" if run["tag_name"] == run["run_name"] else run["tag_name"] if run["tag_name"] else "<none>", not args.no_jobs),
            "",
...
def get_runner_name(run_or_job):
    if run_or_job["runner_name"]:
        if run_or_job["runner_user_name"] and run_or_job["runner_user_name"] != run_or_job["user_name"]:
...
def print_runners(profile):
    ...
    for runner in runners:
        ...
        table_rows.append(
            [runner["runner_name"], runner.get("host_name") or "<none>", runner["status"].upper(), updated_at_str])
    print(tabulate(table_rows, headers=table_headers, tablefmt="plain"))
