...
class Connection:
    ...
    def _post_attachments(
        ...
        response = requests.post(url=self.api_base_url + path, headers=self._request_headers, files=files)
    ...
    def _update_attachments(
        ...
        if self._request_headers.get("Content-Type"):
            self._request_headers.pop("Content-Type")
        response = requests.put(url=self.api_base_url + path, headers=self._request_headers, files=files)
