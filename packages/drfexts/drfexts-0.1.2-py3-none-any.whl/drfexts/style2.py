...
class Filter(BaseModel):
    ...
    data: Any  # 自定义渲染的数据值（当使用自定义模板时可能会用到）
...
class CellRender(BaseModel):
    ...
    optionProps: Any  # 只对 name=select 有效，下拉选项属性参数配置
    optionGroups: List[Any]  # 只对 name=select 有效，下拉分组选项列表
    optionGroupProps: Any  # 只对 name=select 有效，下拉分组选项属性参数配置
    events: Any
    content: str
...
class VueColumn(BaseModel):
    type: TypeEnum  # 列的类型
    field: str  # 列字段名（注：属性层级越深，渲染性能就越差，例如：aa.bb.cc.dd.ee）
    ...
    filters: List[Filter]  # 配置筛选条件（注：筛选只能用于列表，如果是树结构则过滤根节点）
    ...
    cellRender: CellRender  # 默认的渲染器配置项
    editRender: EditRender  # 可编辑渲染器配置项
    contentRender: ContentRender  # 内容渲染器配置项
