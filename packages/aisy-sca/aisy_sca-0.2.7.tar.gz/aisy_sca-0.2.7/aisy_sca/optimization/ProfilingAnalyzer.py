...
class ProfilingAnalyzer(HyperparameterSearch):
    """
    Class that perform random or grid hyper-parameter search with the analysis of different profiling set sizes.
    ...
    def run(self, hp_combinations_reproducible, da_function=None, grid_search=False):
        ...
        self.set_metrics()
        for step_index, n_profiling_traces in enumerate(self.settings["profiling_analyzer_steps"]):
            hp_ids[f"{n_profiling_traces}"] = []
        ...
        for search_index in range(self.random_search_definitions["max_trials"]):
            ...
            if self.random_states is None:
                hp_values = search_hp_combinations
            else:
                hp_values = search_hp_combinations[search_index]
            model, hp = self.generate_model(hp_values)

            """ Make sure we have the same weight initialization for all different profiling set sizes """
            ...
            self.custom_callbacks[f"{search_index}"] = {}
            ...
            for step_index, n_profiling_traces in enumerate(self.settings["profiling_analyzer_steps"]):
                ...
                print(f"=====================================================")
                print(f"Start profiling phase for {n_profiling_traces} traces")
                print(f"=====================================================")

                history = profiling.train_model(model, n_profiling_traces=n_profiling_traces)
                self.custom_callbacks[f"{search_index}"][f"{n_profiling_traces}"] = profiling.get_custom_callbacks()
                ...
                hp_ids[f"{n_profiling_traces}"].append(hp_id)

                """ Retrieve metrics from model and save them to database """
                self.loss_search_pa[f"{n_profiling_traces}"].append(history.history["loss"])
                self.acc_search_pa[f"{n_profiling_traces}"].append(history.history["accuracy"])
                ...
                self.compute_metrics(model, search_index, self.analysis_db_controls, profiling.get_builtin_callbacks(), hp_id,
                                     n_profiling_traces=n_profiling_traces, steps=self.settings["profiling_analyzer_steps"])
                ge = self.ge_search_pa[f"{n_profiling_traces}"][search_index]
                sr = self.sr_search_pa[f"{n_profiling_traces}"][search_index]
                ...
                self.save_generic_metrics(profiling, history, search_index, hp_id, n_profiling_traces=n_profiling_traces)
                ...
                """ Find best model"""
                self.get_best_model(n_profiling_traces=n_profiling_traces)

                """ Save best model so far in h5 """
                self.save_best_model(model, search_index, n_profiling_traces=n_profiling_traces)

                if search_index == self.random_search_definitions["max_trials"] - 1 and step_index == len(
                        self.settings["profiling_analyzer_steps"]) - 1:
                    self.finalize_search(profiling, model_descriptions, hp_ids, steps=self.settings["profiling_analyzer_steps"])
