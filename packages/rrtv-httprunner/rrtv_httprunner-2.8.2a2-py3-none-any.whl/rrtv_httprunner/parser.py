...
from rrtv_httprunner.utils import execute_sql, execute_cmd, get_statement_type, execute_redis, execute_mongo, \
    remove_bracket_first, legitimate_method_call, execute_es
            ...
            if suffix_re:
                ...
                if var_value is None:
                    if (suffix is not None or suffix2 is not None):
                        replaceValue1 = var_name_whole + "['" + suffix_re[0] + "']"
                        replaceValue2 = var_name_whole + "[\"" + suffix_re[0] + "\"]"
                        return raw_string.replace(replaceValue1, "None").replace(replaceValue2, "None").replace(
                            var_name_whole, "None")
                    else:
                        return var_value
