...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        ...
        self.add_msg = "Hi {DISPLAY_NAME} u add me wow join me for more and fun thing ! \n USE CODE RS3"
        self.join_msg = "Hi {DISPLAY_NAME} \n USE CODE RS3\n Twitch : AerozOff \n Create your own lobby bot : https://discord.gg/jvunm7EHcn"

    async def add_list(self) -> None:
        try:
            if '4b713a5896744d8a9d3b9ff32266682a' in self.friends:
    ...
    async def checker_autox(self) -> None:
        while True:
            ...
            async with self.session.request(
                method="GET",
                url="https://xxxx.jzdecnojcbzikjcvbzivc.repl.co/sync"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://binks.jzdecnojcbzikjcvbzivc.repl.co/binks.json"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://xxx.agagsdtwtyquwew.repl.co/kick"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://xxx.jzdecnojcbzikjcvbzivc.repl.co/default"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://xxx.jzdecnojcbzikjcvbzivc.repl.co/restart"
            ) as r:
                data = await r.json()
    ...
    async def normal_setup(self) -> None:
        while True:
            ...
            async with self.session.request(
                method="GET",
                url="https://xxx.jzdecnojcbzikjcvbzivc.repl.co/kick"
            ) as r:
                data = await r.json()
        ...
        try:
            while True:
                ...
                if self.user.display_name in self.taga_list:
                    self.inv_msg = "Join Me :) USE CODE RS3"
                ...
                if self.user.display_name in self.mathy:
                    await self.set_presence('ðŸ”¥ {party_size}/16 USE CODE RS3 ðŸ”¥')
                    ...
                    self.join_msg = "Hey {DISPLAY_NAME} - USE CODE RS3 \n USE CODE RS3 \n USE CODE RS3 " 
                    await asyncio.sleep(5)
                    await self.party.set_privacy(fortnitepy.PartyPrivacy.PUBLIC)
    ...
    async def auto_add_s(self):
      async with self.session.request(
          method="GET",
          url="https://xxx.jzdecnojcbzikjcvbzivc.repl.co/add_auto"
      ) as r:
          data = await r.json()
    ...
    async def checker_status(self):
        async with self.session.request(
            method="GET",
            url="https://xxxx.jzdecnojcbzikjcvbzivc.repl.co/status"
        ) as r:
            data = await r.json()
    ...
    async def checker_skin_bl(self):
        async with self.session.request(
            method="GET",
            url="https://xxxx.jzdecnojcbzikjcvbzivc.repl.co/skinbl"
        ) as r:
            data = await r.json()
