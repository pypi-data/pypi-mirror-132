    ...
    for item in result:
        if item.get(key, None)\
            ...
            for match in matches:
                ...
                try:
                    collection.update_one({'_id': item['_id']}, {'$set': {key: {'value': float(amount), 'currency': currency}}})
                except ValueError:
                    continue
...
class DBWriter():
    ...
    def __init__(self, connection_str: str, host: str, port: str, user: str, passwrd: str, auth_db: str, db: str, collection: str) -> None:
        if connection_str:
            self.__mongo__ = MongoClient(connection_str)            
        else:
            self.__mongo__ = MongoClient(host=host,username=user, 
                password=passwrd, port=port)
    ...
    def process_dates(self):
        ...
        for item in result:
            ...
            if item.get('released', None)\
                ...
                self.__collection__.update_one(
                    {'_id': item['_id']},
                    {'$set': {'released': int(item['released'])}}
                )
