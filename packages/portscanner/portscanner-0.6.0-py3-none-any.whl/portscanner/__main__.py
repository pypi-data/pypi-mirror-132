...
from portscanner.parsing import port_range
from portscanner.colors import Clr
...
def style(info: ScanInfo, as_json: bool = False, first: bool = False):
    if not as_json:
        ...
        print(("" if first else ",\n") + json.dumps(data, indent=4), end="", flush=True)
...
def run():
    ap = ArgumentParser("portscanner", conflict_handler="resolve")

    ap.add_argument(
        "targets",
        type=str,
        nargs="+",
        help="Target hostnames, IP Addresses, or CIDRs to perform a port scan"
    )
    ap.add_argument(
        "-p",
        "--ports",
        type=port_range,
        default=TOP_PORTS,
        help="Target TCP Ports to scan on each hostname, IP, or CIDR",
    )
    ap.add_argument(
        "-t",
        "--timeout",
        default=1.0,
        type=float,
        help="Connection timeout for port scans and DNS resolution",
    )
    ap.add_argument(
        "-d",
        "--dns",
        ...
        type=str,
        nargs="+",
        help="DNS Servers to use for name resolution",
    )
    ap.add_argument(
        "-w",
        "--workers",
    ...
    ap.add_argument(
        ...
        default=None,
        type=int,
        help="Buffer size for reading the target service banner (likely slows scan)",
    )
    ap.add_argument(
        "--open",
        action="store_true",
        help="Show open ports only",
    )
    ap.add_argument(
        "-f",
        "--first",
        action="store_true",
        help="Only use first resolved IP",
    )
    ap.add_argument(
        "-j",
        "--json",
    ...
    ap.add_argument(
        ...
        "--ipv6",
        action="store_true",
        help="Use IPv6 name resolution",
    )
    ap.add_argument(
        "-4",
        "--ipv4",
        action="store_true",
        help="Use IPv4 name resolution",
    )
    ap.add_argument(
        "-v",
        "--verbose",
