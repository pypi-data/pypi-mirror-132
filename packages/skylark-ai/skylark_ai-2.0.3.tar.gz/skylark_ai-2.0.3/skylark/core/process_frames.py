...
class ProcessFrameClient:
    def __init__(self, batch_size, service, token):
        self.json = ""
        self.syncid = -1
    ...
    def createTasks(self):
        tasks = [asyncio.ensure_future(self.client.receive_message()),]
    ...
    def process(self, cv2_frame):
        # encode_params = [cv2.IMWRITE_JPEG_QUALITY, self.quality]
        byte_array = cv2.imencode('.jpg', cv2_frame)[1].tostring()
        self.get_or_create_eventloop().run_until_complete(self.client.start_stream(byte_array))
        # await self.client.start_stream(byte_array)
