...
class NightDayStreamClient(StreamClient):
	def __init__(self,id, fps, batch_size, sampling_rate, token, show_processed_stream=False,
	 save_raw_frames=False, scale_percent=100, quality=100, stream=0):
		...
		self.token = token
		print("real time hit")
		self.client = RealTimeClient(Service.NIGHT_DAY, self.on_receive, batch_size, self.on_network_status_change, self.token)
		print("hit done")
		self.response_jsons = []
	...
	def scale_coordinates(self, json):
		...
		if 'results' in json:
			...
			results = self.np_from_json(results)
			# cv2.imshow("TEsting", cv2.imdecode(results, 1))
			# cv2.waitKey(1000)
			print("result shape is")
			print(results.shape)
			json['results'] = cv2.imdecode(results, 1)
			print("SCALED")
			print(json)
				...
				try:
					...
					try:
						...
						for i in range(self.sample_length):
							...
							print(results.shape)
							# results = cv2.imdecode(results, 1)
							# results = results.astype(np.float32)
							cv2.imshow(str(self.id), results)
							cv2.waitKey(5)
