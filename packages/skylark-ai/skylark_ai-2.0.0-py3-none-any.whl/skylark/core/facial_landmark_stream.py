...
class FacialLandmarkStream(StreamClient):
	def __init__(self, fps, batch_size, sampling_rate, token):
		...
		self.token = token
		self.client = RealTimeClient(Service.FACIAL_LANDMARK, self.on_receive, batch_size,
					...
					for result in results:
						for i in range(self.sample_length):
							...
							for key in json_output:
								landmarks = json_output[key]
								for landmark_key in landmarks:
									landmark = landmarks[landmark_key]
									for coords in landmark:
										cv2.circle(frame, tuple(coords), 2, color, -1)
							cv2.imshow('outputstream', frame)
