...
> pip install tinyrpc or pip install --upgrade tinyrpc==1.1.0 (ensure support of Python3)
...
class PulseStreamer():
    ...
    def __init__(self, ip_hostname='pulsestreamer'):
        ...
        try:
            client = RPCClient(JSONRPCProtocol(), HttpPostClientTransport(url, timeout=10))
            self.proxy = client.get_proxy()
            try:
                self.proxy.getSerial()
                firmware_version=self.proxy.getFirmwareVersion()
            except Exception as e:
                if type(e).__name__ == 'UnexpectedIDError':
                    raise
                self.proxy.isRunning()
                assert False, "Pulse Streamer class not compatible with current firmware. Please update your firmware." \
                    "For detailed information visit https://www.swabianinstruments.com " \
                    "or contact support@swabianinstruments.com"
        except AssertionError:
            raise
        except Exception as e:
            if type(e).__name__ == 'UnexpectedIDError':
                assert False, "Python package tinyrpc is not compatible with current firmware. " \
                    "Please update your firmware or downgrade tinyrpc package to version <=1.1.0 " \
                    "For detailed information visit https://www.swabianinstruments.com " \
                    "or contact support@swabianinstruments.com"
            else:
        ...
        print("Client software: v" + __version__)
        if(_compare_version_number(firmware_version) >= 0):
            print("The Pulse Streamer 8/2 firmware is more up to date than your client software. We recommend updating your client software.")
            print("For detailed information visit https://www.swabianinstruments.com/pulse-streamer-8-2/downloads/ or contact support@swabianinstruments.com")
        elif(_compare_version_number('.'.join(__version__.split('.')[0:2]+['0']),'.'.join(firmware_version.split('.')[0:2]+['0'])) > 0):
            print("Your client software is more up to date than the Pulse Streamer 8/2 firmware. We recommend updating the firmware of your Pulse Streamer 8/2.")
            print("For detailed information visit https://www.swabianinstruments.com/pulse-streamer-8-2/downloads/ or contact support@swabianinstruments.com")
    ...
    def __stream_binary(self, seq, n_runs=REPEAT_INFINITELY, final=OutputState.ZERO()):
        ...
        if isinstance(seq, Sequence):
            try:
                s_enc = self.enc_binary(seq.getData(as_ndarray=True))
                pulses=len(s_enc)//9
            except TypeError:
    ...
    def enc_binary(self, seq):
        ...
        if isinstance(seq, np.ndarray):
            elements_per_pulse=9
            convert_list=seq.tobytes('A')
        elif type(seq[0][1])== list:
        ...
        assert len(convert_list)//elements_per_pulse<=2e6, "The resulting sequence length exceeds the limit of two million sequnence steps"
    ...
    def chans_to_mask(self, chans):
        ...
        for chan in chans:
            mask |= 1<<chan
        assert mask <= 255, "Pulse Streamer 8/2 supports "\
