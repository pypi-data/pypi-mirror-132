...
try:
    ...
    from pulsestreamer.grpc.pulse_streamer_pb2 import VoidMessage, PulseMessage, SequenceMessage, ClockMessage, DrpClkSetMessage, TriggerMessage, SetNetworkMessage, GetNetworkMessage, HostnameMessage, PulseStreamerStub, SetSquareWaveMessage, AnalogCalibrationData, SupplyStatusData
...
class PulseStreamer():
    ...
    def reboot(self):
        void = VoidMessage()
        return self.stub.reboot(void, timeout=PulseStreamer.TIMEOUT).value
    ...
    def getHardwareVersion(self):
        void = VoidMessage()
        return self.stub.getHardwareVersion(void, timeout=PulseStreamer.TIMEOUT).string_value
    ...
    def getSupplyState(self):
        void = VoidMessage()
        response = self.stub.getSupplyState(void, timeout=PulseStreamer.TIMEOUT)
        key_list=['Current/A', 'Voltage_12/V', 'Voltage_3.3/V']
        return dict( zip(key_list, [response.efuse_current, response.v12, response.v33] ))

    def setSquareWave125MHz(self, channels=[]):
        s=SetSquareWaveMessage()
        s.digi = self.chans_to_mask(channels)
        return self.stub.setSquareWave125MHz(s, timeout=PulseStreamer.TIMEOUT).value
    
    def setNetworkConfiguration(self, dhcp, ip='', netmask='', gateway='', testmode=True):
        assert dhcp in [True, False]
        ...
        n.gateway=gateway
        n.testmode=testmode
        return self.stub.setNetworkConfiguration(n, timeout=PulseStreamer.TIMEOUT).value

    def getNetworkConfiguration(self, permanent=False):
        assert permanent in [True, False]
        g=GetNetworkMessage()
        g.permanent=permanent
        response = self.stub.getNetworkConfiguration(g, timeout=PulseStreamer.TIMEOUT)
        key_list=['dhcp', 'ip', 'netmask', 'gateway']
        return dict( zip(key_list, [response.dhcp, response.ip, response.netmask, response.gateway]))

    def applyNetworkConfiguration(self):
        void = VoidMessage()
        return self.stub.applyNetworkConfiguration(void, timeout=PulseStreamer.TIMEOUT).value
