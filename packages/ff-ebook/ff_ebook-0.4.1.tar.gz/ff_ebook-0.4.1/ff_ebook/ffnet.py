...
class Munger:
    def __init__(
        ...
        self.last_request_time = datetime.now() - timedelta(seconds=RATE_LIMIT_DELAY_SEC)

        self._cleaner = ParagraphCleaner()

        if cookies:
            domain = urlparse(self.story_url).netloc
            for k, v in cookies.items():
                self.scraper.cookies.set(k, v, domain=domain)
    ...
    def DownloadChapter(self, chapter):
        ...
        for i in range(5): # Retry max 5 times.
            if datetime.now() - timedelta(seconds=RATE_LIMIT_DELAY_SEC) < self.last_request_time:
                time_diff = self.last_request_time - (datetime.now() - timedelta(seconds=RATE_LIMIT_DELAY_SEC))
                time.sleep(time_diff.total_seconds())
                continue
            self.last_request_time = datetime.now()
            response = self.scraper.get(url)
            soup = BeautifulSoup(response.text)
            is_captcha = bool(soup.find("meta", id="captcha-bypass"))
            if not is_captcha:
                return soup
            else:
                time.sleep(10)
...
def main():
    ...
    parser.add_argument(
        "--cookies",
        help="add cookies to the request",
        nargs="+",
        type=str,
    )
    parser.add_argument(
        "--user_agent",
        help="set useragent for the request",
        type=str,
    )
    ...
    cookies = {k: v for k, v in map(lambda kv: kv.split('='), args.cookies)} if args.cookies else None
    print(cookies)
    browser = dict(custom=args.user_agent) if args.user_agent else dict(browser='chrome')
