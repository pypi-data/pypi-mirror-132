...
def validate_configs(config: CLIContext, config_dir: str) -> None:
    ...
    spinner = Halo(text=f"Validating configs in '{config_dir}'...", spinner="dots")
    spinner.start()
    repo, branch, commit = config.mql.validate_configs(config_dir)
    spinner.succeed(f"🎉 Successfully validated configs for commit ({commit}) in repo ({repo}), on branch ({branch})")


@cli.command()
@config_validation_options
@click.option("--pin", type=bool, required=False, help="Whether to pin the model or not")
@click.option("--force-primary", is_flag=True, default=False, help="Make this model current")
@pass_config
@click.pass_context
@exception_handler
def commit_configs(
    ...
    spinner.start()
    model = config.mql.commit_configs(config_dir, force_primary)
    spinner.succeed(
