...
class BackendInterface(BaseInterface):  # noqa: D
    ...
    def _get_and_upload_configs(
        self, mode: str, config_dir: str, repo: str, branch: str, commit: str, is_current: Optional[bool] = None
    ) -> requests.Response:  # noqa: D
        try:
            return upload_configs(
                mode, self.auth_header, repo, branch, commit, config_dir, is_current, self.rest_api_url
            )
        except Exception as e:
    ...
    def commit_configs(self, config_dir: str, is_current: Optional[bool] = None) -> ModelKey:
        """Finds and validates yaml configs in the provided directory"""
        repo, branch, commit = resolve_local_commit_info(config_dir)
        r = self._get_and_upload_configs("commit", config_dir, repo, branch, commit, is_current)
