...
class Client(Singleton):
    ...
    def get(self, path, url, force_copy=False, min_size=0, timeout=60):
        if '://' in url:
    ...
    async def put_from_cloud_async(self, url, bucket, bucket_name, timeout=10, file_ttl=-1, mq=None):
        # save to cache
        task_id = 'tmp_file_{}_{}'.format(bucket, bucket_name)
    ...
    async def put_async(self, path, bucket, bucket_name, timeout=10, file_ttl=-1, mq=None):
        ret, e = self.put_to_cloud(path, bucket, bucket_name)
        if ret:
            ret, e = await self.put_from_cloud_async(e, bucket, bucket_name, timeout, file_ttl)
        return ret, e
    
    async def get_async(self, path, url, force_copy=False, min_size=0, timeout=10, mq=None):
        if '://' in url:
            ret, e = self.get_from_cloud(path, url, force_copy, min_size)
            return ret, e
