...
cache_engine = CacheManager(opt.redis_host, opt.redis_pass, opt.redis_port, opt.redis_db)
    ...
    with cache_engine.lock(key, ttl=30) as lock:
        if lock:
            try:
                ...
                ret, e = hmfs_client.get_from_cloud(file_path, url)
...
async def file_transfer_get(bucket, bucket_name):
    try:
        remote_state = await cache_engine.get_cache_async('tmp_file_{}_{}'.format(bucket, bucket_name))
