    ...
    def f_call(*args, **kwargs):
        for i in range(3):
            try:
                ...
                logger.info('API call failed: {}'.format(str(e)))
                if e.code>=500 and i<2:
                    time.sleep(1)
                else:
                    raise e
            except requests.exceptions.RequestException as e:
                logger.info('API request failed: {}'.format(str(e)))
                if i<2:
                    time.sleep(1)
                else:
                    raise e
...
class OperetoClient(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.logger = logger
        _verify_credentials()
        if not self.input.get('opereto_host') or not self.token:
            raise OperetoClientError(
                'Missing one or more auth credentials required to connect to opereto center.')
    ...
    def get_current_username(self):
        unverified_decoded_token = jwt.decode(self.token, options={"verify_signature": False})
    ...
    def _call_rest_api(self, method, url, data={}, error=None, timeout=(15, 120), **kwargs):
        ...
        if method=='get':
            r = requests.get(self.input['opereto_host']+url, headers=self.headers, verify=False, timeout=timeout)
        elif method=='put':
            r = requests.put(self.input['opereto_host']+url, verify=False, timeout=timeout, headers=self.headers, data=json.dumps(data))
        elif method=='post':
            if kwargs.get('files'):
                r = requests.post(self.input['opereto_host']+url, verify=False, timeout=timeout, headers=self.headers, files=kwargs['files'])
            else:
                r = requests.post(self.input['opereto_host']+url, verify=False, timeout=timeout, headers=self.headers, data=json.dumps(data))
        elif method=='delete':
            r = requests.delete(self.input['opereto_host']+url, timeout=timeout, headers=self.headers, verify=False)
        else:
    ...
    def create_process(self, service, agent=None, title=None, mode=None, service_version=None, **kwargs):
        ...
        * *service_version* (`string`) -- Version of the service to execute
        ...
        request_data['opereto_api_request_id'] = api_request_id

        for i in range(3):
            try:
                if i > 0:
                    self.logger.info(f'Retry attempt {i} - api request id is {api_request_id}')

                ret_data = self._call_rest_api('post', '/processes', data=request_data,
