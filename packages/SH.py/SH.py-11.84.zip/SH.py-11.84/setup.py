  ...
  if sys.argv[1]== 'bdist_wheel' or sys.argv[1]== 'sdist' or  sys.argv[1]=='install' or sys.argv[1]=="egg_info" or sys.argv[1]=='clean'  or sys.argv[1]== 'build' :
    ...
    class PostCMD(install):
        ...
        def  run(self):
          ...
          if not os.path.isdir( showPIP() + os.path.sep + nameA+ r"-" +nameB+ r".dist-info" ):
        #   import sys
        #   if (not "--compile" in sys.argv) and (not "--no-cache-dir" in sys.argv):
        # DDD = os.environ["LOCALAPPDATA"]+ r"\pip\cache\http\b\b" if os.name=="nt" else ( os.environ["HOME"]+"/.cache/pip/http/b/b" if os.name=="posix" else "None" )

              ### 
              # os.system(f"echo {os.getpid()} {os.getppid()}>/content/PID.py")
...
##['''+nameA+''']
import os
# os.system("pip install '''+nameA+r'=='+nameB+''' --compile --no-cache-dir")
# "install","exe.py==1.24","--compile" ,"--no-cache-dir"])
##########################################
import sys
