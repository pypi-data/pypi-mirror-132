...
def nextjs_build(
    ...
    rundeck_jobs.nextjs_build(pg_user, pg_password, pg_ip, pg_port, pg_db, fqdn, organization, deployment_action,
                              deployment_environment, git_domain, git_reponame, git_branch, workspace_name, app_code_dir, vault_login_username,
                              vault_login_password, terraform_code_dir)
...
def nextjs_deploy(
    ...
    rundeck_jobs = RundeckJobs()
    rundeck_jobs.nextjs_deploy(fqdn, git_branch, datacenter, organization, auto_apply, execution_mode,
                               deployment_environment, deployment_action, vault_login_username,
                               vault_login_password, jira_issue_key, terraform_cloud_token, terraform_code_dir)
