...
class Message(BaseModel):
    class Extensions(BaseModel):
        sticker:           Optional[Sticker]
        originalStickerId: Optional[str]
        replyMessage:      Optional[list[Reply]]
    ...
    def val_reply(val: dict) -> Optional[list[Reply]]:
        if val is None:
            return None

        reply_list = [Reply(**val)]
        sticker = val.get("sticker")
        stickerId = val.get("originalStickerId")

        while message := val.get("replyMessage"):
            reply_list.append(Reply(**message))
            val = message.get("extensions")
...
class GetMessages(BaseModel):
    class Paging(BaseModel):
        next: Optional[str] = Field(alias="nextPageToken")
        prev: Optional[str] = Field(alias="prevPageToken")

    messageList: Optional[list[Message]]
    paging:      Optional[Paging]


class GetBlogs(BaseModel):
    class Paging(BaseModel):
        next: Optional[str] = Field(alias="nextPageToken")
        prev: Optional[str] = Field(alias="prevPageToken")

    blogList: Optional[list[Message]]
    paging:  Optional[Paging]
