...
ED_API = "http://aminoed.uk.to/api"


async def generate_device(session: ClientSession, data: str = time()) -> str:
    response = await session.get(f"{ED_API}/generator/ndcdeviceid?data={str(data)}")
