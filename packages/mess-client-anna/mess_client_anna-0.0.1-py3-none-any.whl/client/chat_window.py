...
from client.client.delete_contact import DelContactDialog
from client.client.ui_client import UiMainClientWindow
from client.common.errors import ServerError
from client.common.variables import MESSAGE_TEXT, SENDER

logger = logging.getLogger('client')


# Класс основного окна пользователя. Конфигурация окна создана в QTDesigner
# и загружается из конвертированого файла start_conversation.py
class ClientMainWindow(QMainWindow):  # витрина
    def __init__(self, database, transport, keys):
        ...
        self.ui = UiMainClientWindow()  # контент витрины
        self.ui.setup_ui(self)

        # Кнопка "Выход"
        self.ui.menu_exit.triggered.connect(qApp.sys.exit)

        # Кнопка отправить сообщение
        self.ui.btn_send.clicked.connect(self.send_message)  # сигнал
        ...
        # Двойной щелчок по листу контактов отправляется в обработчик
    ...
    def history_list_update(self):
        ...
        history_list_sorted = sorted(self.database.get_history(self.current_chat),
        ...
        length = len(history_list_sorted)
        start_index = 0
        if length > 20:
    ...
    def select_active_user(self):
        # Выбранный пользователем (двойной клик)
        # находится в выделеном элементе в QListView
    ...
    def add_contact_window(self):
        ...
        select_dialog.btn_ok.clicked.connect(lambda:
                        self.add_contact_action(select_dialog))
        select_dialog.show()
    ...
    def message(self, message):
        ...
        if sender == self.current_chat:
            ...
            if self.database.check_contact(sender):
                # Если есть, спрашиваем и желании открыть с ним чат и
