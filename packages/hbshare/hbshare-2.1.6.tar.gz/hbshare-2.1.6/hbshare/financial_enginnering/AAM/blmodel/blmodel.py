...
from hbshare.financial_enginnering.XZ import db_engine
...
class BL_Model:
    ...
    def calculate_view_return(self,trade_month,asset_name):
        ...
        ret=((val_df['temp_pe'][-1:]/val_df['pe'][-1:]).pow(1/5)-1)*100+val_df['gxl'][-1:]+val_df['roe'][-1:]
    ...
    def load_asset_return_data(self,trade_date,asset_name,asset_type):
        ...
        if (asset_type == 'index'):
            ...
            # sql="select jzplfs,jjdm from st_fund.t_st_gm_jjxx where jjdm='{0}' ".format(asset_name)
            # method=self.hbdata.db2df(sql, db='funduser')['jzplfs'].values[0]
    ...
    def cov_mat_from_return(self,trade_date,asset_list,asset_type):
        ...
        for asset in asset_list:
            ...
            # cov_mat=pd.concat([cov_mat,dm.load_asset_return_data(trade_date_week,asset)],axis=1)
    ...
    def get_risk_aversion(self):
        ...
        # sql="select hb1y,tjyf from st_market.t_st_zs_yhb where zqdm='000012'"
    ...
    def get_weight(self,end_date,version,engine):

        # weight_list=[]
        # for asset in asset_list:
        #     sql="select zqdm,zsz from st_market.t_st_zs_hq where zqdm='{0}' and jyrq ={1} ".format(asset,end_date)
        #     weight_list.append(self.hbdata.db2df(sql,db='alluser')['zsz'][0])

        sql="select mkt_value from market_value where Date='{0}' and version='{1}'".format(end_date,version)
        weight_list=pd.read_sql(sql,con=engine)['mkt_value'].values.tolist()
    ...
    def bl_model_index_data_preparation(self, end_date, asset_list,version):
        ...
        trade_date_month = self.get_trade_date(end_date, fre='M')[-121:]
        ...
        # since we do not have the market value data from db, mannually input the market value here
        # mkt_val = [276394827983772, 186440976280608, 25632330283447,
        #         12915605543667,90605422793238,46137329457712]
        #
        # self.return2db(tabel_name='market_value',ret_list=mkt_val,
        #                engine=engine,asset_list=asset_list,end_date=end_date,version=version,col='mkt_value')

        # calculate the prio return based on the market value weights, COV matrix and risk aversion
        mkt_val=self.get_weight(end_date,version,engine)
        ...
        # write implied return into database
        self.return2db('implied_return',prio_return, engine, asset_list, end_date, version,'implied_ret')
        ...
        self.return2db('view_return', view_ret, engine, asset_list, end_date, version,'view_ret')
