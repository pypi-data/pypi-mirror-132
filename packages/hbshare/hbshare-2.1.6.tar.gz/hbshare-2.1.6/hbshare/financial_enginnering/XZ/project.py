...
class Projects:
    ...
    def bl_model(self,end_date,asset_list,version,tau,ub,lb,asset_type):
        ...
        prio_return = blm.read_return_fromdb(asset_list, engine, version, end_date, 'implied_return', 'implied_ret')

        # get the view_ret from local db
        view_ret = blm.read_return_fromdb(asset_list, engine, version, end_date, 'view_return', 'view_ret')
    ...
    # def bl_model_index(self,end_date,asset_list,version,tau,ub,lb):
    #
    #     from ..AAM.blmodel import  blmodel
    #
    #     blm=blmodel.BL_Model()
    ...
    #
    #     #get the cov matrix from local database
    #     cov_matrix=blm.read_cov_fromdb(engine,asset_list,version,end_date)
    #
    #     #the cov matrix of average return is a shrink of the cov matrix of return itself
    #     cov_matrix=tau*cov_matrix.values
    #
    ...
    # def bl_model_bond(self,end_date,asset_list):
    #
    #     from ..AAM.blmodel import  blmodel
    #
    #     blm=blmodel.BL_Model()
    ...
    #     trade_date_week=blm.get_trade_date(end_date=end_date,fre='W')[-(182+1):]
    #
    #     #calcualte the Cov matrix based on weekly return for the laset  3.5 years/ 182 weeks
    #     cov_matrix=blm.cov_mat_from_return(trade_date_week,asset_list,asset_type='public_fund')
    #
    #     #calcualte the risk_aversion
    #     risk_aversion=blm.get_risk_aversion()
    #
    #     #calculate the prio return based on the market value weights, COV matrix and risk aversion
    #     #mkt_val=blm.get_weight(end_date,asset_list)
    #     #since we do not have the market value data from db, mannually input the market value here
    #     # mkt_val=[277355929574535,189126278507110,26909992142220,46865702805647,12784724380556,
    #     #          7377133769326,91204018181995]
    #     mkt_val = [100]*len(asset_list)
    #
    #     prio_return=blm.implied_return_from_weight(mkt_val=mkt_val,risk_aversion=risk_aversion,
    #         asset_list=asset_list,cov_matrix=cov_matrix)
    #
    #     tau=0.05
    #     #the cov matrix of average return is a shrink of the cov matrix of return itself
    #     cov_matrix=tau*cov_matrix
    #
    #     # calculate the view return if necessary
    #     # use the last 2 years weekly data to calculate view return,it's simply the mean of the last two years return
    #     view_ret = []
    #     trade_date_week=trade_date_week[-(105+1):]
    #     for asset in asset_list:
    #         view_ret.append(blm.load_asset_return_data(trade_date_week,asset,'public_fund')['ret_'+asset].mean()*52)
    #
    #     views = np.eye(len(asset_list))
    #
    #     # calculate the view confidence if not read from database directlly
    ...
    #
    #     #set upper and lower bound
    #     lb=[-0.1]*len(asset_list)
    #     ub=[0.2]*len(asset_list)
    #     constrains=blm.set_boundary(len(asset_list), lb, ub)
    #
    #     opt_weight=blm.blm_solver(sigma=cov_matrix,mu=prio_return,P=views,Q=view_ret,
    #                          Omega=view_confidence,delta=risk_aversion,constrains=constrains)
