...
from hbshare.quant_research.Kevin.rm_associated.util.data_loader import EquityBrinsonAttributionLoader
from hbshare.quant_research.Kevin.rm_associated.util.verifier import verify_type, verify_trading_day, \
    verify_df_not_null, verify_simple_data_duplicate
from hbshare.quant_research.Kevin.rm_associated.config import industry_names
...
class EquityBrinsonAttribution:
    ...
    def _load_data(self):
        self.data_param = EquityBrinsonAttributionLoader(
            self.fund_id, self.benchmark_id, self.start_date, self.end_date).load()
    ...
    def _init_data_param(self):
        ...
        # 判断
        for trading_day in self.trading_day_list[:-1]:
...
if __name__ == '__main__':
    attr_result = EquityBrinsonAttribution(
        fund_id='001743', benchmark_id='000906', start_date='20170101', end_date='20210701').get_attribution_result()
