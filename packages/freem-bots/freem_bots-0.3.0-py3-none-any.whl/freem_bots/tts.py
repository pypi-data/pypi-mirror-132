...
class TTSVoicelinePart:
	...
	def _scrub_duplicate_voice_definitions(input_ssml: str) -> str:
		...
		input_text_arr = list(input_ssml)
	...
	async def _get_audio_bytes_azure(self, ssml: str, target_sample_rate: int) -> bytes:
		...
		async with ClientSession() as session:
			...
			async with session.post(
				...
				# have 48 kHz, need to resample to 96 kHz
				with io.BytesIO(response_bytes) as response_file:
					response_file.name = 'audio.wav'
					(response_audio, original_sample_rate) = soundfile.read(response_file)
					target_audio = librosa.resample(
						response_audio,
						orig_sr = original_sample_rate,
						target_sr = target_sample_rate,
					)
					with io.BytesIO() as target_bytes:
						target_bytes.name = 'audio.wav'
						soundfile.write(target_bytes, target_audio, target_sample_rate)
						target_bytes.seek(0)
						response_bytes_resampled = target_bytes.read()
						return response_bytes_resampled
