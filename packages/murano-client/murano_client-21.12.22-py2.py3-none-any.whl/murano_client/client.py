...
class HTTPWatchThread(StoppableThread):
    ...
    def process_response(self, response):
        ...
        if response.code in [304, 204]:
            ...
            LOG.critical(
                "Please verify your Murano IoT Connector resources are defined correctly.\n\n"
                "\tResources required: {}.\n\n"
    ...
    def run(self):
        ...
        while not can_connect and not self.is_stopped():
            try:
                # Pick a resource, test whether we can read/connect, don't process the response
                LOG.debug("Reading resource '{}' as connection test - ignoring response payload".format(self.watchlist[0]))
        ...
        for resource in self.watchlist:
            try:
                LOG.warning(
                    "Initialization (initial read) of resource: {}"
                    .format(resource)
                )
        ...
        while not self.is_stopped():
            # NOTE: Given responses (acknowledgements to the cloud) are passed-off for
            # processing in a separate thread, (re)starting a long-poll without giving
            # a chance for the acknowledgement to complete could result in a duplicate
            # processing

            # Adding short sleep to avoid retrieving the same value twice
            time.sleep(1)
            LOG.info("(Re)starting watch (long-poll) of: {}".format(self.watchlist))
...
class NoMemoryQueueMuranoClient(MuranoHTTP, MuranoMQTT):
    ...
    def watch(self, **kwargs):
        if kwargs.get('timeout'):
            ...
            LOG.info(
                "Read from Murano, putting into inbound queue: {}".format(from_murano))
            return from_murano
        else:
            while True:
    ...
    def __del__(self):
        # NOTE: Added to handle the diamond inheritance structure of this Class
        # Control which parent class deconstructor is ultimately called
        # (i.e. don't call __del__ on MuranoMQTT if no instance was never initialized)
        if self.outbound_protocol == 'mqtt':
            MuranoMQTT.__del__(self)
        elif self.outbound_protocol == 'https':
            MuranoHTTP.__del__(self)
