...
class Material(object):
    ...
    @fispact_material_atom_kilogram.setter
    def fispact_material_atom_kilogram(self, value):
        if not isinstance(value, list):
            raise ValueError('fispact_material_atom_kilogram should be a list')
        fraction_sum = 0.0
        for i in range(len(value)):
            if not isinstance(value[i], float) and not isinstance(value[i], int):
                raise ValueError(
                    'fispact_material_atom_kilogram should be a list of float/int')
        if len(self._fispact_material_nuclide) != 0 and len(value) != len(self._fispact_material_nuclide):
            raise ValueError(
                'the length of fispact_material_atom_kilogram should equals to the length of fispact_material_nuclide!')
        self._fispact_material_atom_kilogram = value
    ...
    def mcnp2fispact(self):
        ...
        Properties to be set:
            ...
            - fispact_material_atom_kilogram
        """
    ...
    def fispact2mcnp(self, ntrans_avail_nucs, record_drop=False,
                     record_file=None, record_file_total=None, iter_step=None):
        """
        Convert the mcnp material to fispact material
        ...
        for i, nuc in enumerate(self.fispact_material_nuclide):
            ...
            if nuc_m in ntrans_avail_nucs:
                ...
                total_atoms += self.fispact_material_atom_kilogram[i]
            else:
                drop_atoms += self.fispact_material_atom_kilogram[i]
                if record_file:
                    with open(record_file, 'a') as fo:
                        fo.write(
                            f"{nuc},{utils.format_single_output(self.fispact_material_atom_kilogram[i])}\n")
        # record the total dropped atoms
        if record_file_total:
            with open(record_file_total, 'a') as fo:
                fo.write(
                    f"{iter_step},{utils.format_single_output(drop_atoms)}\n")
        ...
        for i, nuc_m in enumerate(mcnp_nuclides):
            ...
            mcnp_atom_fraction.append(
                self.fispact_material_atom_kilogram[nidx]/total_atoms)
        # assign the value
    ...
    def treat_fispact_nuc_composition(self, nuc, level):
        ...
        self.fispact_material_atom_kilogram[nidx] *= level
        # update total_atoms
        total_atoms = 0.0
        for i, nuc in enumerate(self.fispact_material_atom_kilogram):
            total_atoms += self.fispact_material_atom_kilogram[i]
        # calculate atom density
        vol = 1e3 / self.density  # [cm3]
        self.atom_density = total_atoms * 1e-24 / vol
        # update mcnp_material_atom_fraction
        # for i, nuc_m in enumerate(self.mcnp_material_nuclide):
        #    nuc = nuc_mcnp2fispact(nuc_m)
        #    nidx = self.fispact_material_nuclide.index(nuc)
        #    self.mcnp_material_atom_fraction[i] = self.fispact_material_atom_kilogram[nidx]/total_atoms
