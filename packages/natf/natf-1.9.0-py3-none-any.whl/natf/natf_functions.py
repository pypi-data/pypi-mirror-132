...
from natf.mcnp_output import get_cell_neutron_flux, get_cell_names_from_line, \
    ...
    is_cell_info_start, get_cell_icl_cellid_matid_matdensity, \
...
@log
def get_fispact_materials(fispact_material_list, aim=None):
...
def get_radwaste_standards(config, verbose=False):
    ...
    if len(radwaste_standards) == 0:
        if verbose:
            print(f"WARNING: no radwaste standards used")
    else:
        if verbose:
...
def natf_cell_mat_evo(config):
    ...
    # [debug], optional
    try:
        moniter_cell = int(config.get('debug', 'moniter_cell'))
    except:
        moniter_cell = None
    ...
    # init tbr.csv
    tbr_file = 'tbr.csv'
    if not os.path.isfile(tbr_file):
        with open(tbr_file, 'w') as fo:
            fo.write("Operation time (MWY),TBR\n")
    # init record_file
    record_file_total = None
    if moniter_cell:
        record_file_total = f"c{moniter_cell}_drop.csv"
        if not os.path.isfile(record_file_total):
    ...
    if stat == 'POST' and current_step < total_steps - 1:
        ...
        if moniter_cell:
            record_file = os.path.join(
                step_dir, f"c{moniter_cell}_drop_ele.csv")
        ...
        for c in cells_need_cal:
            ...
            for i, cnl in enumerate(item_nuc_treatments):
                if item_nuc_treatments[i][0] == c.id:
                    mat.treat_fispact_nuc_composition(nuc=cnl[1], level=cnl[2])
            if c.id == moniter_cell:
                record_drop = True
            mat.fispact2mcnp(ntrans_avail_nucs, record_drop=record_drop,
                             record_file=record_file,
                             record_file_total=record_file_total,
