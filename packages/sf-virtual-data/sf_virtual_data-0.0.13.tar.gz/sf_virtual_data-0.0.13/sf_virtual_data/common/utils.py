...
def get_grpc_channel_according_to_args(args: models.VTCommandLineArgsBase) -> grpc.Channel:
    if args.insecure:
        channel = grpc.insecure_channel(args.execution_url)
    else:
        dir_path = os.path.dirname(os.path.realpath(__file__))
        # cert_path = os.path.join(dir_path, "api", args.cert)
        # with open(cert_path, 'rb') as f:
        #     trusted_certs = f.read()
        # credentials = grpc.ssl_channel_credentials(root_certificates=trusted_certs)
        credentials = grpc.ssl_channel_credentials()
        channel = grpc.secure_channel(args.execution_url, credentials)
    return channel
