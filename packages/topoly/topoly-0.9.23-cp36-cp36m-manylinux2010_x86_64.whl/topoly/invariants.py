...
class Invariant(Graph):
    ...
    def calculate(self, invariant, closure=Closure.TWO_POINTS, tries=200, direction=0, reduce_method=ReduceMethod.KMT,
                  ...
                  boundaries=None, hide_trivial=True, hide_rare=True, max_cross=15, matrix=False, density=1,                     level=0, cuda=True, matrix_plot=False, plot_ofile="KnotFingerPrintMap",
                  plot_format=PlotFormat.PNG, output_file='', matrix_cutoff=0.48, 
        ...
        if self.run_from_code:
        ...
        elif matrix and cuda and invariant.__name__ == 'AlexanderGraph' and check_cuda():
            ...
            result = self.calculate_matrix(invariant, closure=closure, tries=tries, direction=direction,
                        ...
                        level=level, run_parallel=run_parallel, parallel_workers=parallel_workers, debug=debug)
        ...
        if translate:
            result = translate_polvalues(result, invariant.name, translate=translate, chiral=chiral, 
                                         minimal=minimal, external_dictionary=external_dictionary)
    ...
    def calculate_from_code(self, invariant, max_cross=15, poly_reduce=True, debug=False):
...
class AlexanderGraph(Invariant):
    ...
    def point(self, max_cross=15, debug=False):
        ...
        if code in super().known[__class__.name].keys():
            ...
            #print("WANDA, point(), nieznany pdcode, policzony poly =", str(p_red), end="")
            if debug:
    ...
    def calc_cuda(self, closure=Closure.TWO_POINTS, tries=200, direction=0, reduce=ReduceMethod.KMT,
                       density=1, level=0, debug=False, translate=True, max_cross=15):
        matrix = find_alexander_fingerprint_cuda(self.coordinates_c[0], density, level, closure,
                                                 tries, False, max_cross)
