...
class baseWebBrowserController:
    ...
    def createSearchResultDataFrame(self, locater:str = By.CSS_SELECTOR) -> DataFrame:
        ...
        if self.config.ScrapingType == self.config.settingValueStruct.ScrapingType.selenium.value:
            ...
            # スクレイピング
            return self.cbCreateSearchResultDataFrameByWebDriver(self, element)
        elif self.config.ScrapingType == self.config.settingValueStruct.ScrapingType.beutifulSoup.value:
            # 待機
            self.Delay()
            # スクレイピング
            return self.cbCreateSearchResultDataFrameByBeutifulSoup(self, self.bSoup)

    @Logger.loggerDecorator('Create SearchResultDataFrame By WebDriver')
    def createSearchResultDataFrameByWebDriver(self, element) -> DataFrame:
    ...
    def createSearchResultDataFrameByBeutifulSoup(self, soup:BeautifulSoup) -> DataFrame:
