...
def behave_extract_bot_response_file(channel,bot_response):
    if channel == 'whatsapp':
        file_json = json.loads(bot_response[0]['text'])
...
def behave_extract_bot_response_image(channel,bot_response):
    if channel == 'whatsapp':
        image_json = json.loads(bot_response[0]['text'])
...
def behave_extract_bot_response_catalogue(channel,bot_response):
    if channel == 'whatsapp':
        ...
        if len(bot_response) > 1:
            for resp in bot_response:
                if resp['text'][0] == '{':
                    cat_json = json.loads(resp['text'])
                    for item in cat_json['items']:
                        bot_items.append(item)
                    break
        else:
            cat_json = json.loads(bot_response[0]['text'])
            for item in cat_json['items']:
                bot_items.append(item)           
        return bot_items  
...
def behave_extract_bot_response_generic_card(channel,bot_response):
    if channel == 'whatsapp':
        ...
        if len(bot_response) > 1:
            for resp in bot_response:
                if resp['text'][0] == '{':
                    gencard = json.loads(resp['text'])
                    bot_dict = gencard['mml']
                    break
        else:
            gencard = json.loads(bot_response[0]['text'])
            bot_dict = gencard['mml']           
        return bot_dict
