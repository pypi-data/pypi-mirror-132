...
async def school_meal(region_code: str, school_code:str, date: str) -> list[SchoolMealInfo]:
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=64, ssl=False)) as session:
        with open('KSchoolMeal/data/data.dat') as file:
            app_key = file.read()
        async with session.get('https://open.neis.go.kr/hub/mealServiceDietInfo', params={'KEY': app_key, 'Type': 'json', 'ATPT_OFCDC_SC_CODE': region_code, 'SD_SCHUL_CODE': school_code, 'MLSV_YMD': date}) as response:
