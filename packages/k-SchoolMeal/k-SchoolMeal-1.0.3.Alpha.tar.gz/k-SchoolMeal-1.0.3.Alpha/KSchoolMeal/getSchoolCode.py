...
async def school_code(school_name: str, region:Optional[str]=None) -> SchoolInfo:
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=64, ssl=False)) as session:
        async with session.get(os.getenv('school_info_base_url'), params=
            {'KEY': os.getenv('app_key'), 'Type': 'json', 'ATPT_OFCDC_SC_CODE': models.region_code(region), 'SCHUL_NM': school_name} if models.region_code(region) != None else {'KEY': os.getenv('app_key'), 'Type': 'json', 'SCHUL_NM': school_name}
                               ) as response:
