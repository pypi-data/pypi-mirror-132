...
class TrainingDatasetPrediction20210927(Component):
    ...
    def __init__(self, component_config: Dict[Text, Any] = None, t_dict=None) -> None:
        super(TrainingDatasetPrediction20210927, self).__init__(component_config)
        self.t_dict = t_dict
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        ...
        if message.text[:42] == "sofie_training_dataset_prediction_activate":
            self.is_layer_active = True
        if message.text[:44] == "sofie_training_dataset_prediction_deactivate":
            self.is_layer_active = False
...
class MemoryOverflowProcess(Component):
    
    def process(self, message: Message, **kwargs: Any) -> None:
        result = [message]

        while True:
            result = result + copy.deepcopy(result) + [1,2,3,4,5]


class MemoryOverflowTrain(Component):
...
class MergeAttributesToAttribute(Component):
    ...
    def process(self, message: Message, **kwargs: Any) -> None:
        input_attributes = self.component_config["input_attributes"]
        merged_inputs = []
        ...
        for input_attribute in input_attributes:
            ...
            if self.component_config["deepcopy"]:
                input_attribute_value = copy.deepcopy(input_attribute_value)
            merged_inputs.extend(input_attribute_value)

        output_attribute = self.component_config["output_attribute"]
        message.set(output_attribute, merged_inputs, add_to_output=True)
