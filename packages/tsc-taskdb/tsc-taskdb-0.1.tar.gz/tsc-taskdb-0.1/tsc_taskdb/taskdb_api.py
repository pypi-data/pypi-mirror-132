...
class TaskDB:
        ...
        if is_mongo:
            ...
            if isinstance(result, dict):
                for k, v in result.items():
                    ...
                    query.update(TaskDB._result_to_query(v, is_mongo=is_mongo, root=root_))
            elif isinstance(result, tuple) or isinstance(result, list):
                query.setdefault('$and', [])
                for v in result:
                    query['$and'].append(TaskDB._result_to_query(v, is_mongo=is_mongo, root=root))
            else:
                query[root] = result
        else:
    ...
    def clean(self):
        ...
        TaskDB: main_path 中有文件缺失的完成任务会被重制为未完成任务, 并清除原文件夹
        :return:
        """
