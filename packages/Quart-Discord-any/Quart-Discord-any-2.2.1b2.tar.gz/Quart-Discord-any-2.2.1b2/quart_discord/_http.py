...
class DiscordOAuth2HttpClient(abc.ABC):
    ...
    def __init__(self, app=None, client_id=None, client_secret=None, redirect_uri=None, bot_token=None, users_cache=None,
                 locks_cache=None):
    ...
    def init_app(self, app):
        ...
        self.locks_cache = self.locks_cache
        self.locksmith_lock = asyncio.Lock() if self.locks_cache is not None else None
        if not issubclass(self.users_cache.__class__, Mapping):
            raise ValueError("Instance users_cache must be a mapping like object.")
        if "http://" in self.redirect_uri:
            os.environ["OAUTHLIB_INSECURE_TRANSPORT"] = "true"
