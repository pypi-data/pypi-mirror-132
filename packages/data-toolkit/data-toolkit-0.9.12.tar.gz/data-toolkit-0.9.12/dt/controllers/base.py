...
class Base(Controller):
    ...
    def hist(self):
        if not self.app.pargs.s:
            ...
            print(f'Saving {n_lines} lines from history to snippet {snippet_name}')
    ...
    @ex(
        ...
        arguments=[
            ...
            (['--ls'],{
                "help" : "Whether to list all snippets at different levels (0,1,2)",
                "action" : "store"
            })
        ]
    )
    def run(self):
        is_save = self.app.pargs.save or False
        is_ls = self.app.pargs.ls or False
        ...
        if not is_save:
            ...
            run_command(n_lines)
        if is_ls:
            from ..ext.run import list_snippets
            list_snippets(int(self.app.pargs.ls))
        else:
            from ..ext.run import save_command
            n_lines = [int(x) for x in str(self.app.pargs.n).split(',')]
            save_command(n_lines)
    ...
    def config(self):
        ...
        if self.app.pargs.action=='loc':
            from ..ext.config import show_config_path
            show_config_path()
        ...
        elif self.app.pargs.action not in ['set','loc', 'show']:
