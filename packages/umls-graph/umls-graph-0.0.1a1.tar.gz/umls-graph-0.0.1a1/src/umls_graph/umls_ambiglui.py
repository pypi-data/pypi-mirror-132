...
def make_umls_ambiglui(mysql_info,save_path,save_folder=""):
    ...
    sql = "SELECT * from ambiglui"
    sql_count = "SELECT count(*) from ambiglui"

    # write file
    header = ":START_ID,:END_ID,:TYPE,LUI,CUI\n"
    ...
    for offset in range(0, count, batch_size):
        ...
        for row in cursor:
            ...
            # print results
            line = str(LUI) + "," + str(CUI) + ",\"" + str("AMBIGLUI") + "\",\"" + LUI + "\",\"" + CUI + "\""
            # print(line)
            fo.write(line + "\n")
            counter = counter + 1
...
if __name__=="__main__":
    ...
    make_umls_ambiglui(mysql_info,"../../examples/umls_datasets/umls_ambig_lui.csv")