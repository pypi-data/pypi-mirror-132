...
class UNetAmbisonic(nn.Module, Base):
    def __init__(
        self, input_channels: int, output_channels: int, target_sources_num: int
    ):
        r"""UNet."""
        super(UNetAmbisonic, self).__init__()
    ...
    def forward(self, input_dict: Dict) -> Dict:
        ...
        _mag = mag[:, 0:1, :, :].repeat(1, 2, 1, 1)
        _sin_in = sin_in[:, 0:1, :, :].repeat(1, 2, 1, 1)
        _cos_in = cos_in[:, 0:1, :, :].repeat(1, 2, 1, 1)
        separated_audio = self.feature_maps_to_wav(
            x, _mag, _sin_in, _cos_in, audio_length
        )
...
class UNetInputPhaseAmbisonic(nn.Module, Base):
    def __init__(
        self, input_channels: int, output_channels: int, target_sources_num: int
    ):
        r"""UNet."""
        super(UNetInputPhaseAmbisonic, self).__init__()
    ...
    def forward(self, input_dict: Dict) -> Dict:
        ...
        '''
        real = (mag * cos_in).data.cpu().numpy()[0, 0].T
        imag = (mag * sin_in).data.cpu().numpy()[0, 0].T
        mag = mag.data.cpu().numpy()[0, 0].T
        
        fig, axs = plt.subplots(3,1, sharex=True, figsize=(10, 8))
        vmax = np.max(np.log(mag))
        vmin = np.min(np.log(mag))
        axs[0].matshow(np.log(mag), origin='lower', aspect='auto', cmap='jet', vmin=vmin, vmax=vmax)
        axs[1].matshow(np.log(real), origin='lower', aspect='auto', cmap='jet', vmin=vmin, vmax=vmax)
        axs[2].matshow(np.log(imag), origin='lower', aspect='auto', cmap='jet', vmin=vmin, vmax=vmax)
        axs[0].grid(linestyle='solid', linewidth=0.3)
        axs[1].grid(linestyle='solid', linewidth=0.3)
        axs[2].grid(linestyle='solid', linewidth=0.3)
        plt.savefig('_zz.pdf')
        '''
        x = torch.cat((mag, mag * cos_in, mag * sin_in), dim=1)
...
class UNetInputDeltaPhaseAmbisonic(nn.Module, Base):
    def __init__(
        self, input_channels: int, output_channels: int, target_sources_num: int
    ):
        r"""UNet."""
        super(UNetInputDeltaPhaseAmbisonic, self).__init__()
    ...
    def get_delta_phase(self, cos_mix, sin_mix, cos_1, sin_1):

        cos_delta = cos_mix * cos_1 + sin_mix * sin_1
        sin_delta = sin_mix * cos_1 - cos_mix * sin_1
    ...
    def forward(self, input_dict: Dict) -> Dict:
        ...
        _cos1, _sin1 = self.get_delta_phase(
            cos_in[:, 0:1, :, :],
            sin_in[:, 0:1, :, :],
            cos_in[:, 1:2, :, :],
            sin_in[:, 1:2, :, :],
        )
        _cos2, _sin2 = self.get_delta_phase(
            cos_in[:, 0:1, :, :],
            sin_in[:, 0:1, :, :],
            cos_in[:, 2:3, :, :],
            sin_in[:, 2:3, :, :],
        )
        _cos3, _sin3 = self.get_delta_phase(
            cos_in[:, 0:1, :, :],
            sin_in[:, 0:1, :, :],
            cos_in[:, 3:4, :, :],
            sin_in[:, 3:4, :, :],
        ...
        _real1 = mag[:, 1:2, :, :] * _cos1
        _imag1 = mag[:, 1:2, :, :] * _sin1
        _real2 = mag[:, 2:3, :, :] * _cos2
        _imag2 = mag[:, 2:3, :, :] * _sin2
        _real3 = mag[:, 3:4, :, :] * _cos3
        _imag3 = mag[:, 3:4, :, :] * _sin3

        '''
        mag = mag.data.cpu().numpy()[0, 0].T
        real = _real1.data.cpu().numpy()[0, 0].T
        imag = _imag1.data.cpu().numpy()[0, 0].T
        ...
        x = torch.cat((mag, _cos1, _sin1, _cos2, _sin2, _cos3, _sin3), dim=1)
...
class DnnAmbisonic(nn.Module, Base):
    def __init__(
        self, input_channels: int, output_channels: int, target_sources_num: int
    ):
        r"""UNet."""
        super(DnnAmbisonic, self).__init__()
        ...
        self.fc1 = nn.Linear((window_size // 2 + 1) * 4, 1024)
        self.fc2 = nn.Linear(1024, 1024)
        self.fc3 = nn.Linear(1024, 1024)
        self.fc4 = nn.Linear(1024, (window_size // 2 + 1) * 2 * 3)
    ...
    def forward(self, input_dict: Dict) -> Dict:
        ...
        x = x.reshape(
            batch_size, time_steps, self.output_channels * 3, self.window_size // 2 + 1
        )
        x = x.transpose(1, 2)
...
class RnnAmbisonic(nn.Module, Base):
    def __init__(
        self, input_channels: int, output_channels: int, target_sources_num: int
    ):
        r"""UNet."""
        super(RnnAmbisonic, self).__init__()
        ...
        self.fc_final = nn.Linear(1024, (window_size // 2 + 1) * 2 * 3)
