class _ResUNet143_Subbandtime(nn.Module, Base):
    def __init__(self, input_channels: int, output_channels, target_sources_num: int):
        super(_ResUNet143_Subbandtime, self).__init__()
    ...
    def feature_maps_to_wav(
        ...
        # x: (batch_size, target_sources_num, input_channles, self.K, time_steps, freq_bins)
    ...
    def forward(self, input_dict):
        ...
        C1 = (
            x.shape[1] // self.subbands_num
        )  # target_sources_num * input_channels * self.K
        C2 = mag.shape[1] // self.subbands_num  # input_channels

        separated_subband_audio = torch.cat(
            [
                self.feature_maps_to_wav(
                    input_tensor=x[:, j * C1 : (j + 1) * C1, :, :],
                    # input_tensor: (batch_size, target_sources_num * input_channels * self.K, T, F')
                    sp=mag[:, j * C2 : (j + 1) * C2, :, :],
