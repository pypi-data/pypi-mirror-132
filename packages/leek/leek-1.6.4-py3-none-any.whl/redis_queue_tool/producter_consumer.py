...
# 配置连接信息
try:
    import redis_queue_tool_config as leek_config
    ...
    logger.info('读取到redis_queue_tool.py redis配置')
except ModuleNotFoundError:
    logger.warning('未读取到redis_queue_tool.py redis配置.')
...
try:
    ...
    logger.info('读取到redis_queue_tool.py kafka配置,使用自定义配置')
except ModuleNotFoundError:
    logger.warning('未读取到redis_queue_tool.py kafka配置.')
except AttributeError:
    logger.warning('未读取到redis_queue_tool.py kafka配置。')
...
class RedisCustomer(object):
    ...
    def __init__(self, queue_name, consuming_function: Callable = None, process_num=1, threads_num=50,
        ...
        if middleware == MiddlewareEum.SQLITE:
            ...
            self._redis_quenen = RedisQueue(queue_name, priority=priority,
