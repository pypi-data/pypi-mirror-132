...
# @Last Modified time: 2021-12-27 16:21:10
...
class ThirdInfo(object):
	...
	def get_container_ip(self,service_name,env="uat",tag="group1"):
		...
		try:
			...
			if env == "fat":
				...
				groupsUrl = "https://hke.hellobike.cn/container-business-service/open-api/v2/app/{}/fat/groups".format(service_name)
			elif env == "pre":
				# groupsUrl = "https://hke.hellobike.cn/container-business-service/api/v1/apps/groups/appname/{}/env/9".format(service_name)
				groupsUrl = "https://hke.hellobike.cn/container-business-service/open-api/v2/app/{}/pre/groups".format(service_name)
				
			else:
				# groupsUrl = "https://hke.hellobike.cn/container-business-service/api/v1/apps/groups/appname/{}/env/2".format(service_name)
				groupsUrl = "https://hke.hellobike.cn/container-business-service/open-api/v2/app/{}/uat/groups".format(service_name)
			...
			if env == "fat":
				...
				containerUrl = "https://hke.hellobike.cn/container-business-service/open-api/v1/app/{service_name}/fat/{tag}/pods".format(
				service_name=service_name,tag=tag)
			elif env == "pre":
				...
				# service_name=service_name,tag=tag)
				containerUrl = "https://hke.hellobike.cn/container-business-service/open-api/v1/app/{service_name}/pre/{tag}/pods".format(
				service_name=service_name,tag=tag)
			else:
				...
				# service_name=service_name,tag=tag)
				containerUrl = "https://hke.hellobike.cn/container-business-service/open-api/v1/app/{service_name}/uat/{tag}/pods".format(
				service_name=service_name,tag=tag)
			...
			if cnRep[0] == 200:
				try:
					ip = cnRep[1].get('data')[0].get('podList',[])[0].get("ipAddress")
					print(ip)
				except Exception as e:
					print(traceback.format_exc())
					print("异常返回为:",str(cnRep))
					raise Exception("获取ip返回异常")
				return ip
	...
	def exe_redis(self,name,command,env="uat",database=None):
		...
		try:
			...
			print('allRedis',str(allRedisRep))
			...
			print("redis:",str(getSelfRedisRep))
	...
	def get_server_port(self,service_name,env="pre"):
		...
		"""
		print(env)
		requestUrl = "https://tt.hellobike.cn/v1/gateway"
		data = {"env": env.upper(), "action": "tt.soa.manage.group_list", "app_name": service_name}
		# if env == "pre":
		...
		# 	requestUrl = "http://hke.hellobike.cn/container-business-service/api/v1/soa/2/{service_name}/list".format(service_name=service_name)
		# elif env == "fat":
		# 	requestUrl = "http://hke.hellobike.cn/container-business-service/api/v1/soa/6/{service_name}/list".format(service_name=service_name)
		# else:
		# 	return "你输入的环境名不正确"
		...
		rep = Request().request_client(url=requestUrl,method='post',data=data,headers=headers)
		# print (rep)
		if rep[0] == 200:
			data = rep[1]['data']
			if data:
...
if __name__ == '__main__':
	td = ThirdInfo()
	print(td.get_container_ip("AppHellobikeCardCoreService"))
	...
	# print(td.get_server_port("AppEvBizProcessService",env="pre"))
