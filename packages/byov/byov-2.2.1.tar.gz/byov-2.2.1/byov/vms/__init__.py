...
class VM(object):
    ...
    def setup_over_ssh(self):
        """Once ssh is setup, packages can be installed and scripts run."""
        # Save the defined config. Errors after that point should not break the
        # vm hard enough that one cannot ssh to it and debug.
        self.save_existing_config()
        # Finish the setup
        self.update()
        self.install_packages()
        # Either update() or install_packages() can trigger a reboot
        check_reboot_command = [
            'test', '-f', '/var/run/reboot-required']
        try:
            subprocesses.run(self.get_ssh_command(*check_reboot_command))
    ...
    def shell(self, command, *args):
        if command and command.startswith('@'):
            retcode = self.run_script(command[1:], args=args, captured=False)
        else:
            ssh_command = self.get_ssh_command(command, *args)
            retcode = subprocesses.raw_run(ssh_command)
        return retcode
    ...
    def install_packages(self):
        ...
        install_command = ['install'] + self.conf.get('vm.packages')
        try:
            self.do_apt_get(install_command)
        except errors.CommandError:
    ...
    def run_script(self, script_path, args=None, captured=True):
        ...
        logger.info('Running {}{} on {}'.format(
            script_path, largs, self.conf.get('vm.name')))
        ...
        try:
            ...
            if captured:
                ...
                retcode = self.shell(remote_path, *args)
        finally:
            os.remove(tmp.name)
    ...
    def _wait_for_cloud_init(self, timeouts_name):
        ...
        # ssh to check, there is no race around the deletion.
        check_cloud_init_command = ['test', '-f',
                                    '/var/lib/cloud/instance/boot-finished']
        ssh_command = self.get_ssh_command(*check_cloud_init_command)
        for attempt, sleep in enumerate(citos, start=1):
            try:
                if attempt > 1:
                    logger.debug('Re-trying {} {}/{}'.format(
                        me, attempt, citos.retries))
                subprocesses.run(ssh_command)
