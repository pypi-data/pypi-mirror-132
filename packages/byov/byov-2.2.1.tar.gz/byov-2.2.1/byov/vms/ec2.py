...
class Ec2Client(object):
    ...
    def create_server(self, name, itype, image_id, user_data, sec_groups,
                      subnet):
        # Surprisingly setting the name is the hardest part...
        tag_spec = dict(ResourceType='instance',
...
class Ec2Server(vms.VM):
    ...
    def find_ec2_image(self, image_id=None):
        ...
        # We want to select an image either by its AMI id or from a combination
        # distribution/release/arch (taking the most recent)
        image_re = re.compile(self.conf.get('ec2.image'))
