...
class LxdUserMountsOption(options.ListOption):
    ...
    def __init__(self, *args, **kwargs):
        # Foce invalid values to be an error (and forbids overriding it) to
        # catch invalid file names
        super(LxdUserMountsOption, self).__init__(
            *args, invalid='error', **kwargs)

    def from_unicode(self, string):
        values = super(LxdUserMountsOption, self).from_unicode(string)
        if not values:
            return values
        converted = []
        for pair in values:
...
'''))
register(options.Option('vm.user.shell', default=None,
                        help_string='''\
...
register(TimeoutsOption('qemu.cloud_init.timeouts', default='0, 120, 10',
                        help_string='''\
When waiting for cloud-init completion.'''))
...
ec2_image_re_registry.register(
    'amazon',
    '^{vm.release}-ami-ecs-hvm-.*-ebs',
    'Amazon Linux 2 images AMI regexp.')
ec2_image_re_registry.register(
    'debian',
...
register(TimeoutsOption('ec2.boot.timeouts', default='10, 600, 60',
                        help_string='''\
...
register(TimeoutsOption('ec2.create_image.timeouts', default='0, 240, 20',
                        help_string='''\
...
register(options.ListOption(
    'apt.command',
    default='sudo, DEBIAN_FRONTEND=noninteractive, apt-get, {apt.options}',
