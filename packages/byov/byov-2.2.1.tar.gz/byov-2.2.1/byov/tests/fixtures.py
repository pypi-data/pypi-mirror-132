...
# (after isolation). 2) At least lxd and qemu backends co-exist rather than
...
def setup_tests_config(test):
    ...
    isolate_from_disk(test)  # calls isolate_from_env
...
def override_logging(test, debug=False):
...
vm.name = {vm_name}
vm.class = {kls}
'''.format(vm_name=test.vm_name, kls=test.kls))
    ...
    def ec2_setup(test, conf):
        ...
        # when the used one become unavailable -- vila 2021-12-03
        conf.set('ec2.distribution.images', '{vm.distribution}')
        conf.set('ec2.image', '{ec2.distribution.images}')
        conf.set('ec2.image.id', 'ami-04505e74c0741db8d')
