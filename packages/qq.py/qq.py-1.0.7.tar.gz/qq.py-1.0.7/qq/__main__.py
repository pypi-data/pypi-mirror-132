...
def show_version():
    ...
    version_info = qq.version_info
    entries.append('- qq.py v{0.major}.{0.minor}.{0.micro}-{0.releaselevel}'.format(version_info))
    if version_info.releaselevel != 'final':
        pkg = pkg_resources.get_distribution('qq.py')
        if pkg:
            entries.append(f'    - qq.py pkg_resources: v{pkg.version}')
...
_cog_template = '''from qq.ext import commands
import qq
...
def add_newbot_args(subparser):
    parser = subparser.add_parser('newbot', help='快速创建命令机器人项目')
    parser.set_defaults(func=newbot)

    parser.add_argument('name', help='机器人项目名称')
    parser.add_argument('directory', help='放置它的目录（默认：.）', nargs='?', default=Path.cwd())
    parser.add_argument('--prefix', help='机器人前缀（默认：$）', default='$', metavar='<prefix>')
    parser.add_argument('--sharded', help='是否使用 AutoShardedBot', action='store_true')
    parser.add_argument('--no-git', help='不要创建 .gitignore 文件', action='store_true', dest='no_git')


def add_newcog_args(subparser):
    parser = subparser.add_parser('newcog', help='快速创建一个新的 cog 模板')
    parser.set_defaults(func=newcog)

    parser.add_argument('name', help='齿轮名称')
    parser.add_argument('directory', help='放置它的目录（默认：cogs）', nargs='?',
                        default=Path('cogs'))
    parser.add_argument('--class-name', help='cog 的类名（默认：<name>）', dest='class_name')
    parser.add_argument('--display-name', help='齿轮名称（默认值：<name>）')
    parser.add_argument('--hide-commands', help='是否隐藏 cog 中的所有命令', action='store_true')
    parser.add_argument('--full', help='添加所有特殊方法', action='store_true')


def parse_args():
    parser = argparse.ArgumentParser(prog='qq', description='qq.py 帮助工具')
    parser.add_argument('-v', '--version', action='store_true', help='显示库版本')
