...
class IngestHistory:
    ...
    def list(self):
        return [
            Ingest(self._stub, self._feature_set, ingest)
            for ingest in self._ingest_history
        ]
    ...
    def get_by_ingest_number(self, ingest_number):
        ingests = [
            ingest
            for ingest in self._ingest_history
            if ingest.ingest_number == ingest_number
        ]
        if ingests:
            return Ingest(self._stub, self._feature_set, ingests[0])
        else:
            raise Exception(
                "No ingest has been found for the ingest number " + ingest_number
            )

    def get_by_ingest_id(self, ingest_id):
        ingests = [
            ingest for ingest in self._ingest_history if ingest.ingest_id == ingest_id
        ]
        if ingests:
            return Ingest(self._stub, self._feature_set, ingests[0])
        else:
